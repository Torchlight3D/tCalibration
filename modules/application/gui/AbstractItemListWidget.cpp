    ...
    : QWidget(parent)
{
    ...
    _focusFrame->setRadiuses(theStyle ? theStyle->theme().borderRadius : 0);
    _focusFrame->setWidget(this);

    // Badge.
    _badgeFont =
        theStyle ? theStyle->fontForTextRole(TextRole::Caption) : this->font();
    _badgeFont.setBold(true);
}

AbstractItemListWidget ::~AbstractItemListWidget() = default;

int AbstractItemListWidget::itemCount() const { return _items.size(); }

int AbstractItemListWidget::currentIndex() const { return _currentIndex; }
...
int AbstractItemListWidget::addItem(const QString& text, const QIcon& icon,
                                    const QString& badge, const QVariant& data)
{
    const auto* style = this->style();
    const auto animDuration =
        style->styleHint(QStyle::SH_Widget_Animation_Duration) *
        animationFactor;

    auto* bgColorAnimation = new QVariantAnimation(this);
    QObject::connect(bgColorAnimation, &QVariantAnimation::valueChanged, this,
                     [this]() { update(); });

    auto* fgColorAnimation = new QVariantAnimation(this);
    QObject::connect(fgColorAnimation, &QVariantAnimation::valueChanged, this,
                     [this]() { update(); });

    auto* badgeBgColorAnimation = new QVariantAnimation(this);
    QObject::connect(badgeBgColorAnimation, &QVariantAnimation::valueChanged,
                     this, [this]() { update(); });

    auto* badgeFgColorAnimation = new QVariantAnimation(this);
    QObject::connect(badgeFgColorAnimation, &QVariantAnimation::valueChanged,
                     this, [this]() { update(); });
    ...
    const auto [bgColor, fgColor, badgeBgColor, badgeFgColor] =
        getItemBgAndFgColor(_items.size(), MouseState::Normal);
    ...
    badgeBgColorAnimation->setStartValue(
        QVariant::fromValue<QColor>(badgeBgColor));
    badgeBgColorAnimation->setEndValue(
        QVariant::fromValue<QColor>(badgeBgColor));
    badgeBgColorAnimation->setDuration(animDuration);
    badgeBgColorAnimation->setEasingCurve(QEasingCurve::Type::InOutCubic);

    badgeFgColorAnimation->setStartValue(
        QVariant::fromValue<QColor>(badgeFgColor));
    badgeFgColorAnimation->setEndValue(
        QVariant::fromValue<QColor>(badgeFgColor));
...
int AbstractItemListWidget::findItemIndex(const QVariant& data) const
{
    ...
    const auto it =
        std::find_if(_items.constBegin(), _items.constEnd(),
                     [&data](const auto& item) { return item.data == data; });
    return it != _items.constEnd()
               ? static_cast<int>(std::distance(_items.constBegin(), it))
               : -1;
}
...
{
    setCurrentIndex(
        std::min(_currentIndex + 1, static_cast<int>(_items.size()) - 1));
}
...
const QSize& AbstractItemListWidget::iconSize() const { return _iconSize; }
...
bool AbstractItemListWidget::hitTestItemRect(const QPoint& pos,
                                             const QRect& itemRect,
                                             int leftPadding,
                                             int rightPadding) const
{
    // To make it easier for the user to click, we take into account the padding
    // around the item.
    return pos.x() >= itemRect.x() - leftPadding // Padding on the left.
           && pos.x() <= itemRect.x() + itemRect.width() +
                             rightPadding          // Padding on the right.
...
void AbstractItemListWidget::keyPressEvent(QKeyEvent* e)
{
    ...
    if (key == Qt::Key_Right) {
        const auto next =
            std::min(static_cast<int>(_items.size()) - 1, _focusedIndex + 1);
        ...
        e->accept();
             ...
             (key == Qt::Key_Space || key == Qt::Key_Enter ||
              key == Qt::Key_Return)) {
...
void AbstractItemListWidget::keyReleaseEvent(QKeyEvent* e)
{
    ...
    setPressedIndex(-1);
    const auto key = e->key();
    if (!e->isAutoRepeat() && (key == Qt::Key_Space || key == Qt::Key_Enter ||
                               key == Qt::Key_Return)) {
...
{
    // Make a first pass to ask every item how much space they need if space was
    // unlimited.
...
void AbstractItemListWidget::updateItemsSizeHints()
{
    ...
    const auto badgeH =
        badgeFm.height() + badgePadding.top() + badgePadding.bottom();
    const auto itemH = std::max({textH, iconH, badgeH}) + itemPadding.top() +
                       itemPadding.bottom();
    ...
    for (auto& item : _items) {
        ...
        if (!item.text.isEmpty()) {
            itemW += textWidth(fm, item.text);
            ++elementCount;
        }

        if (!item.badge.isEmpty()) {
            const auto badgeW =
                std::max(badgeH, badgePadding.left() + badgePadding.right() +
                                     textWidth(badgeFm, item.badge));
            itemW += badgeW;
            ++elementCount;
        }
...
void AbstractItemListWidget::updateItemRects()
{
    ...
    // Compute theoritical necessary width to display items in their necessary
    // size.
    ...
    if (itemsNecessaryW > availableItemsRect.width()) {
        ...
        const auto itemW =
            itemCount > 0
                ? (availableItemsRect.width() - (itemCount - 1) * spacing) /
                      itemCount
                : 0;
        ...
        // Enough space: we can increase each items widths with the superfluous
        // with (very basic algorithm...).
        ...
        // Enough space but we keep the items centered with their necessary
        // size.
        const auto centeredItemsRectX =
            availableItemsRect.x() +
            (availableItemsRect.width() - itemsNecessaryW) / 2;
        const auto centeredItemsRect =
            QRect{centeredItemsRectX, itemY, itemsNecessaryW, itemH};
...
MouseState AbstractItemListWidget::getItemMouseState(int index,
...
std::tuple<QColor, QColor, QColor, QColor>
AbstractItemListWidget::getItemBgAndFgColor(int index, MouseState mouse) const
{
    const auto* theStyle = qobject_cast<const QlementineStyle*>(style());
    const auto& palette = this->palette();
    const auto& itemBgColor = theStyle
                                  ? getItemBgColor(mouse, theStyle->theme())
                                  : getItemBgColor(mouse, palette);
    const auto& itemFgColor =
        theStyle
            ? getItemFgColor(mouse, index == _currentIndex, theStyle->theme())
            : getItemFgColor(mouse, index == _currentIndex, palette);
    const auto& badgeBgColor =
        theStyle ? getItemBadgeBgColor(mouse, index == _currentIndex,
                                       theStyle->theme())
                 : getItemBadgeBgColor(mouse, index == _currentIndex, palette);
    const auto& badgeFgColor =
        theStyle ? getItemBadgeFgColor(mouse, index == _currentIndex,
                                       theStyle->theme())
...
const QColor& AbstractItemListWidget::getCurrentItemIndicatorColor() const
{
    const auto* theStyle = qobject_cast<const QlementineStyle*>(style());
    if (theStyle) {
        return theStyle->buttonBackgroundColor(
            isEnabled() ? MouseState::Normal : MouseState::Disabled,
            ColorRole::Primary);
    }

    return palette().color(isEnabled() ? QPalette::Normal : QPalette::Disabled,
...
QRect AbstractItemListWidget::getCurrentItemRect() const
{
    ...
    const auto padding = getPadding();
    return {padding.left(), padding.top(), 0,
            height() - padding.top() - padding.bottom()};
}

QRect AbstractItemListWidget::getFocusedItemRect() const
{
...
QRect AbstractItemListWidget::getAnimatedCurrentItemRect() const
{
...
void AbstractItemListWidget::updateCurrentIndexAnimation(bool immediate)
{
    ...
    if (immediate || !_currentIndexAnimation.currentValue().isValid() ||
        !_currentIndexAnimation.startValue().isValid() || !_firstShow) {
        _currentIndexAnimation.setStartValue(currentItemRect);
    }
    else {
        _currentIndexAnimation.setStartValue(
            _currentIndexAnimation.currentValue());
    }
...
void AbstractItemListWidget::updateItemsAnimations()
{
    for (auto i = 0; i < _items.size(); ++i) {
        ...
        const auto [itemBgColor, itemFgColor, badgeBgColor, badgeFgColor] =
            getItemBgAndFgColor(i, itemMouse);
        item.bgColorAnimation->stop();
        item.fgColorAnimation->stop();
        item.badgeBgAnimation->stop();
        item.badgeFgAnimation->stop();

        item.bgColorAnimation->setStartValue(
            item.bgColorAnimation->currentValue().value<QColor>());
        item.fgColorAnimation->setStartValue(
            item.fgColorAnimation->currentValue().value<QColor>());
        item.badgeBgAnimation->setStartValue(
            item.badgeBgAnimation->currentValue().value<QColor>());
        item.badgeFgAnimation->setStartValue(
            item.badgeFgAnimation->currentValue().value<QColor>());

        item.bgColorAnimation->setEndValue(
            QVariant::fromValue<QColor>(itemBgColor));
        item.fgColorAnimation->setEndValue(
            QVariant::fromValue<QColor>(itemFgColor));
        item.badgeBgAnimation->setEndValue(
            QVariant::fromValue<QColor>(badgeBgColor));
        item.badgeFgAnimation->setEndValue(
            QVariant::fromValue<QColor>(badgeFgColor));
...
const QFont& AbstractItemListWidget::badgeFont() const { return _badgeFont; }

const QRect& AbstractItemListWidget::actualRect() const { return _actualRect; }

void AbstractItemListWidget::initStyleOptionFocus(
    QStyleOptionFocusRoundedRect& opt) const
{
    const auto* style = this->style();
    const auto deltaX =
...
void AbstractItemListWidget::showEvent(QShowEvent* e)
{
    ...
    if (!_firstShow) {
        ...
        QObject::connect(&_currentIndexAnimation,
...
{
    const auto* theStyle = qobject_cast<const QlementineStyle*>(style());
    return theStyle ? getBgColor(theStyle->theme()) : getBgColor(palette());
}
    ...
    MouseState mouse, bool /*selected*/, const QPalette& palette) const
{
    return palette.color(
        mouse != MouseState::Disabled ? QPalette::Normal : QPalette::Disabled,
        QPalette::Dark);
}
...
void AbstractItemListWidget::drawItemBackground(QPainter& p,
                                                const Item& item) const
{
...
void AbstractItemListWidget::drawItemForeground(QPainter& p,
                                                const Item& item) const
{
    const auto itemPadding = getItemPadding();
    const auto itemSpacing = getItemSpacing();
    ...
    const auto itemContentX =
        itemRect.x() + (itemRect.width() - itemContentW) / 2;
    const auto itemContentY = itemRect.y();
    const auto itemContentRect = QRect{QPoint{itemContentX, itemContentY},
                                       QSize{itemContentW, itemContentH}};
    const auto itemFgColor =
        item.fgColorAnimation->currentValue().value<QColor>();
    ...
    auto availableW =
        itemContentRect.width() - itemPadding.left() - itemPadding.right();

    // Compute rects.
    const auto iconX = availableX;
    const auto iconY = itemContentRect.y() +
                       (itemContentRect.height() - iconSize.height()) / 2;
    ...
    const auto badgeX = itemContentRect.x() + itemContentRect.width() -
                        itemPadding.left() - badgeW;
    const auto badgeY =
        itemContentRect.y() + (itemContentRect.height() - badgeH) / 2;
    ...
    const auto badgeTakenSpace =
        showBadge ? badgeRect.width() + itemSpacing : 0;
    ...
    const auto showText =
        ((!hasIcon && !hasBadge) || hasText) &&
        (showIcon ? availableW > itemSpacing * 3 : itemRect.width() > 0);

    if (showIcon && !showText) {
        if (showBadge) {
            // Center icon and badge.
            const auto newItemContentW =
                iconSize.width() + itemSpacing + badgeSize.width();
            const auto itemContentX =
                itemContentRect.x() +
                (itemContentRect.width() - newItemContentW) / 2;
            iconRect = QRect{QPoint{itemContentX, iconY}, iconSize};
            badgeRect = QRect{
                QPoint{itemContentX + iconSize.width() + itemSpacing, badgeY},
                badgeSize};
        }
        else {
            // Center icon.
            const auto iconX = itemContentRect.x() +
                               (itemContentRect.width() - iconSize.width()) / 2;
        ...
        if (showBadge) {
            if (badgeRect.width() >
                std::max(static_cast<int>(itemContentRect.width() / 2.5),
                         itemSpacing * 4)) {
                // Don't show badge, give priority to text.
                showBadge = false;
                textRect = itemRect.marginsRemoved(
                    {itemSpacing / 2, 0, itemSpacing / 2, 0});
                if (textRect.width() > itemSpacing * 4) {
                    textRect = textRect.marginsRemoved(
            ...
            if (textRect.width() > itemSpacing * 4) {
                textRect =
                    textRect.marginsRemoved({itemSpacing, 0, itemSpacing, 0});
            }
        }
    }
    ...
    if (showIcon) {
        const auto pixmap =
            item.icon.pixmap(iconSize.height(), QIcon::Normal, QIcon::Off);
        const auto coloredPixmap = qimg::colorizePixmap(pixmap, itemFgColor);
        // Draw image.
...
bool AbstractItemListWidget::shouldDrawItemBgForCurrent() const { return true; }

QFont AbstractItemListWidget::labelFont() const { return this->font(); }
...
QMargins AbstractItemListWidget::getItemPadding() const
{
    ...
    const auto top = static_cast<int>(
        std::ceil(style->pixelMetric(QStyle::PM_LayoutTopMargin) * 0.375));
    const auto right = style->pixelMetric(QStyle::PM_LayoutRightMargin);
    const auto bottom = static_cast<int>(
        std::ceil(style->pixelMetric(QStyle::PM_LayoutBottomMargin) * 0.375));
    return {left, top, right, bottom};
}
...
int AbstractItemListWidget::getItemMinimumHeight() const
{
    const auto* style = this->style();
    const auto* theStyle = qobject_cast<const QlementineStyle*>(style);
    const auto height = theStyle ? theStyle->theme().controlHeightLarge : 0;
    const auto padding = getPadding();
...
{
    const auto* theStyle = qobject_cast<const QlementineStyle*>(style());
    return theStyle ? theStyle->theme().borderRadius : 0;
}
...
const QColor& AbstractItemListWidget::getItemBgColor(
    MouseState mouse, const QPalette& palette) const
{
    return palette.color(
        mouse != MouseState::Disabled ? QPalette::Normal : QPalette::Disabled,
        QPalette::Button);
