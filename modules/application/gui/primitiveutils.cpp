...
double getLength(double x, double y) { return std::sqrt(x * x + y * y); }

QPointF getColinearVector(const QPointF& point, double partLength,
...
AngleRadius getAngleRadius(const QPointF& p1, const QPointF& angularPoint,
                           const QPointF& p2, double radius)
{
    // Vector 1.
    ...
    // We have to choose a smaller radius if there is not enough space
    // available.
    ...
    auto startAngle = -std::atan2(startPoint.y() - circleCenter.y(),
                                  startPoint.x() - circleCenter.x());
    auto endAngle = -std::atan2(endPoint.y() - circleCenter.y(),
    ...
    const auto pointOnCircle = getColinearVector(
        circleCenter, radius, circleCenter.x() - angularPoint.x(),
        circleCenter.y() - angularPoint.y());
    const auto translationX =
        2 * static_cast<int>(angularPoint.x() - pointOnCircle.x());
    const auto translationY =
        2 * static_cast<int>(angularPoint.y() - pointOnCircle.y());
...
{
    auto* const window =
        w && w->window() ? w->window()->windowHandle() : nullptr;
    const auto pixelRatio = window ? window->devicePixelRatio() : 1.;
    return pixelRatio;
}

std::tuple<QString, QString> getMenuLabelAndShortcut(const QString& text)
{
    // TODO
...
void drawEllipseBorder(QPainter* p, const QRectF& rect, const QColor& color,
                       qreal const borderWidth)
{
    const auto halfBorderW = borderWidth / 2.;
    const auto borderRect = rect.marginsRemoved(
        {halfBorderW, halfBorderW, halfBorderW, halfBorderW});
    p->setPen(QPen(color, borderWidth, Qt::SolidLine));
    p->setBrush(Qt::NoBrush);
    p->drawEllipse(borderRect);
}

QPainterPath getMultipleRadiusesRectPath(const QRectF& rect,
                                         const CornerRadius& radiuses)
{
    ...
    const auto bottomRight =
        std::min(half, std::max(0., radiuses.bottomRight * 2));
    const auto bottomLeft =
        std::min(half, std::max(0., radiuses.bottomLeft * 2));
    ...
    if (bottomRight > 0.) {
        ...
        path.arcTo(x + w - bottomRight, y + h - bottomRight, bottomRight,
                   bottomRight, 0., -90.);
    }
    else {
        path.lineTo(x + w, y + h);
...
void drawRoundedRect(QPainter* p, const QRectF& rect, const QBrush& brush,
                     qreal radius)
{
    if (radius < 0.1) {
...
void drawRoundedRectF(QPainter* p, const QRectF& rect, const QBrush& brush,
                      const CornerRadius& radiuses)
{
...
void drawRoundedRect(QPainter* p, const QRect& rect, const QBrush& brush,
                     qreal radius)
{
    if (radius < 0.1) {
        p->fillRect(rect, brush);
...
void drawRoundedRectBorder(QPainter* p, const QRectF& rect, const QColor& color,
                           qreal borderWidth, qreal radius)
{
    if (borderWidth > 0) {
        p->setRenderHint(QPainter::RenderHint::Antialiasing);
        p->setPen(QPen{color, borderWidth, Qt::SolidLine, Qt::RoundCap,
                       Qt::RoundJoin});
        p->setBrush(Qt::NoBrush);
        const auto halfBorderW = borderWidth / 2.;
        const auto borderRect = rect.marginsRemoved(
...
void drawRoundedRectBorder(QPainter* p, const QRectF& rect, const QColor& color,
                           qreal borderWidth, const CornerRadius& radiuses)
{
    if (borderWidth > 0) {
        if (radiuses.hasSameRadius()) {
            drawRoundedRectBorder(p, rect, color, borderWidth,
                                  radiuses.topLeft);
        }
        else {
            p->setRenderHint(QPainter::RenderHint::Antialiasing);
            p->setPen(QPen{color, borderWidth, Qt::SolidLine, Qt::SquareCap,
                           Qt::MiterJoin});
            p->setBrush(Qt::NoBrush);
            ...
            if (borderRadiuses < 0.1) {
                ...
                const auto path =
                    getMultipleRadiusesRectPath(borderRect, borderRadiuses);
                p->drawPath(path);
            }
        }
    }
...
void drawRectBorder(QPainter* p, const QRectF& rect, const QColor& color,
                    qreal borderWidth)
{
    if (borderWidth > 0) {
        p->setRenderHint(QPainter::RenderHint::Antialiasing);
...
void drawRoundedTriangle(QPainter* p, const QRectF& rect, qreal radius)
{
    ...
    */
    ...
    path.arcTo(QRectF(angle1.centerPoint.x() - radius,
                      angle1.centerPoint.y() - radius, diameter, diameter),
               angle1.startAngle, angle1.sweepAngle);

    path.lineTo(angle2.startPoint);
    path.arcTo(QRectF(angle2.centerPoint.x() - radius,
                      angle2.centerPoint.y() - radius, diameter, diameter),
               angle2.startAngle, angle2.sweepAngle);

    path.lineTo(angle3.startPoint);
    path.arcTo(QRectF(angle3.centerPoint.x() - radius,
                      angle3.centerPoint.y() - radius, diameter, diameter),
               angle3.startAngle, angle3.sweepAngle);

    path.lineTo(angle1.startPoint);
    path.closeSubpath();

    const auto tr_x = (angle1.translation.x() + angle2.translation.x() +
                       angle3.translation.x());
    const auto tr_y = (angle1.translation.y() + angle2.translation.y() +
                       angle3.translation.y());
...
void drawCheckerboard(QPainter* p, const QRectF& rect, const QColor& darkColor,
                      const QColor& lightColor, qreal cellWidth)
{
...
void drawProgressBarValueRect(QPainter* p, const QRect& rect,
                              const QColor& color, qreal min, qreal max,
                              qreal value, qreal const radius, bool inverted)
{
...
void drawColorMark(QPainter* p, const QRect& rect, const QColor& color,
                   const QColor& borderColor, int borderWidth)
{
    const auto circleDiameter = rect.height();
    const auto markRect = QRect((rect.width() - circleDiameter) / 2, 0,
    ...
    if (color.alphaF() < 1.) {
        ...
        {
            p->setClipPath(clipPath);
            drawCheckerboard(p, markRect, darkCellColor, lightCellColor,
                             cellWidth);
        }
        p->restore();
    }
    ...
    drawEllipseBorder(p, markRect, borderColor,
                      borderWidth * 1.5); // get better readability.
}

void drawColorMarkBorder(QPainter* p, const QRect& rect,
                         const QColor& borderColor, int borderWidth)
{
    const auto circleDiameter = rect.height();
    const auto markRect = QRect(int((rect.width() - circleDiameter) * 0.5), 0,
                                circleDiameter, circleDiameter);
    drawEllipseBorder(p, markRect, borderColor,
                      borderWidth * 1.5); // get better readability.
}
...
QPainterPath getMenuIndicatorPath(const QRect& rect)
{
    ...
    const QPointF p1{(3.5 / intendedSize) * w + x,
                     (6.5 / intendedSize) * h + y};
    const QPointF p2{(8.0 / intendedSize) * w + x,
                     (11.0 / intendedSize) * h + y};
    const QPointF p3{(12.5 / intendedSize) * w + x,
                     (6.5 / intendedSize) * h + y};
...
void drawComboBoxIndicator(const QRect& rect, QPainter* p)
{
    ...
    const QPointF p1{(5.5 / intendedSize) * w + x,
                     (5.5 / intendedSize) * h + y};
    const QPointF p2{(8.0 / intendedSize) * w + x,
                     (3.0 / intendedSize) * h + y};
    const QPointF p3{(10.5 / intendedSize) * w + x,
                     (5.5 / intendedSize) * h + y};
    ...
    const QPointF p4{(5.5 / intendedSize) * w + x,
                     (10.5 / intendedSize) * h + y};
    const QPointF p5{(8.0 / intendedSize) * w + x,
                     (13.0 / intendedSize) * h + y};
    const QPointF p6{(10.5 / intendedSize) * w + x,
                     (10.5 / intendedSize) * h + y};
...
void drawCheckBoxIndicator(const QRect& rect, QPainter* p, qreal progress)
{
    ...
    const QPointF p1{(4.5 / intendedSize) * w + x,
                     (8.5 / intendedSize) * h + y};
    const QPointF p2{(7.0 / intendedSize) * w + x,
                     (11.0 / intendedSize) * h + y};
    const QPointF p3{(11.5 / intendedSize) * w + x,
                     (5.5 / intendedSize) * h + y};
...
void drawPartiallyCheckedCheckBoxIndicator(const QRect& rect, QPainter* p,
                                           qreal progress)
{
    constexpr double intendedSize = 16.;
    ...
    const QPointF p1{(4. / intendedSize) * w + x, (8. / intendedSize) * h + y};
    const QPointF p2{(12. / intendedSize) * w + x, (8. / intendedSize) * h + y};
...
void drawRadioButtonIndicator(const QRect& rect, QPainter* p, qreal progress)
{
    ...
    const QRectF ellipseRect{rect.x() + (rect.width() - indicatorW) / 2.,
                             rect.y() + (rect.height() - indicatorH) / 2.,
                             indicatorW, indicatorH};
    p->drawEllipse(ellipseRect);
}

void drawSpinBoxArrowIndicator(const QRect& rect, QPainter* p,
                               ...
                               const QSize& iconSize)
{
    ...
    const QRect iconRect{iconRectX, iconRectY, iconSizeW, iconSizeH};
    ...
    if (buttonSymbol == QAbstractSpinBox::PlusMinus) {
        if (subControl == QStyle::SC_SpinBoxUp) {
            const auto p1 = QPointF(
                x + w / 2, y); // NOLINT (we do want integer division here)
            const auto p2 = QPointF(
                x + w / 2, y + h); // NOLINT (we do want integer division here)
            p->drawLine(p1, p2);

            const auto p3 = QPointF(
                x, y + h / 2); // NOLINT (we do want integer division here)
            const auto p4 = QPointF(
                x + w, y + h / 2); // NOLINT (we do want integer division here)
            p->drawLine(p3, p4);
        }
        ...
        if (subControl == QStyle::SC_SpinBoxUp) {
            const QPointF p1{(1. / intendedSize) * w + x,
                             (5. / intendedSize) * h + y};
            const QPointF p2{(4. / intendedSize) * w + x,
                             (2. / intendedSize) * h + y};
            const QPointF p3{(7. / intendedSize) * w + x,
                             (5. / intendedSize) * h + y};
...
void drawArrowRight(const QRect& rect, QPainter* p)
{
    ...
    const QPointF p1{(6.5 / intendedSize) * w + x,
                     (4.5 / intendedSize) * h + y};
    const QPointF p2{(10. / intendedSize) * w + x,
                     (8.0 / intendedSize) * h + y};
    const QPointF p3{(6.5 / intendedSize) * w + x,
                     (11.5 / intendedSize) * h + y};
    QPainterPath path;
    path.moveTo(p1);
    path.lineTo(p2);
    path.lineTo(p3);
...
void drawArrowLeft(const QRect& rect, QPainter* p)
{
    ...
    const QPointF p1{(9.5 / intendedSize) * w + x,
                     (4.5 / intendedSize) * h + y};
    const QPointF p2{(6.0 / intendedSize) * w + x,
                     (8.0 / intendedSize) * h + y};
    const QPointF p3{(9.5 / intendedSize) * w + x,
                     (11.5 / intendedSize) * h + y};
    QPainterPath path;
    path.moveTo(p1);
    path.lineTo(p2);
...
void drawSubMenuIndicator(const QRect& rect, QPainter* p)
{
    ...
    const QPointF p1{(10.5 / intendedSize) * w + x,
                     (4.5 / intendedSize) * h + y};
    const QPointF p2{(14. / intendedSize) * w + x,
                     (8.0 / intendedSize) * h + y};
    const QPointF p3{(10.5 / intendedSize) * w + x,
                     (11.5 / intendedSize) * h + y};
    QPainterPath path;
...
void drawDoubleArrowRightIndicator(const QRect& rect, QPainter* p)
{
    ...
    {
        const QPointF p1{(4. / intendedSize) * w + x,
                         (4. / intendedSize) * h + y};
        const QPointF p2{(8. / intendedSize) * w + x,
                         (8. / intendedSize) * h + y};
        const QPointF p3{(4. / intendedSize) * w + x,
                         (12. / intendedSize) * h + y};
    ...
    {
        const QPointF p4{(9. / intendedSize) * w + x,
                         (4. / intendedSize) * h + y};
        const QPointF p5{(13. / intendedSize) * w + x,
                         (8. / intendedSize) * h + y};
        const QPointF p6{(9. / intendedSize) * w + x,
                         (12. / intendedSize) * h + y};
...
void drawToolBarExtensionIndicator(const QRect& rect, QPainter* p)
{
...

    {
        ...
        const QPointF p2{(8. / intendedSize) * w + x,
                         (8. / intendedSize) * h + y};
        const QPointF p3{(5.5 / intendedSize) * w + x,
                         (10.5 / intendedSize) * h + y};
        QPainterPath arrowPath1;
        arrowPath1.moveTo(p1);
        arrowPath1.lineTo(p2);
    ...
    {
        const QPointF p4{(9.5 / intendedSize) * w + x,
                         (5.5 / intendedSize) * h + y};
        const QPointF p5{(12. / intendedSize) * w + x,
                         (8. / intendedSize) * h + y};
        const QPointF p6{(9.5 / intendedSize) * w + x,
                         (10.5 / intendedSize) * h + y};
        QPainterPath arrowPath2;
        arrowPath2.moveTo(p4);
        arrowPath2.lineTo(p5);
...
void drawCloseIndicator(const QRect& rect, QPainter* p)
{
...

    {
        const QPointF p1{(4. / intendedSize) * w + x,
                         (4. / intendedSize) * h + y};
        const QPointF p2{(12. / intendedSize) * w + x,
                         (12. / intendedSize) * h + y};
        p->drawLine(p1, p2);
    }
    {
        const QPointF p3{(12. / intendedSize) * w + x,
                         (4. / intendedSize) * h + y};
        const QPointF p4{(4. / intendedSize) * w + x,
                         (12. / intendedSize) * h + y};
        p->drawLine(p3, p4);
    }
}
...
{
    constexpr double defaultSize = 16.;
    constexpr double defaultPenWidth = 1.01;
    constexpr double defaultRadius = 2.5;
    constexpr double defaultDayRadius = 0.5;
    ...
    p->setBrush(Qt::NoBrush);
    p->setPen(
        QPen{color, penWidth, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin});
    drawRoundedRectBorder(
        p,
        QRectF{rect.x() + sizeRatio * 1., rect.y() + sizeRatio * 1.,
               sizeRatio * 14, sizeRatio * 14},
        Qt::black, penWidth, radius);
    ...
    p->drawRoundedRect(
        QRectF{rect.topLeft() + sizeRatio * QPointF{4., 7.}, dayBlockSize},
        defaultDayRadius, defaultDayRadius);
    p->drawRoundedRect(
        QRectF{rect.topLeft() + sizeRatio * QPointF{7., 7.}, dayBlockSize},
        defaultDayRadius, defaultDayRadius);
    p->drawRoundedRect(
        QRectF{rect.topLeft() + sizeRatio * QPointF{10., 7.}, dayBlockSize},
        defaultDayRadius, defaultDayRadius);
    p->drawRoundedRect(
        QRectF{rect.topLeft() + sizeRatio * QPointF{4., 10.}, dayBlockSize},
        defaultDayRadius, defaultDayRadius);
    p->drawRoundedRect(
        QRectF{rect.topLeft() + sizeRatio * QPointF{7., 10.}, dayBlockSize},
        defaultDayRadius, defaultDayRadius);
}

void drawRadioButton(QPainter* p, const QRect& rect, const QColor& bgColor,
                     const QColor& borderColor, const QColor& fgColor,
                     const qreal borderWidth, qreal progress)
{
    // Background.
    p->setRenderHint(QPainter::RenderHint::Antialiasing);
    p->setPen(Qt::NoPen);
    p->setBrush(bgColor);
    // To avoid ugly visual artifacts in the rounded corners, we cheat by
    // reducing a bit the size
    const auto& ellipseRect = borderWidth > 0.1
...
void drawCheckButton(QPainter* p, const QRect& rect, qreal radius,
                     ...
                     Qt::CheckState checkState)
{
    ...
    if (radius < 1) {
        ...
        // To avoid ugly visual artifacts in the rounded corners, we cheat by
        // reducing a bit the size
        const auto& buttonRect = borderWidth > 0.1
                                     ? QRectF(rect).marginsRemoved(QMarginsF(
                                           borderWidth / 2., borderWidth / 2.,
                                           borderWidth / 2., borderWidth / 2.))
                                     : rect;
    ...
    if (progress > 0.01) {
        ...
        p->setBrush(Qt::NoBrush);
        p->setPen(QPen{fgColor, checkThickness, Qt::SolidLine, Qt::RoundCap,
                       Qt::RoundJoin});
...
void drawItemForeground(QPainter* p, const QRect& rect,
                        const QPixmap& iconPixmap, const QString& text,
                        const QFontMetrics& fontMetrics,
                        const QColor& textColor, int spacing,
                        Qt::Alignment alignment, const QString& secondaryText,
                        const QColor& secondaryTextColor, bool useMnemonic,
                        bool showMnemonic, Qt::TextElideMode elideMode)
{
    p->setRenderHint(QPainter::Antialiasing, true);
    ...
    const auto textW =
        hasText ? fontMetrics.size(Qt::TextShowMnemonic, text).width() : 0;
    const auto secondaryTextW =
        hasSecondaryText
            ? fontMetrics.size(Qt::TextShowMnemonic, secondaryText).width()
            : 0;
    ...
    const auto contentW =
        centered || rightAligned
            ? std::min(rect.width(), iconW + iconSpacing + textW +
                                         secondaryTextSpacing + secondaryTextW)
            : rect.width();
    ...
    if (hasIcon) {
        ...
        const auto pixmapW =
            pixmapPixelRatio != 0
                ? (int)((qreal)iconPixmap.width() / pixmapPixelRatio)
                : 0;
        const auto pixmapH =
            pixmapPixelRatio != 0
                ? (int)((qreal)iconPixmap.height() / pixmapPixelRatio)
                : 0;
    ...
    if (hasSecondaryText && availableWidth > secondaryTextW) {
        const auto secondaryTextX =
            contentRect.x() + contentRect.width() - secondaryTextW;
        const auto secondaryTextRect =
            QRect{secondaryTextX, contentRect.y(), secondaryTextW,
                  contentRect.height()};
        constexpr auto textFlags = Qt::AlignVCenter | Qt::AlignBaseline |
    ...
    if (hasText && availableWidth > 0) {
        ...
        const auto textRect =
            QRect{textX, contentRect.y(), textW, contentRect.height()};
...
void drawMenuSeparator(QPainter* p, const QRect& rect, const QColor& color,
                       int thickness)
{
    const auto x = rect.x();
    const auto w = rect.width();
...
int getTickInterval(int tickInterval, int singleStep, int pageStep, int min,
                    int max, int sliderLength)
{
    if (tickInterval <= 0) {
        tickInterval = singleStep;
        const auto posInterval = QStyle::sliderPositionFromValue(
            min, max, tickInterval, sliderLength);
        const auto posZero =
            QStyle::sliderPositionFromValue(min, max, 0, sliderLength);
...
void drawSliderTickMarks(QPainter* p, const QRect& tickmarksRect,
                         const QColor& tickColor, int min, int max,
                         int interval, int tickThickness, int singleStep,
                         int pageStep)
{
    const auto sliderLength = tickmarksRect.width();
    const auto tickInterval =
        getTickInterval(interval, singleStep, pageStep, min, max, sliderLength);
    ...
    while (v <= max) {
        const auto x1 =
            std::min(tickmarksRect.right(),
                     tickmarksRect.left() + QStyle::sliderPositionFromValue(
                                                min, max, v, sliderLength));
        p->drawLine(x1, y1, x1, y2);
        v += tickInterval;
    }
}

void drawDialTickMarks(QPainter* p, const QRect& tickmarksRect,
                       const QColor& tickColor, int min, int max,
                       int tickThickness, int tickLength, int singleStep,
                       int pageStep, int minArcLength)
{
    p->setRenderHint(QPainter::Antialiasing, true);
    p->setPen(QPen(tickColor, tickThickness, Qt::SolidLine, Qt::FlatCap));
    p->setBrush(Qt::NoBrush);
    ...
    const auto anglePerStep =
        max != min ? 6. * QLEMENTINE_PI_4 / (max - min) : 0.;
    const auto arcPerSteep = anglePerStep * r1;
    const auto tickInterval =
        arcPerSteep > minArcLength ? singleStep : pageStep;

    auto v = static_cast<qreal>(min);
    while (v <= max) {
        const auto ratio = max != min ? (v - min) / (max - min) : 0;
        const auto angle =
            (ratio - 1.) * 6. * QLEMENTINE_PI_4 + QLEMENTINE_PI_4;
...
void drawDial(QPainter* p, const QRect& dialRect, int min, int max,
              ...
              int markThickness)
{
    constexpr int totalAngleDegrees = 360;
    constexpr int deadAngleDegrees = 90;
    constexpr int angleSpreadDegrees = totalAngleDegrees - deadAngleDegrees;
    constexpr int startAngleDegrees = angleSpreadDegrees - 45;
    constexpr int qtAnglePrecision = 16; // Do not change (cf Qt documentation).
    constexpr double angleSpreadRadians = 6. * QLEMENTINE_PI_4;
    constexpr double startAngleRadians = QLEMENTINE_PI_4;
    ...
    const auto arcRect = QRectF(dialRect).marginsRemoved(
        {halfGrooveThickness, halfGrooveThickness, halfGrooveThickness,
         halfGrooveThickness});
    ...
    p->setBrush(Qt::NoBrush);
    p->setPen(QPen(grooveColor, grooveThickness, Qt::SolidLine, Qt::RoundCap,
                   Qt::RoundJoin));
    p->setPen(QPen(valueColor, grooveThickness, Qt::SolidLine, Qt::RoundCap,
                   Qt::RoundJoin));
    p->drawArc(arcRect, startAngle, int(angleLength));

    // Cheat a bit: draw a pie below to avoid that nasty imprecise antialiasing:
    // it soften the colors.
    if (value > min) {
        p->setPen(Qt::NoPen);
        p->setBrush(valueColor);
        const auto circlePerimeter = QLEMENTINE_PI * dialRect.width();
        const auto cropLength =
            (halfGrooveThickness / circlePerimeter) * deadAngleDegrees;
    ...
    // Front.
    const auto dialFrontRect = dialRect.marginsRemoved(
        {grooveThickness, grooveThickness, grooveThickness, grooveThickness});
    p->setPen(Qt::NoPen);
    ...
    const auto markAngleRadians =
        startAngleRadians + (ratio - 1.) * angleSpreadRadians;
    ...
    const auto p1 =
        QPointF{p1Radius * markAngleCosinus, p1Radius * markAngleSinus} +
        markRect.center();
    const auto p2 =
        QPointF{p2Radius * markAngleCosinus, p2Radius * markAngleSinus} +
        markRect.center();
    p->setBrush(Qt::NoBrush);
...
QPainterPath getTabPath(const QRect& rect, const CornerRadius& radiuses)
{
    QPainterPath path;
    const auto w = static_cast<qreal>(rect.width());
    const auto h = static_cast<qreal>(rect.height());
    ...
    if (bottomRight > 0.) {
        path.lineTo(x + w, y + h - bottomRight);
        path.arcTo(x + w, y + h - bottomRight, bottomRight, bottomRight, 180.,
                   90.);
    }
    else {
        path.lineTo(x + w, y + h);
    ...
    if (bottomLeft > 0.) {
        path.lineTo(x - bottomLeft, y + h);
        path.arcTo(x - bottomLeft, y + h - bottomLeft, bottomLeft, bottomLeft,
                   270., 90.);
    }
    else {
        path.lineTo(x, y + h);
...
void drawTab(QPainter* p, const QRect& rect, const CornerRadius& radius,
             const QColor& bgColor, bool drawShadow, const QColor& shadowColor)
{
    if (drawShadow) {
...
void drawTabShadow(QPainter* p, const QRect& rect, const CornerRadius& radius,
                   const QColor& color)
{
    ...
    const auto shadowPixmap =
        qimg::getDropShadowPixmap(pathPixmap, blurRadius, color);

    // Draw the shadow buffer.
    const auto deltaX = (shadowPixmap.width() - pathRect.width()) / 2 + shadowX;
    const auto deltaY =
        (shadowPixmap.height() - pathRect.height()) / 2 + shadowY - blurRadius;
    const auto shadowRect =
        QRect(QPoint(pathRect.x() - deltaX, pathRect.y() - deltaY),
              shadowPixmap.size());
...
void drawElidedMultiLineText(QPainter& p, const QRect& rect,
                             const QString& text,
                             const QPaintDevice* paintDevice)
{
    ...
    for (auto i = 0; i <= lastLine; ++i) {
        ...
        const auto lineRect = QRect(rect.topLeft() + line.position().toPoint(),
                                    QSize(maxWidth, int(line.height())));
        if ((i < lastLine || lastLine == lineCount - 1) &&
            line.naturalTextWidth() < maxWidth) {
            line.draw(&p, rect.topLeft());
        }
        else {
            const auto lineText = removeTrailingWhitespaces(
                textLayout.text().mid(line.textStart(), line.textLength()));
            const auto ellipsis = QString("…");
            const auto ellipsisWidth = textWidth(fontMetrics, ellipsis);
            auto elidedLineText =
                removeTrailingWhitespaces(fontMetrics.elidedText(
                    lineText, Qt::TextElideMode::ElideRight,
                    maxWidth - ellipsisWidth, Qt::TextSingleLine));
...
{
    auto shortcutStr = shortcut.toString(QKeySequence::NativeText);
    shortcutStr.replace(QApplication::translate("QShortcut", "Left"),
                        QChar(0x2190));
    shortcutStr.replace(QApplication::translate("QShortcut", "Right"),
                        QChar(0x2192));
    shortcutStr.replace(QApplication::translate("QShortcut", "Up"),
                        QChar(0x2191));
    shortcutStr.replace(QApplication::translate("QShortcut", "Down"),
                        QChar(0x2193));
    return shortcutStr;
}

void drawShortcut(QPainter& p, const QKeySequence& shortcut, const QRect& rect,
                  const Theme& theme, bool enabled, Qt::Alignment alignment)
{
    ...
    const auto parts =
        shortcutStr.split('+', Qt::SkipEmptyParts, Qt::CaseInsensitive);
    ...
    const auto minimumTextW =
        fm.capHeight() + 2 * borderW; // Ensure (approximatively) a square.
    const auto bgMargins =
        QMargins(borderW, borderW, borderW, static_cast<int>(borderW * radius));
    auto x = 0;

    for (const auto& part : parts) {
        const auto textRect = QRect(
            0, 0, std::max(minimumTextW, textWidth(fm, part)), fm.height());
        ...
        // Background.
        p.setBrush(enabled ? theme.secondaryAlternativeColor
                           : theme.secondaryAlternativeColorDisabled);
        p.setPen(Qt::NoPen);
        p.drawRoundedRect(bgRect.translated(translation), radius + borderW,
                          radius + borderW);

        // Foreground.
        p.setBrush(enabled ? theme.backgroundColorMain2
                           : theme.backgroundColorMain3);
        p.setPen(Qt::NoPen);
        p.drawRoundedRect(fgRect.translated(translation), radius, radius);

        // Text.
        constexpr auto textFlags =
            Qt::AlignCenter | Qt::TextSingleLine | Qt::TextHideMnemonic;
        p.setBrush(Qt::NoBrush);
...
QPixmap getPixmap(const QIcon& icon, const QSize& iconSize, MouseState mouse,
                  Qt::CheckState checked)
{
    const auto iconMode = getIconMode(mouse);
    const auto iconState = getIconState(checked);
    // QIcon::pixmap will automatically get the correct pixel ratio based on the
    // QApplication's pixel ratio.
    return icon.pixmap(iconSize, iconMode, iconState);
}

QRect drawIcon(const QRect& rect, QPainter* p, const QIcon& icon,
               MouseState mouse, Qt::CheckState checked, bool colorize,
               const QColor& color)
{
    ...
    const auto& targetPixmap =
        colorize ? qimg::getColorizedPixmap(pixmap, color) : pixmap;
    ...
    // The pixmap may be smaller than the requested size, so we center it in the
    // rect by default.
    const auto targetPixelRatio = targetPixmap.devicePixelRatio();
    const auto targetW =
        static_cast<int>((qreal)targetPixmap.width() / targetPixelRatio);
    const auto targetH =
        static_cast<int>((qreal)targetPixmap.height() / targetPixelRatio);
...
void updateUncheckableButtonIconPixmap(QIcon& icon, const QSize& size,
                                       const QlementineStyle& style,
                                       const PixmapMakerFunc& func)
{
    if (!func)
        return;
    ...
    for (const auto factor : {1.0, 2.0}) {
        ...
        auto pixmapNormal = func(scaledSize, style.toolButtonForegroundColor(
                                                 MouseState::Normal, role));
        pixmapNormal.setDevicePixelRatio(factor);
        icon.addPixmap(pixmapNormal, QIcon::Mode::Normal);

        auto pixmapMouseOver = func(scaledSize, style.toolButtonForegroundColor(
                                                    MouseState::Hovered, role));
        pixmapMouseOver.setDevicePixelRatio(factor);
        icon.addPixmap(pixmapMouseOver, QIcon::Mode::Selected);

        auto pixmapPressed = func(scaledSize, style.toolButtonForegroundColor(
                                                  MouseState::Pressed, role));
        pixmapPressed.setDevicePixelRatio(factor);
        icon.addPixmap(pixmapPressed, QIcon::Mode::Active);

        auto pixmapDisabled = func(scaledSize, style.toolButtonForegroundColor(
                                                   MouseState::Disabled, role));
        pixmapDisabled.setDevicePixelRatio(factor);
        icon.addPixmap(pixmapDisabled, QIcon::Mode::Disabled);
    }
}

void updateCheckIcon(QIcon& icon, const QSize& size,
                     const QlementineStyle& style)
{
    ...
    for (const auto factor : {1.0, 2.0}) {
        ...
        auto pixmapNormal = makeCheckPixmap(
            scaledSize, style.buttonForegroundColor(MouseState::Normal, role));
        pixmapNormal.setDevicePixelRatio(factor);

        auto pixmapMouseOver = makeCheckPixmap(
            scaledSize, style.buttonForegroundColor(MouseState::Hovered, role));
        pixmapMouseOver.setDevicePixelRatio(factor);

        auto pixmapPressed = makeCheckPixmap(
            scaledSize, style.buttonForegroundColor(MouseState::Pressed, role));
        pixmapPressed.setDevicePixelRatio(factor);

        auto pixmapDisabled = makeCheckPixmap(
            scaledSize,
            style.buttonForegroundColor(MouseState::Disabled, role));
        pixmapDisabled.setDevicePixelRatio(factor);
        ...
        icon.addPixmap(pixmapNormal, QIcon::Normal, QIcon::On);
        icon.addPixmap(pixmapMouseOver, QIcon::Selected, QIcon::On);
        icon.addPixmap(pixmapPressed, QIcon::Active, QIcon::On);
        icon.addPixmap(pixmapDisabled, QIcon::Disabled, QIcon::On);

        icon.addPixmap(uncheckedPixmap, QIcon::Normal, QIcon::Off);
        icon.addPixmap(uncheckedPixmap, QIcon::Selected, QIcon::Off);
        icon.addPixmap(uncheckedPixmap, QIcon::Active, QIcon::Off);
        icon.addPixmap(uncheckedPixmap, QIcon::Disabled, QIcon::Off);
    }
}

QPixmap makeClearButtonPixmap(const QSize& size, const QColor& fgColor)
{
    const auto w = size.width();
    const auto h = size.width();
    constexpr auto intendedSize = 16.;

    const QPointF p1{(4.7 / intendedSize) * w, (4.7 / intendedSize) * h};
    const QPointF p2{(11.3 / intendedSize) * w, (11.3 / intendedSize) * h};
    const QPointF p3{(4.7 / intendedSize) * w, (11.3 / intendedSize) * h};
    const QPointF p4{(11.3 / intendedSize) * w, (4.7 / intendedSize) * h};
    ...
    p.setRenderHint(QPainter::Antialiasing, true);
    p.setBrush(Qt::NoBrush);
    p.setPen({fgColor, penWidth, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin});
...
QPixmap makeCheckPixmap(const QSize& size, const QColor& color)
{
...
QPixmap makeCalendarPixmap(const QSize& size, const QColor& color)
{
    QPixmap pixmap{size};
    pixmap.fill(Qt::transparent);
    QPainter p{&pixmap};
...
QPixmap makeDoubleArrowRightPixmap(const QSize& size, const QColor& color)
{
...
QPixmap makeToolBarExtensionPixmap(const QSize& size, const QColor& color)
{
    const QRect rect{0, 0, size.width(), size.height()};
...
QPixmap makeArrowLeftPixmap(const QSize& size, const QColor& color)
{
    const QRect rect{0, 0, size.width(), size.height()};
...
QPixmap makeArrowRightPixmap(const QSize& size, const QColor& color)
{
    const QRect rect{0, 0, size.width(), size.height()};
...
QPixmap makeMessageBoxWarningPixmap(const QSize& size, const QColor& bgColor,
                                    const QColor& fgColor)
{
    const auto bgSvgPath =
        u":/qlementine/resources/icons/messagebox_warning_bg.svg"_s;
    const auto fgSvgPath =
        u":/qlementine/resources/icons/messagebox_warning_fg.svg"_s;
    const auto pixmap =
        qimg::makePixmapFromSvg(bgSvgPath, bgColor, fgSvgPath, fgColor, size);
    return pixmap;
}

QPixmap makeMessageBoxCriticalPixmap(const QSize& size, const QColor& bgColor,
                                     const QColor& fgColor)
{
    const auto bgSvgPath =
        u":/qlementine/resources/icons/messagebox_critical_bg.svg"_s;
    const auto fgSvgPath =
        u":/qlementine/resources/icons/messagebox_critical_fg.svg"_s;
    const auto pixmap =
        qimg::makePixmapFromSvg(bgSvgPath, bgColor, fgSvgPath, fgColor, size);
    return pixmap;
}

QPixmap makeMessageBoxQuestionPixmap(const QSize& size, const QColor& bgColor,
                                     const QColor& fgColor)
{
    const auto bgSvgPath =
        u":/qlementine/resources/icons/messagebox_question_bg.svg"_s;
    const auto fgSvgPath =
        u":/qlementine/resources/icons/messagebox_question_fg.svg"_s;
    const auto pixmap =
        qimg::makePixmapFromSvg(bgSvgPath, bgColor, fgSvgPath, fgColor, size);
    return pixmap;
}

QPixmap makeMessageBoxInformationPixmap(const QSize& size,
                                        const QColor& bgColor,
                                        const QColor& fgColor)
{
    const auto bgSvgPath =
        u":/qlementine/resources/icons/messagebox_information_bg.svg"_s;
    const auto fgSvgPath =
        u":/qlementine/resources/icons/messagebox_information_fg.svg"_s;
    const auto pixmap =
        qimg::makePixmapFromSvg(bgSvgPath, bgColor, fgSvgPath, fgColor, size);
    return pixmap;
}

void updateMessageBoxWarningIcon(QIcon& icon, const QSize& size,
                                 const Theme& theme)
{
    ...
    for (const auto factor : {1., 2.}) {
        ...
        auto disbledPixmap = makeMessageBoxWarningPixmap(
            scaledSize, bgColorDisabled, fgColorDisabled);
...
void updateMessageBoxCriticalIcon(QIcon& icon, const QSize& size,
                                  const Theme& theme)
{
    ...
    for (const auto factor : {1., 2.}) {
        const auto scaledSize = size * factor;
        auto pixmap =
            makeMessageBoxCriticalPixmap(scaledSize, bgColor, fgColor);
        pixmap.setDevicePixelRatio(factor);
        icon.addPixmap(pixmap, QIcon::Mode::Normal, QIcon::State::Off);
        icon.addPixmap(pixmap, QIcon::Mode::Active, QIcon::State::Off);
        icon.addPixmap(pixmap, QIcon::Mode::Selected, QIcon::State::Off);
        ...
        auto disbledPixmap = makeMessageBoxCriticalPixmap(
            scaledSize, bgColorDisabled, fgColorDisabled);
        disbledPixmap.setDevicePixelRatio(factor);
        icon.addPixmap(disbledPixmap, QIcon::Mode::Disabled, QIcon::State::Off);
        icon.addPixmap(disbledPixmap, QIcon::Mode::Disabled, QIcon::State::On);
...
void updateMessageBoxQuestionIcon(QIcon& icon, const QSize& size,
                                  const Theme& theme)
{
    ...
    for (const auto factor : {1., 2.}) {
        const auto scaledSize = size * factor;
        auto pixmap =
            makeMessageBoxQuestionPixmap(scaledSize, bgColor, fgColor);
        pixmap.setDevicePixelRatio(factor);
        icon.addPixmap(pixmap, QIcon::Mode::Normal, QIcon::State::Off);
        icon.addPixmap(pixmap, QIcon::Mode::Active, QIcon::State::Off);
        icon.addPixmap(pixmap, QIcon::Mode::Selected, QIcon::State::Off);
        ...
        auto disbledPixmap = makeMessageBoxQuestionPixmap(
            scaledSize, bgColorDisabled, fgColorDisabled);
        disbledPixmap.setDevicePixelRatio(factor);
        icon.addPixmap(disbledPixmap, QIcon::Mode::Disabled, QIcon::State::Off);
        icon.addPixmap(disbledPixmap, QIcon::Mode::Disabled, QIcon::State::On);
                                     ...
                                     const Theme& theme)
{
    ...
    for (const auto factor : {1., 2.}) {
        const auto scaledSize = size * factor;
        auto pixmap =
            makeMessageBoxInformationPixmap(scaledSize, bgColor, fgColor);
        pixmap.setDevicePixelRatio(factor);
        icon.addPixmap(pixmap, QIcon::Mode::Normal, QIcon::State::Off);
        icon.addPixmap(pixmap, QIcon::Mode::Active, QIcon::State::Off);
        icon.addPixmap(pixmap, QIcon::Mode::Selected, QIcon::State::Off);
        ...
        auto disbledPixmap = makeMessageBoxInformationPixmap(
            scaledSize, bgColorDisabled, fgColorDisabled);
        disbledPixmap.setDevicePixelRatio(factor);
        icon.addPixmap(disbledPixmap, QIcon::Mode::Disabled, QIcon::State::Off);
        icon.addPixmap(disbledPixmap, QIcon::Mode::Disabled, QIcon::State::On);
