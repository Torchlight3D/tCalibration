...
void horizontal_blur(const T* in, T* out, int w, int h, int r)
{
    double iarr = 1. / (r + r + 1);
    for (int i = 0; i < h; i++) {
        int ti = i * w;
        ...
        for (int ch = 0; ch < C; ++ch) {
            fv[ch] = P == EdgePolicy::Extend ? in[ti * C + ch]
                                             : 0; // unused with Crop policy
            lv[ch] = P == EdgePolicy::Extend ? in[(ti + w - 1) * C + ch]
                                             : 0; // unused with kcrop policy
        ...
        for (int j = 0; j <= r; j++, ri++, ti++) {
            for (int ch = 0; ch < C; ++ch) {
                val[ch] += P == EdgePolicy::Extend ? in[ri * C + ch] - fv[ch]
                                                   : in[ri * C + ch];
                out[ti * C + ch] = P == EdgePolicy::Extend
                                       ? val[ch] * iarr
                                       : val[ch] / (r + j + 1);
            }
        }
        ...
        for (int j = w - r; j < w; j++, ti++, li++) {
            for (int ch = 0; ch < C; ++ch) {
                val[ch] += P == EdgePolicy::Extend ? lv[ch] - in[li * C + ch]
                                                   : -in[li * C + ch];
                out[ti * C + ch] = P == EdgePolicy::Extend
                                       ? val[ch] * iarr
                                       : val[ch] / (r + w - j);
...
template <typename T>
void horizontal_blur(const T* in, T* out, int w, int h, int channelCount, int r)
{
    switch (channelCount) {
        case 1:
...
void flip_block(const T* in, T* out, int w, int h)
{
...
template <typename T>
void flip_block(const T* in, T* out, int w, int h, int channelCount)
{
    switch (channelCount) {
        case 1:
...
inline void sigma_to_box_radius(int* boxes, double sigma, int passCount)
{
    // ideal filter width
    ...
    double mi = (12 * sigma * sigma - passCount * wl * wl - 4 * passCount * wl -
                 3 * passCount) /
                (-4 * wl - 4);
    // NOLINT (avoid std::round by adding 0.5f and cast to integer type)
    int m = mi + .5;
...
void fast_gaussian_blur(T*& in, T*& out, int w, int h, int channelCount,
                        double sigma)
{
...
template <typename T>
void fast_gaussian_blur(T*& in, T*& out, int w, int h, int channelCount,
                        double sigma, unsigned int passCount)
{
    switch (passCount) {
        case 1:
