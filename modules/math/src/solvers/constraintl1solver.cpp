...
//  minimize ||Ax - b||_1, subject to: Cx - d > 0
      ...
      num_inequality_constraints_(geq_vec.size())
{
    ...
    for (int i = 0; i < geq_mat.outerSize(); i++) {
        // Iterate over inside
        for (Eigen::SparseMatrix<double>::InnerIterator it(geq_mat, i); it;
             ++it) {
    ...
    const Eigen::VectorXd& vec, const double kappa)
{
    ...
    output.tail(num_inequality_constraints_).array() =
        inequality_array.max(0.0);
    return output;
}
