...
typename Derived::Scalar FindLargestAbsoluteValueInColumn(
    const Eigen::MatrixBase<Derived>& matrix, int column, int starting_row,
    int ending_row, int* max_value_index)
{
...
template <typename Derived>
int TopDownGaussJordan(int last_row, Eigen::MatrixBase<Derived>* input)
{
    using ScalarType = typename Derived::Scalar;
    ...
    // This for loop eliminates entries in the lower-left triangular part, and
    // it operates from top to bottom of the matrix.
    for (int current_row = 0; current_row < num_rows_to_process;
         ++current_row) {
        ...
        for (int temp_row = current_row + 1; temp_row < num_rows_to_process;
             ++temp_row) {
            // If the number is too small, it is best to consider it as zero
            // already.
            ...
            // Multiply current_row by the leading coeff and subtract it from
            // temp_row to eliminate or set the leading coefficient to zero.
...
//         |0 0 ... 1 |                           | 0 0 ... 1 |
template <typename Derived>
void BottomUpGaussJordan(int start_row, int last_row_to_process,
                         Eigen::MatrixBase<Derived>* input)
{
    ...
    // This for loop eliminates entries in the upper-left triangular part, and
    // it operates from bottom to top of the matrix.
...
//         | i j k l |                            | 0 0 w q |
template <typename Derived>
void GaussJordan(int top_down_last_row, int bottom_up_last_row,
                 Eigen::MatrixBase<Derived>* input)
{
...
template <typename Derived>
void GaussJordan(const int top_down_last_row, Eigen::MatrixBase<Derived>* input)
{
    using internal::TopDownGaussJordan;
