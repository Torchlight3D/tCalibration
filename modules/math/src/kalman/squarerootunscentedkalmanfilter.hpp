...
public:
    ...
    using SystemModelType =
        typename UnscentedBase::template SystemModelType<Control,
                                                         CovarianceBase>;
...
public:
     ...
     *
     * @param alpha Scaling parameter for spread of sigma points (usually \f$
     * 1E-4 \leq \alpha \leq 1 \f$)
     * @param beta Parameter for prior knowledge about the distribution (\f$
     * \beta = 2 \f$ is optimal for Gaussian)
     * @param kappa Secondary scaling parameter (usually 0)
     */
    SquareRootUnscentedKalmanFilter(T alpha = T(1), T beta = T(2),
                                    T kappa = T(0))
                         ...
                         const Control& u)
    {
        ...
        x = this->template computeStatePrediction<Control, CovarianceBase>(s,
                                                                           u);
        ...
        const Measurement& z)
    {
        ...
        Measurement y =
            this->template computeMeasurementPrediction<Measurement,
                                                        CovarianceBase>(
...
protected:
     ...
     * @param [in] noiseCov The system or measurement noise covariance (as
     * square root)
        ...
        CovarianceSquareRoot<Type>& cov)
    {
        ...
        tmp.template topRows<2 * State::RowsAtCompileTime>() =
            ...
            (sigmaPoints.template rightCols<SigmaPointCount - 1>().colwise() -
             mean)
                .transpose();
        ...
        // TODO: According to the paper (Section 3, "Cholesky factor updating")
        // the update
        //       is defined using the square root of the scalar, however the
        //       correct result is obtained when using the weight directly
        //       rather than using the square root It should be checked whether
        //       this is a bug in Eigen or in the Paper
    ...
    /**
     * @brief Compute the Kalman Gain from predicted measurement and sigma
     *points and the innovation covariance.
     *
     * @note This covers equations (27) and (28) of Algorithm 3.1 in the Paper
     *
     * We need to solve the equation \f$ K (S_y S_y^T) = P \f$ for \f$ K \f$
     *using backsubstitution. In order to apply standard backsubstitution using
     *the `solve` method we must bring the equation into the form \f[ AX = B
     *\qquad \text{with } A = S_yS_y^T \f] Thus we transpose the whole equation
     *to obtain \f{align*}{
    ...
    bool computeKalmanGain(
        ...
        KalmanGain<Measurement>& K)
    {
        // Note: The intermediate eval() is needed here (for now) due to a bug
        // in Eigen that occurs when Measurement::RowsAtCompileTime == 1 AND
