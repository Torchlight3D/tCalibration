...
public:
                         ...
                         const Control& u)
    {
        ...
        // predict covariance
        computePredictedCovarianceSquareRoot<State>(
            s.F, S, s.W, s.getCovarianceSquareRoot(), S);
...
protected:
     ...
     * can be deduced in a very straight forward way using the method outlined
     * in the [Unscented Kalman Filter
     * Tutorial](https://cse.sc.edu/~terejanu/files/tutorialUKF.pdf) by Gabriel
     * A. Terejanu for the UKF (Section 3, formulas (27) and (28)).
     *
     * Starting from the standard update formula
     *
     *     \f[ \hat{P} = FPF^T + WQW^T \f]
     *
     * and using the square-root decomposition \f$ P = SS^T \f$ with \f$S\f$
     * being lower-triagonal as well as the (lower triangular) square root
     ...
     * the transpose of the upper-triangular matrix obtained from
     * QR-decompositon of the augmented block matrix
     *
    ...
    /**
     * @brief Compute Kalman gain from state covariance, innovation covariance
     ...
     * \f$P_yy = S_yS_y^T\f$ and transposition of the whole equation the above
