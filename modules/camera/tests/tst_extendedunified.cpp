...
TEST(ExtendedUnifiedCameraModel, SetFromCameraMetaDatas)
{
...
TEST(ExtendedUnifiedCameraModel, constantParameterIndices)
{
    ...
    for (int i = 0; i < constant_subset.size(); i++) {
        EXPECT_NE(constant_subset[i], ExtendedUnifiedCameraModel::Fx);
    }
    ...
    for (int i = 0; i < constant_subset.size(); i++) {
        EXPECT_NE(constant_subset[i], ExtendedUnifiedCameraModel::Cx);
        EXPECT_NE(constant_subset[i], ExtendedUnifiedCameraModel::Cy);
    }
    ...
    EXPECT_EQ(constant_subset.size(), camera.numParameters() - 1);
    for (int i = 0; i < constant_subset.size(); i++) {
        EXPECT_NE(constant_subset[i], ExtendedUnifiedCameraModel::YX);
    }
...
{
    constexpr double kPrincipalPoint[2]{620.8604896644665, 512.7395432585965};
    ExtendedUnifiedCameraModel camera;
    camera.setFocalLength(500.94519367444846);
    camera.setPrincipalPoint(kPrincipalPoint[0], kPrincipalPoint[1]);
    camera.setDistortion(0.6011819970697154, 1.1714262727609582);
