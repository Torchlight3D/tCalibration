...
public:
    ...
    ExtendedUnifiedProjection(double alpha, double beta, double focalLengthU,
...
private:
    /// \brief the alpha parameter that relates ellipsoid and pinhole
    /// projection.
    double _alpha;
...
template <typename DISTORTION_T>
ExtendedUnifiedProjection<DISTORTION_T>::ExtendedUnifiedProjection(
    double alpha, double beta, double focalLengthU, double focalLengthV,
    ...
    const Eigen::MatrixBase<DERIVED_JK> &outJk) const
{
    ...
    const double d_k_d_r2 =
        0.5 * _alpha * _beta *
        (-2 * _alpha * tmp2 + tmp1 * (_alpha - gamma) / tmp_sqrt) * tmp2_inv2;
    ...
    const double d_norm_inv_d_r2 =
        -0.5 * (1 + 2 * k * d_k_d_r2) * norm_inv * norm_inv * norm_inv;
...
template <typename DISTORTION_T>
template <typename DERIVED_P, typename DERIVED_JI>
void ExtendedUnifiedProjection<DISTORTION_T>::
...
template <typename DISTORTION_T>
template <class Archive>
void ExtendedUnifiedProjection<DISTORTION_T>::save(
...
// \brief creates a random valid keypoint.
template <typename DISTORTION_T>
Eigen::VectorXd ExtendedUnifiedProjection<DISTORTION_T>::createRandomKeypoint()
    const
{
...
template <typename DISTORTION_T>
Eigen::Vector3d
ExtendedUnifiedProjection<DISTORTION_T>::createRandomVisiblePoint(
    double depth) const
{
...
template <typename DISTORTION_T>
void ExtendedUnifiedProjection<DISTORTION_T>::getParameters(
...
template <typename DISTORTION_T>
void ExtendedUnifiedProjection<DISTORTION_T>::setParameters(
...
{
    return _alpha == rhs._alpha && _beta == rhs._beta && _fu == rhs._fu &&
...
template <typename DISTORTION_T>
ExtendedUnifiedProjection<DISTORTION_T>
ExtendedUnifiedProjection<DISTORTION_T>::getTestProjection()
{
    return ExtendedUnifiedProjection<DISTORTION_T>(
        0.63, 1.04, 380, 380, 640, 512, 1280, 1024,
