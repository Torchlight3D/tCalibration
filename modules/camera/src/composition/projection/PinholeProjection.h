...
public:
    ...
    template <typename Point_t>
    bool isHomogeneousVisible(const Eigen::MatrixBase<Point_t> &point) const;
    ...
    bool euclideanToKeypoint(const Eigen::MatrixBase<Point_t> &point,
                             Eigen::MatrixBase<Keypoint_t> &keypoint) const;
    template <typename Point_t, typename Keypoint_t>
    bool homogeneousToKeypoint(const Eigen::MatrixBase<Point_t> &point,
                               Eigen::MatrixBase<Keypoint_t> &keypoint) const;

    template <typename Keypoint_t, typename Point_t>
    bool keypointToEuclidean(const Eigen::MatrixBase<Keypoint_t> &keypoint,
                             const Eigen::MatrixBase<Point_t> &point) const;
    template <typename Keypoint_t, typename Point_t>
    bool keypointToHomogeneous(const Eigen::MatrixBase<Keypoint_t> &keypoint,
                               Eigen::MatrixBase<Point_t> &point) const;

    // point mapping with jacobian
    //    template <typename Point_t, typename Keypoint_t, typename Jacobian_t>
    //    bool euclideanToKeypoint(const Eigen::MatrixBase<Point_t> &point,
    //                             Eigen::MatrixBase<Keypoint_t> &keypoint,
    //                             Eigen::MatrixBase<Jacobian_t> &jacobian)
    //                             const;
    //    template <typename Point_t, typename Keypoint_t, typename Jacobian_t>
    //    bool homogeneousToKeypoint(const Eigen::MatrixBase<Point_t> &point,
    //                               Eigen::MatrixBase<Keypoint_t> &keypoint,
    ...
    //    template <typename Keypoint_t, typename Point_t, typename Jacobian_t>
    //    bool keypointToEuclidean(const Eigen::MatrixBase<Keypoint_t>
    //    &keypoint,
    //                             Eigen::MatrixBase<Point_t> &point,
    //                             Eigen::MatrixBase<Jacobian_t> &jacobian)
    //                             const;
    //    template <typename Keypoint_t, typename Point_t, typename Jacobian_t>
    //    bool keypointToHomogeneous(const Eigen::MatrixBase<Keypoint_t>
    //    &keypoint,
    //                               Eigen::MatrixBase<Point_t> &point,
    //                               Eigen::MatrixBase<Jacobian_t> &jacobian)
    //                               const;
    ...
    //    virtual Eigen::Vector3d createRandomVisiblePoint(double depth = -1.0)
    //    const;

    //    /// \brief Get a set of border rays
    //    void getBorderRays(Eigen::MatrixXd &rays);

    //    /// \brief initialize the intrinsics based on a list of views of a
    //    gridded
    ...
    //    /// observation. \return the number of corners successfully observed
    //    and
    //    /// projected
    //    size_t computeReprojectionError(const CalibObservation &obs,
    //                                    const Eigen::Isometry3d
    //                                    &T_target_camera, double &outErr)
    //                                    const;

    //    /// \brief estimate the transformation of the camera with respect to
    //    the
    //    /// calibration target
    //    ///        On success out_T_t_c is filled in with the transformation
    //    that
...
{
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_P>, 4);
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_K>, 2);
    EIGEN_STATIC_ASSERT_MATRIX_SIZE(Eigen::MatrixBase<DERIVED_JP>, 2, 4);
...
{
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_P>, 3);
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_K>, 2);
    EIGEN_STATIC_ASSERT_MATRIX_SIZE(Eigen::MatrixBase<DERIVED_JK>, 3, 2);
    ...
    const Eigen::MatrixBase<DERIVED_JK> &outJk) const
{
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_P>, 4);
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_K>, 2);
    EIGEN_STATIC_ASSERT_MATRIX_SIZE(Eigen::MatrixBase<DERIVED_JK>, 4, 2);
...
{
    EIGEN_STATIC_ASSERT_VECTOR_SIZE(Eigen::MatrixBase<DERIVED_P>, 3);
    EIGEN_STATIC_ASSERT_MATRIX_SIZE(Eigen::MatrixBase<DERIVED_JI>, 2, 4);
...
{
    return _fu == rhs._fu && _fv == rhs._fv && _cu == rhs._cu &&
...
PinholeProjection<DISTORTION_T>::getTestProjection()
{
...
public:
    ...
    static std::vector<cv::Point2d> intersectCircles(double x1, double y1,
                                                     double r1, double x2,
                                                     double y2, double r2)
    {
        ...
        ipts.push_back(
            cv::Point2d(x3 + h * (y2 - y1) / d, y3 - h * (x2 - x1) / d));
        ipts.push_back(
            cv::Point2d(x3 - h * (y2 - y1) / d, y3 + h * (x2 - x1) / d));
        return ipts;
    }
...
/// target These functions are based on functions from Lionel Heng and the
    ...
    const std::vector<GridCalibrationTargetObservation> &observations)
{
    ...
    for (size_t i = 0; i < nImages; ++i) {
        ...
        for (size_t r = 0; r < target.rows(); ++r) {
            ...
            PinholeHelpers::fitCircle(circle, center[r](0), center[r](1),
                                      radius[r]);
        }
        ...
        for (size_t j = 0; j < target.rows(); ++j) {
            for (size_t k = j + 1; k < target.cols(); ++k) {
                ...
                std::vector<cv::Point2d> ipts;
                ipts = PinholeHelpers::intersectCircles(
                    center[j](0), center[j](1), radius[j], center[k](0),
                    center[k](1), radius[k]);
                if (ipts.size() < 2)
                    continue;
    ...
    if (f_guesses.empty()) {
        const char *manual_input =
            std::getenv("KALIBR_MANUAL_FOCAL_LENGTH_INIT");
        if (manual_input) {
            double input_guess;
            std::cout << "Initialization of focal length failed. Provide "
            ...
            std::cout
                << "Initialization of focal length failed. You can enable"
