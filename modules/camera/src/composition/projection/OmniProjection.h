...
private:
    ...
    double _fov_parameter; // PM: is = xi for xi=<1, = 1/xi for x>1. Used for
                           // determining valid projections. Better name?
};

template <typename Distortion_t>
OmniProjection<Distortion_t>::OmniProjection()
    : ProjectionBase<Distortion_t>(0., 0., 0., 0.0, 0, 0), _xi(0.0)
{
    updateTemporaries();
}
    ...
    const Eigen::MatrixBase<DERIVED_P> &outPointConst) const
{
    ...
    outPoint[2] =
        1 - _xi * (rho2_d + 1) / (_xi + sqrt(1 + (1 - _xi * _xi) * rho2_d));
    ...
    const Eigen::MatrixBase<DERIVED_JK> &outJk) const
{
    ...
    double mul = -_xi * (2.0 * recip_tmpA + recip_tmpA2 * (_xi * _xi - 1.0) *
                                                tmpB * (rho2_d + 1.0));
    Eigen::Vector2d J3;
    J3[0] = r0 * mul;
    J3[1] = r1 * mul;
...
{
    return _xi == rhs._xi && _fu == rhs._fu && _fv == rhs._fv &&
           _cu == rhs._cu && _cv == rhs._cv && _ru == rhs._ru &&
           _rv == rhs._rv && _recip_fu == rhs._recip_fu &&
    ...
    const std::vector<GridCalibrationTargetObservation> &observations)
{
    ...
    // go though all images and pick the best estimate (=lowest mean reproj.
    // err)
    for (size_t i = 0; i < observations.size(); ++i)
    {
        ...
        for (size_t r = 0; r < target.rows(); ++r)
        {
            ...
            if (count > MIN_CORNERS)
            {
                ...
                if (t < 0)
                {
                    // SM_DEBUG_STREAM("Skipping a bad SVD solution on row " <<
                    // r);
                    continue;
                }
                ...
                // SM_DEBUG_STREAM("Testing a focal length estimate of " <<
                // gamma);
                _fu = gamma;
                _fv = gamma;
                updateTemporaries();
                ...
                {
                    // SM_DEBUG_STREAM("Skipping row " << r << " as the
                    // transformation estimation failed.");
                    continue;
                }

                double reprojErr = 0.0;
                size_t numReprojected =
                    computeReprojectionError(obs, T_target_camera, reprojErr);
                if (numReprojected > MIN_CORNERS)
                {
                    ...
                    {
                        // SM_DEBUG_STREAM("Row " << r << " produced the new
                        // best estimate: " << avgReprojErr << " < " <<
                        // minReprojErr);
            ...
            {
                // SM_DEBUG_STREAM("Skipping row " << r << " because it only had
                // " << count << " corners. Minimum: " << MIN_CORNERS);
            }
