...
public:
    ...
    DoubleSphereProjection(double xi, double alpha, double focalLengthU,
    ...
    // properties
    bool isProjectionInvertible() const override { return true; }

    int keypointDimension() const override { return KeypointDimension; }

    // intrinsic
    double xi() const { return _xi; }
    double alpha() const { return _alpha; }

    void resizeIntrinsics(double scale) override;
    ...
    /// \brief compute the reprojection error based on a checkerboard
    /// observation. \return the number of corners successfully observed and
    /// projected
    size_t computeReprojectionError(const CalibObservation &obs,
                                    const Eigen::Isometry3d &T_target_camera,
                                    double &outErr) const;
...
private:
    ...
    double _xi;
    /// \brief the alpha parameter that relates second sphere and pinhole
    /// projection.
    double _alpha;
    ...
    double _fov_parameter; //!< parameter related to the valid region of the
                           //!< camera model
};
      ...
      _rv(1)
{
    ...
    // NOTE @demmeln 2018-05-07: In order to use this with distortion, you need
    // to add the proper calls for projection
    //     and unprojection, including for Jacobian computation. Compare to
    //     distored-omni model.
    EIGEN_STATIC_ASSERT_SAME_TYPE(
        DISTORTION_T, NoDistortion,
        "Currently only implemented for 'NoDistortion'");
}
...
template <typename DISTORTION_T>
DoubleSphereProjection<DISTORTION_T>::DoubleSphereProjection(
    double xi, double alpha, double focalLengthU, double focalLengthV,
    double imageCenterU, double imageCenterV, int resolutionU, int resolutionV,
    ...
    const Eigen::MatrixBase<DERIVED_JP> &outJp) const
{
    ...
    double d_norm_d_r2 = (_xi * (1 - _alpha) * d1_inv +
                          _alpha * (_xi * k * d1_inv + 1) * d2_inv) *
                         norm_inv2;
    ...
    const Eigen::MatrixBase<DERIVED_JP> &outJp) const
{
    ...
    {
        bool success =
            euclideanToKeypoint(-ph.derived().template head<3>(), outKeypoint,
    ...
    const Eigen::MatrixBase<DERIVED_JK> &outJk) const
{
    ...
    const double d_mz_d_r2 = (0.5 * _alpha - _alpha_2) * (r2 * _alpha_2 - 1) *
                                 sqrt2_inv * norm2_inv2 -
                             _alpha_2 * norm2_inv;
...
template <typename DISTORTION_T>
template <typename DERIVED_P, typename DERIVED_JI>
void DoubleSphereProjection<DISTORTION_T>::
...
template <typename DERIVED_P, typename DERIVED_JI>
void DoubleSphereProjection<DISTORTION_T>::
    homogeneousToKeypointIntrinsicsJacobian(
        const Eigen::MatrixBase<DERIVED_P> &p,
        const Eigen::MatrixBase<DERIVED_JI> &outJi) const
{
    EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE_OR_DYNAMIC(
...
template <typename DERIVED_P, typename DERIVED_JD>
void DoubleSphereProjection<DISTORTION_T>::
    homogeneousToKeypointDistortionJacobian(
        const Eigen::MatrixBase<DERIVED_P> &p,
        const Eigen::MatrixBase<DERIVED_JD> &outJd) const
{
    EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE_OR_DYNAMIC(
...
template <typename DISTORTION_T>
template <class Archive>
void DoubleSphereProjection<DISTORTION_T>::save(
...
// \brief creates a random valid keypoint.
template <typename DISTORTION_T>
Eigen::VectorXd DoubleSphereProjection<DISTORTION_T>::createRandomKeypoint()
    const
{
    // This is tricky...The camera model defines a circle on the normalized
    // image plane and the projection equations don't work outside of it. With
    // some manipulation, we can see that, on the normalized image plane, the
    // edge of this circle is at u^2 + v^2 = 1/(xi^2 - 1) So: this function
    // creates keypoints inside this boundary.
...
void DoubleSphereProjection<DISTORTION_T>::updateTemporaries()
{
    ...
    _recip_fv = 1.0 / _fv;
    _fu_over_fv = _fu / _fv;
    _one_over_2alpha_m_1 = _alpha > 0.5 ? 1.0 / (2 * _alpha - 1)
                                        : std::numeric_limits<double>::max();
    const double temp =
        _alpha <= 0.5 ? _alpha / (1 - _alpha) : (1 - _alpha) / _alpha;
...
template <typename DISTORTION_T>
void DoubleSphereProjection<DISTORTION_T>::getParameters(
...
template <typename DISTORTION_T>
void DoubleSphereProjection<DISTORTION_T>::setParameters(
...
{
    return _xi == rhs._xi && _alpha == rhs._alpha && _fu == rhs._fu &&
           _fv == rhs._fv && _cu == rhs._cu && _cv == rhs._cv &&
...
template <typename DISTORTION_T>
DoubleSphereProjection<DISTORTION_T>
DoubleSphereProjection<DISTORTION_T>::getTestProjection()
{
    return DoubleSphereProjection<DISTORTION_T>(
        -0.18, 0.59, 313, 313, 640, 512, 1280, 1024,
...
/// \brief initialize the intrinsics based on one view of a gridded calibration
/// target \return true on success
template <typename DISTORTION_T>
...
/// \brief estimate the transformation of the camera with respect to the
/// calibration target
///        On success out_T_t_c is filled in with the transformation that takes
///        points from the camera frame to the target frame
/// \return true on success
///
/// These functions were developed with the help of Lionel Heng and the
/// excellent camodocal https://github.com/hengli/camodocal
template <typename DISTORTION_T>
    ...
    sm::kinematics::Transformation &out_T_t_c) const
{
    ...
    for (size_t i = 0; i < Ms.size(); ++i)
    {
        ...
        {
            //      SM_DEBUG_STREAM(
            //          "Skipping point " << i << ", point was observed: " <<
            //          imagePoint
            //              << ", projection success: "
            //              << keypointToEuclidean(imagePoint, backProjection)
            //              << ", in front of camera: " << (backProjection[2] >
            //              0.0)
            //              << "image point: " << imagePoint.transpose()
            //              << ", backProjection: " <<
            //              backProjection.transpose()
            //              << ", camera params (xi,fu,fv,cu,cv):" << xi() << ",
            //              " << fu()
            //              << ", " << fv() << ", " << cu() << ", " << cv());
        }
    }
    ...
    {
        //    SM_DEBUG_STREAM(
        //        "At least 4 points are needed for calling PnP. Found " <<
        //        Ps.size());
        return false;
    }

    // Call the OpenCV pnp function.
    //  SM_DEBUG_STREAM("Calling solvePnP with " << Ps.size() << " world points
    //  and "
