      ...
      m_imageHeight(h)
{
    switch (modelType) {
        ...
        case PINHOLE:
            ...
            break;
        case SCARAMUZZA:
            m_nIntrinsics = OCAMCamera::kNumParameters;
            break;
        case MEI:
        default:
            m_nIntrinsics = 9;
...
bool Camera::Parameters::readFromYamlFile(const std::string &filename)
{
    const cv::FileStorage fs{filename,
                             cv::FileStorage::READ | cv::FileStorage::MEMORY};
    if (!fs.isOpened()) {
        std::cerr << "Failed to read camera parameters: "
...
void Camera::Parameters::writeToYamlFile(const std::string &filename) const
{
    {
...
bool Camera::Parameters::read(const cv::FileNode &node)
{
    if (node[keyModelType].isNone()) {
        std::cerr << "Failed to read camera parameters: "
                     "Invalid file format."
                  << std::endl;
        return false;
    ...
    node[keyModelType] >> sModelType;
    if (sModelType.compare(modelTypeName())) {
        std::cerr << "Failed to read camera parameters: "
                     "Inconsistent camera model."
                  << std::endl;
        return false;
    }

    // Meta
    node[key::kCameraName] >> m_cameraName;
    m_imageWidth = (int)node[key::kImageWidth];
    m_imageHeight = (int)node[key::kImageHeight];

    return true;
}

void Camera::Parameters::write(cv::FileStorage &fs) const
{
    // Meta
    fs << keyModelType << modelTypeName();
    fs << key::kCameraName << m_cameraName;
    fs << key::kImageWidth << m_imageWidth;
    fs << key::kImageHeight << m_imageHeight;
}
