...
public:
    ...
    bool precomputePolaritiesAndNN(const SaddlePointVector &points)
    {
        ...
        max_nn =
            std::min(detector_params.max_nearest_neighbours, int(pts.size()));
        ...
        for (int i = 0; i < num_pts; ++i) {
            ...
            partial_sort_indexes(dist.ptr<double>(i), NN.ptr<int>(i),
                                 pts.size(), max_nn);
            ...
            for (int j = 0; j < max_nn; ++j) {
                ...
                SaddlePointType::comparePolarities(
                    polarity.ptr<typename SaddlePointType::PolarityStorageType>(
                        i),
                    polarity.ptr<typename SaddlePointType::PolarityStorageType>(
                        nn[j]),
                    sm[nn[j]], df[nn[j]]);
...
// check quality of the triangle
        ...
        cv::line(DEBUG, cv::Point(pt0.x * 65536, pt0.y * 65536),
                 cv::Point((pt0.x + dst_u[i11]) * 65536,
                           (pt0.y + dst_v[i11]) * 65536),
        ...
        if (maxI - minI < detector_params.rectangle_consistency_threshold)
            printf("Accepting quad candidate: minI: %.3lf, maxI: %.3lf, "
                   "thresh: %.3lf\n",
                   minI, maxI, detector_params.rectangle_consistency_threshold);
        else
            printf("Rejecting quad candidate: minI: %.3lf, maxI: %.3lf, "
                   "thresh: %.3lf\n",
                   minI, maxI, detector_params.rectangle_consistency_threshold);
                                        ...
                                        const double *u, const double *v) const
    {
        ...
        for (size_t r = 0; r < closenn.size(); ++r)
            for (size_t c = r + 1; c < closenn.size(); ++c)
                if (u[closenn[r]] * u[closenn[c]] +
                        v[closenn[r]] * v[closenn[c]] >
                    shadow_threshold)
                              ...
                              std::vector<Quad> &quads) const
    {
        ...
        const std::vector<int> closenn = getClosestNNs(
            i00, max_nn,
            [sm, df](int nn) -> bool { return (sm[nn] + df[nn]) > 0; });
        ...
        for (size_t i = 0; i < same.size(); ++i) {
            ...
            // get all non-diagonal candidates and check if they have correct
            // polarity (different from i00 and i11)
            std::vector<int> candidates;
            candidates.reserve(2);
            double mean;
            for (size_t j = 0; j < diff.size(); ++j) {
                int didx = diff[j];
                // if candidate has proper polarity, go back to image check
                // consistency of the triangle
            ...
            if (candidates.size() != 2 ||
                ...
                continue;
            // ok we have two candidates to finish the quad, check if they are
            // in expected location
            double cost1 =
                       u[candidates[0]] * vec_u.at<double>(candidates[1], i11) +
                       v[candidates[0]] * vec_v.at<double>(candidates[1], i11),
                   cost2 =
                       u[candidates[1]] * vec_u.at<double>(candidates[0], i11) +
            ...
            double a1 =
                atan2(vec_v.at<double>(i00, i01), vec_u.at<double>(i00, i01));
            double a2 =
                atan2(vec_v.at<double>(i01, i11), vec_u.at<double>(i01, i11));
            double a3 =
                atan2(vec_v.at<double>(i11, i10), vec_u.at<double>(i11, i10));
            double a4 =
                atan2(vec_v.at<double>(i10, i00), vec_u.at<double>(i10, i00));
            double cost = mod2pi(a2 - a1) + mod2pi(a3 - a2) + mod2pi(a4 - a3) +
                          mod2pi(a1 - a4);

            if (fabs(cost - 2 * M_PI) > 1.0e-5 &&
                fabs(cost + 2 * M_PI) > 1.0e-5)
                continue;
                                      ...
                                      std::vector<Quad> &quads) const
    {
        ...
        for (size_t i = 0; i < same.size(); ++i) {
...
#if DEBUG_INDEXING == 5
                printf(
                    "Rejecting diagonal %d, %d: no suitable quad candidates.\n",
                    i00, i11);
#endif
                continue;
            }
            ...
            // triangles need to be homogeneous and different enough from each
            // other
            if (good == 2 &&
                fabs(mean[0] - mean[1]) >=
                    detector_params.triangle_consistency_threshold) {
                double cost1 = u[candidates[0]] *
                                   vec_u.at<double>(candidates[1], i11) +
                               v[candidates[0]] *
                                   vec_v.at<double>(candidates[1], i11),
                       cost2 = u[candidates[1]] *
                                   vec_u.at<double>(candidates[0], i11) +
                               v[candidates[1]] *
                                   vec_v.at<double>(candidates[0], i11);
                ...
                if (cost1 > 0 && cost2 < 0)
                    quads.push_back(
                        Quad(i00, i11, candidates[1], candidates[0]));
                else if (cost1 < 0 && cost2 > 0)
                    quads.push_back(
                        Quad(i00, i11, candidates[0], candidates[1]));
            }
        }
        return true;
    }
        ...
        const uint8_t *pol, const double *u, const double *v) const
    {
        ...
        for (size_t r = 0; r < closenn.size(); ++r)
            for (size_t c = r + 1; c < closenn.size(); ++c)
                if (shadow_mask[r] && u[closenn[r]] * u[closenn[c]] +
                                              v[closenn[r]] * v[closenn[c]] >
                                          shadow_threshold)
                    // remove column indexed nn
                    shadow_mask[c] = false;
    ...
    {
        return uint64_t(n) +
               num_pts * (uint64_t(id0) + num_pts * uint64_t(id1));
    }
    ...
    bool verifyEdge(int n, int id0, int id1)
    {
        ...
        if (edge_cache.count(hash) != 0) {
...
#endif
            cv::Mat u =
                (cv::Mat_<double>(3, 1) << pts[n].x, pts[id0].x, pts[id1].x);
            cv::Mat v =
                (cv::Mat_<double>(3, 1) << pts[n].y, pts[id0].y, pts[id1].y);
            ...
            double delta_r_cur =
                std::max(detector_params.delta_r_min,
            ...
            for (double x = detector_params.edge_stability_interval_min;
                ...
                if (u0 < 0 || u0 > width - 1 || v0 < 0 || v0 > height - 1 ||
                    u1 < 0 || u1 > width - 1 || v1 < 0 || v1 > height - 1)
                    break;
                ...
                double I0 =
                    (1.0 - xw) * (1.0 - yw) * input.at<InputImageType>(yi, xi) +
            ...
            result =
                (minDiffI >
                     detector_params
                         .edge_stability_threshold && // absolute threshold
                                                      // on edge contrast
                 minDiffI > maxDiffI / 2 && // the minimum gradient is at least
                                            // half of maximum gradient
        ...
        if (result) {
#if DEBUG_INDEXING > 2
            printf("Accepted %d from: id0: %d, id1: %d, minI: %f, maxI: %f, "
                   "cnt: %d, "
                   "diffI0: %f, diffI1: %f\n",
            ...
            printf("Rejected %d from: id0: %d, id1: %d, minI: %f, maxI: %f, "
                   "cnt: %d, "
                   "diffI0: %f, diffI1: %f\n",
                   n, id0, id1, minDiffI, maxDiffI, cnt, maxI0 - minI0,
                   maxI1 - minI1);
                    ...
                    std::vector<int> &id)
    {
        ...
        const double *vu = vec_u.ptr<double>(id[0]),
                     *vv = vec_v.ptr<double>(id[0]);
        // go through nearest neighbors and find suitable candidates
        std::vector<bool> id_mask =
            computeShadowMaskIgnoreVisitedAndCheckPolarity(
                closenn, visited,
                deltilleGrid ? issame.ptr(id[0]) : isdiff.ptr(id[0]), vu, vv);
    ...
    void initGridIndexMap(const std::vector<CoordIndex> &coords,
                          cv::Mat &idxmap, int &offr, int &offc)
    {
        ...
        for (size_t i = 0; i < coords.size(); ++i)
            idxmap.at<int>(offr + coords[i].r, offc + coords[i].c) =
                coords[i].id;
    }
                  ...
                  cv::Mat &idxmap, int offr, int offc)
    {
        ...
        for (size_t i = 0; i < old_coords.size(); ++i) {
            ...
            if (idx != -1) {
                std::vector<int> id(2);
                id[0] = idx;
                // has left... check if there are two occupied fields left of
                // the target field...
                if (idxmap.at<int>(ri, ci + 1) == -1 &&
                    idxmap.at<int>(ri, ci - 1) != -1) {
                    id[1] = idxmap.at<int>(ri, ci - 1);
                    if (expandGrid(coords, visited, idxmap, offr, offc, ri,
                                   ci + 1, id))
                        added++;
                }
                // has right... check if there are two occupied fields right of
                // the target field...
                if (idxmap.at<int>(ri, ci - 1) == -1 &&
                    idxmap.at<int>(ri, ci + 1) != -1) {
                    id[1] = idxmap.at<int>(ri, ci + 1);
                ...
                if (idxmap.at<int>(ri + 1, ci) == -1 &&
                    ...
                    if (expandGrid(coords, visited, idxmap, offr, offc, ri + 1,
                                   ci, id))
                        added++;
                }
                // has down
                ...
                if (deltilleGrid) {
                    ...
                    if (idxmap.at<int>(ri - 1, ci + 1) == -1 &&
                        ...
                        if (expandGrid(coords, visited, idxmap, offr, offc,
                                       ri - 1, ci + 1, id))
                            added++;
                    }
                    // has up right
    ...
    bool findBestGrid(const cv::Size &board_size, cv::Mat &best_grid)
    {
        ...
        while (trial < detector_params.grid_search_quad_growing_trials &&
            ...
            bool have_quad = deltilleGrid
                                 ? initialDeltilleQuadSelection(idxs, axis)
            ...
            for (size_t k = 0; k < axis.size(); ++k) {
...
#if DEBUG_INDEXING > 1
                cv::line(DEBUG,
                         cv::Point(pts[quad.i00_].x * 65536,
                                   pts[quad.i00_].y * 65536),
                         cv::Point(pts[quad.i10_].x * 65536,
                                   pts[quad.i10_].y * 65536),
                         cv::Scalar(0, 0, 255), 1, cv::LINE_AA, 16);
                cv::line(DEBUG,
                         cv::Point(pts[quad.i10_].x * 65536,
                                   pts[quad.i10_].y * 65536),
                         cv::Point(pts[quad.i01_].x * 65536,
                                   pts[quad.i01_].y * 65536),
                         cv::Scalar(0, 255, 0), 1, cv::LINE_AA, 16);
                cv::line(DEBUG,
                         cv::Point(pts[quad.i01_].x * 65536,
                                   pts[quad.i01_].y * 65536),
                         cv::Point(pts[quad.i11_].x * 65536,
                                   pts[quad.i11_].y * 65536),
                         cv::Scalar(255, 0, 0), 1, cv::LINE_AA, 16);
                cv::line(DEBUG,
                         cv::Point(pts[quad.i11_].x * 65536,
                                   pts[quad.i11_].y * 65536),
                ...
                while (true) {
                    ...
                    // std::cout << "IdxMap:" << std::endl << idxmap <<
                    // std::endl;
...
// std::cout << "Final IDX map:" << std::endl << idxmap << std::endl;
                ...
                printf("Iters: %d, trials: %d, found: %d points, best: %d\n",
                       iter, trial, nnz, nbest);
#endif
                if (nnz > nbest) {
#ifdef DEBUG_INDEXING
                    std::cout << "Best IDX map:" << std::endl
                              << idxmap << std::endl;
#endif
                    nbest = nnz;
            ...
            for (int r = 0; r < obs.board.rows; ++r)
                for (int c = 0; c < obs.board.cols; ++c) {
                    ...
                    if (ptidx > -1) {
                        ...
                        corners.push_back(
                            cv::Point2f(float(pt.x), float(pt.y)));
                    }
                    else
