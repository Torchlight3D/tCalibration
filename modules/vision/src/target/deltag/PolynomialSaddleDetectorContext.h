...
public:
    ...
    int findSaddles(std::vector<SaddlePointType> &refclust)
    {
        ...
        if (!SaddlePointType::isTriangular) {
            ...
            for (size_t i = 0; i < refined.size(); ++i) {
                if (refined[i].s < smax ||
                    std::abs(refined[i].a2) < min_angle_width ||
        ...
        if (SaddlePointType::isTriangular) {
            // a bit hackier monkey saddle second filter, that checks if points
            // would converge to the same location with larger scale...
            ...
            cv::Mat temp;
            cv::filter2D(input_lowres, temp,
                         cv::DataType<FloatImageType>::depth,
            ...
            for (size_t i = 0; i < refclust.size(); i++) {
                ...
                if (dx * dx + dy * dy >
                    detector_params.deltille_stability_threshold) {
        ...
        for (size_t i = 0; i < refclust.size(); i++) {
            ...
            int sz = scaling * 3;
            cv::rectangle(
                DEBUG, cv::Point((s1.x - sz) * 65536, (s1.y - sz) * 65536),
    ...
    int finalizeSaddles(std::vector<BoardObservation> &boards)
    {
        ...
        for (size_t i = 0; i < boards.size(); ++i) {
            ...
            std::vector<SaddlePointType> fullScalePoints(
                fullScaleLocations.size());
            // spend a bit more time on final refinement... to make sure we do
            // not throw away good points after upscaling
            fullFitting.saddleSubpixelRefinement(full, fullScaleLocations,
                                                 fullScalePoints,
                                                 num_iterations * 2, false);
            for (size_t c = 0; c < obs.corner_locations.size(); ++c) {
                ...
                if (PointIsInf(pt)) {
                    ...
                    if (!PointIsInf(fullScaleLocations[c])) {
                        printf(
                            "Refinement failed for: %d, board #%zu, reason: "
                            "%d, x: "
                            "%.3lf -> %.3lf, y: %.3lf -> %.3lf, det: %.3lf\n",
                            obs.board.at<int>(c / obs.board.cols,
                                              c % obs.board.cols),
                            i, int(pt.s), fullScaleLocations[c].x,
                            std::abs(pt.a1), fullScaleLocations[c].y,
                            std::abs(pt.a2), pt.det);
                    }
#endif
                    obs.corner_locations[c].x = -1.0f;
                    obs.corner_locations[c].y = -1.0f;
                    obs.board.at<int>(int(c / obs.board.cols),
                                      int(c % obs.board.cols)) = -1;
                }
                else {
...
private:
    void stretchIntensities(cv::InputArray input, cv::OutputArray output)
    {
        ...
        input.getMat().convertTo(output, input.type(),
    ...
    void preprocessImage(cv::InputArray img)
    {
        ...
        if (img.getMat().channels() == 3)
            // convert to a single channel first, keep original intensity range
            // for now...
        ...
        cv::resize(gray_img, input_lowres, cv::Size(), 1.0 / scaling,
                   1.0 / scaling, cv::INTER_CUBIC);
        // for initial detection adjust the intensity range to (0-255) for
        // proper thresholding
