...
public:
                                  ...
                                  uint8_t &diff)
    {
        ...
        double cost1 = std::abs(p1[PolaritySine1] * p2[PolaritySine2] +
                                p1[PolarityCosine1] * p2[PolarityCosine2]);
        double cost2 = std::abs(p1[PolaritySine1] * p2[PolaritySine1] +
                                p1[PolarityCosine1] * p2[PolarityCosine1]);
        double cost3 = std::abs(p1[PolaritySine2] * p2[PolaritySine2] +
...
#endif
    void plotPolarities(cv::Mat &im, double scaling = 1.)
    {
...
public:
    ...
    void computePolarities(PolarityStorageType *p) const
    {
        ...
        // a1.imag(), a2.real(), a2.imag(), a3.real(), a3.imag());
    ...
    static void comparePolarities(const PolarityStorageType *p1,
                                  ...
                                  uint8_t &dummy)
    {
        const double triangle_polarity_threshold = std::cos(10.0 / 180 * M_PI);
    ...
    void plotPolarities(cv::Mat &im, double scaling = 1.0)
    {
        ...
        subpixel_line(
            im, cv::Point2d(x, y),
            cv::Point2d(x + std::abs(cos(a1)) * r, y + std::abs(sin(a1)) * r),
            cv::Scalar(255, 0, 0), sz);
        r = 20.0;
        subpixel_line(
            im, cv::Point2d(x, y),
            cv::Point2d(x + std::abs(cos(a2)) * r, y + std::abs(sin(a2)) * r),
            cv::Scalar(255, 255, 0), sz);
        r = 15.0;
        subpixel_line(
            im, cv::Point2d(x, y),
            cv::Point2d(x + std::abs(cos(a3)) * r, y + std::abs(sin(a3)) * r),
            cv::Scalar(255, 255, 0), sz);
    }
...
public:
    ...
    static bool comparePolaritiesUnderRotation(const PolarityStorageType *p1,
                                               ...
                                               int rotation)
    {
        const double triangle_polarity_threshold = cos(
            10.0 / 180 * M_PI); // cos(detector_params.triangle_polarity_angle);
        return std::abs(p1[0].dot(p2[rotation])) >
...
public:
    ...
    std::complex<double> a1, a2,
        a3; // these can be possibly complex numbers ...
    double s, det;
};

// clang-format off
constexpr double separable_kernel_coefs[][31] = {
    {},
    {},
    {0.10805848, 0.23395099, 0.31932424, 0.23395099, 0.10805848},
    {0.05745145, 0.13032769, 0.19602977, 0.23725367, 0.19602977, 0.13032769, 0.05745145},
    {0.03399957, 0.08105924, 0.12712243, 0.16627552, 0.18958185, 0.16627552, 0.12712243, 0.08105924, 0.03399957},
    {0.02259464, 0.05455135, 0.08703301, 0.11746379, 0.14276789, 0.15724555, 0.14276789, 0.11746379, 0.08703301, 0.05455135, 0.02259464},
    {0.01579519, 0.03862299, 0.06283421, 0.08618732, 0.10751836, 0.12494305, 0.13461367, 0.12494305, 0.10751836, 0.08618732, 0.06283421, 0.03862299, 0.01579519},
    {0.01143871, 0.02864804, 0.04706062, 0.06534370, 0.08271709, 0.09832237, 0.11088212, 0.11768435, 0.11088212, 0.09832237, 0.08271709, 0.06534370, 0.04706062, 0.02864804, 0.01143871},
                      const cv::Mat &input, const cv::Mat &mask, cv::Mat &b_vec)
{
        ...
        for (int wy = -window_half_size; wy <= window_half_size; wy++) {
            ...
            for (int wx = -window_half_size; wx <= window_half_size; wx++) {
                if (*v > 0) {
                    ...
                    double val = im00[col0] * w00 + im00[col1] * w01 +
...
struct PolynomialFit
{
    ...
    void saddleSubpixelRefinement(
        ...
        std::vector<SaddlePoint> &refined, int max_iterations = 3,
        bool tight_convergence = true)
    {
        ...
        for (size_t idx = 0; idx < refined.size(); idx++) {
            ...
            for (int it = 0; it < max_iterations; it++) {
                if (interpolatePatch<SmoothedImageType>(
                        pt.x, pt.y, window_half_size, smoothed_input, mask,
                        b)) {
                    ...
                    if (detector_params.spatial_convergence_threshold >
                            fabs(dx) &&
                        detector_params.spatial_convergence_threshold >
                            fabs(dy)) {
                    ...
                    // check for divergence due to departure out of convergence
                    // region or point type change
    ...
    void saddleSubpixelRefinement(
        const cv::Mat &smoothed_input,
        const std::vector<LocationsPointType> &initial,
        std::vector<MonkeySaddlePointType> &refined, int max_iterations = 10,
        bool tight_convergence = true)
    {
        cv::Mat b(invAtAAt.cols, 1, CV_64FC1);
        ...
        for (size_t idx = 0; idx < initial.size(); idx++)
            refined[idx] =
                MonkeySaddlePointType(initial[idx].x, initial[idx].y);
        ...
        for (size_t idx = 0; idx < refined.size(); idx++) {
            ...
            for (int it = 0; it < max_iterations; it++) {
                if (interpolatePatch<SmoothedImageType>(
                    ...
                    cv::Mat p = invAtAAt * b;
                    const double *a = p.ptr<double>(0) -
                                      1; // 1 based indexing for convenience

                    // now use second derivatives to find the location of
                    // critical point
                    //
                    ...
                    //      [   a3 3*a4 ]               [ -a7 ]
                    //
                    // this is over determined system, solve it by: inv(A'A) =
                    // A'b
                    //
                    const double AtA_a =
                        9 * a[1] * a[1] + 4 * a[2] * a[2] + a[3] * a[3];
                    const double AtA_b =
                        3 * a[1] * a[2] + 4 * a[2] * a[3] + 3 * a[3] * a[4];
                    const double AtA_d =
                        a[2] * a[2] + 4 * a[3] * a[3] + 9 * a[4] * a[4];

                    // inverse...
                    const double det = AtA_a * AtA_d - AtA_b * AtA_b;

                    const double Atb_a =
                        -3 * a[1] * a[5] - 2 * a[2] * a[6] - a[3] * a[7];
                    const double Atb_b =
                        -a[2] * a[5] - 2 * a[3] * a[6] - 3 * a[4] * a[7];
...
// keep dx, dy for debugging in case the point diverges
                    ...
                    if (detector_params.spatial_convergence_threshold >
                        ...
                        detector_params.spatial_convergence_threshold >
                            fabs(dy)) {
                        // recover angles as roots of cubic equation (it assumes
                        // 0 indexed array, thus pass a+1):
                    ...
                    if (fabs(pt.x - initial[idx].x) > convergence_region ||
                        fabs(pt.y - initial[idx].y) > convergence_region) {
                        divergence_reason =
                            2; // departed from convergence region
                        break;
                    }
                }
                else
