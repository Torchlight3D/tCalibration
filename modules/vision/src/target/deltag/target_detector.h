...
static inline int FindBoardsHelper(
    const cv::Mat &image, const cv::Size &board_size,
...
template <class SaddlePointType, class FloatImageType = double>
static inline int FindBoards(
    const cv::Mat &I, const cv::Size &board_size,
    std::vector<orp::calibration::BoardObservation> &boards)
{
    switch (I.type()) {
...
public:
        ...
        : _indexer()
    {
        ...
        if (_indexer.board_defs.empty()) {
            throw std::invalid_argument("invalid target *.dsc file '" +
                                        target_dsc_fn + "'" +
             ...
             cv::Mat *debug_image = nullptr)
    {
        ...
        const cv::Size board_size(_indexer.chessboard_col,
                                  _indexer.chessboard_row);
        ...
        if (debug_image) {
            ...
            for (const auto &b : boards) {
                ...
                orp::calibration::drawCheckerboardCorners(
                    *debug_image, b, thickness, is_triangular);
            }
        }
    }
