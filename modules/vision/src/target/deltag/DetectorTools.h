                     ...
                     int lineType = cv::LINE_AA)
{
    ...
    cv::rectangle(img, cv::Point(int((pt.x - sz) * ss), int((pt.y - sz) * ss)),
                  cv::Point(int((pt.x + sz) * ss), int((pt.y + sz) * ss)),
...
inline void solveCubicPolynomial(const Float *a, std::complex<Float> roots[3])
{
    ...
    // - (a2*a3)/(6*a4^2))^2 + (a2/(3*a4) - a3^2/(9*a4^2))^3)^(1/2) -
    // a3^3/(27*a4^3)
    ...
    // a2/(3*a4))/(((a1/(2*a4) + a3^3/(27*a4^3) - (a2*a3)/(6*a4^2))^2 +
    // (a2/(3*a4)
...
template <typename PointType>
bool PointIsInf(const PointType &pt)
{
    return (std::isinf(pt.x) || std::isinf(pt.y));
}

template <typename PointType>
bool PointIsValid(const PointType &pt)
{
                    ...
                    double threshold = 2.0)
{
    ...
    for (size_t i = 0; i < pts.size(); i++) {
        ...
        // const int lbx = std::max(0, int(pt1.x - threshold)), lby =
        // std::max(0, int(pt1.y - threshold)),
        ...
        for (int y = lby; y <= uby; y++) {
            for (int x = lbx; x <= ubx; x++) {
                // go through relevant neighbouring clusters and check if they
                // need to be merged in based on the distance to this point
                ...
                for (size_t idx = 0; idx < other.idxs.size(); idx++) {
                    ...
                    if (dist < threshold2) {
                        ...
                        for (size_t k = 0; k < other.idxs.size(); k++) {
                            ...
                            cluster_ids[other.idxs[k]] =
                                cluster_id; // bug fixed by Hyowon !
                        }
                        other.idxs.clear();
                        num_clusters--;
                    }
