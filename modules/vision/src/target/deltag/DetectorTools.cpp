                                ...
                                const float histogramClippingLimit)
{
    ...
    if (type == CV_32F) {
        ...
        cv::normalize(inputMat, outputMat,
                      *(begin + int((end - begin) * histogramClippingLimit)),
                      255.f, cv::NORM_MINMAX);
    }
}
...
void stretchIntensities(cv::InputArray input, cv::OutputArray output)
{
    ...
    if (CV_MAT_TYPE(input.type()) == CV_8U ||
        ...
        // edges limits (here 5-95%)... usefull for HDR pixel errors
        // cancellation
        cv::Mat out;
    ...
    for (int r = 0; r < checkerboard.rows; ++r) {
        ...
        for (int c = 0; c < checkerboard.cols; ++c) {
            ...
            if (c + 1 < checkerboard.cols) {
                ...
                id2 = checkerboard.at<int>(r, c + 1);
                if (id1 != -1 && id2 != -1) {
                    subpixel_line(result, corners[id1], corners[id2], color,
                                  thickness);
                    if (obs.indexed) {
                        subpixel_marker(result, corners[id1], rs, color,
                                        thickness);
                        subpixel_marker(result, corners[id2], rs, color,
                                        thickness);
                        if (!index_shown) {
                            cv::putText(result, std::to_string(obs.board_id),
                                        cv::Point(int(corners[id1].x),
                                                  int(corners[id1].y)),
