                              ...
                              const cv::Mat& distortion)
{
    ...
    cv::projectPoints(ref_points, Rvec, Tvec, intrinsics, distortion,
                      out_points);
    double avg_dist = 0;
    double num_points = 0;
              ...
              unsigned int flag)
{
    ...
    for (unsigned int i = 0; i < detected.getNumSlots(); i++) {
        if (detected.getSlot(i).value() &&
            !detected.getSlot((i + off) % detected.getNumSlots()).discarded() &&
            !detected
                 .getSlot(i - off < 0 ? detected.getNumSlots() - off : i - off)
                 .discarded()) {
            const cv::Matx33d* Sq =
                &detected.getSlot(i).getPayload()->getEllipse();
            const cv::Matx33d* Qi =
                &detected.associatedModel()->modelEllipseAtSlot(i);
    ...
    for (unsigned int i = 0; i < N; i++) {
        ...
        ref_points[i] = cv::Point3f(static_cast<float>(model_center.x),
                                    static_cast<float>(model_center.y), 0.0);
        mark_points[i] = cv::Point2f(
            static_cast<float>(detec_center.x +
                               intrinsics.at<double>(0, 2) /*+0.5*/),
            static_cast<float>(detec_center.y +
                               intrinsics.at<double>(1, 2) /*+0.5*/));
    }

    cv::solvePnP(ref_points, mark_points, intrinsics, distortion, Rvec, Tvec,
                 false, method);
    if (pose_ok != 0)
        *pose_ok = true;

    std::cout << "Avg. Reprojection error: "
              << avg_reprojection_error(ref_points, mark_points, Rvec, Tvec,
    ...
    if ((static_cast<int>(flag) / 2) % 2 == 1) {
        ...
        for (unsigned int iterations = 0; iterations < 5; iterations++) {
            ...
            for (unsigned int kkk = 0; kkk < detec_ellipses.size(); kkk++) {
                ...
                mark_points[kkk] = cv::Point2f(
                    static_cast<float>(el_center(0, 0) * f +
                                       intrinsics.at<double>(0, 2)),
                    static_cast<float>(el_center(1, 0) * f +
            ...
            cv::solvePnP(ref_points, mark_points, intrinsics, distortion, Rvec,
                         Tvec, true, method);

            std::cout << " tvec delta norm: " << cv::norm(Tvec_old - Tvec)
                      << std::endl;
            std::cout << " Rvec delta norm: " << cv::norm(Rvec_old - Rvec)
                      << std::endl;

            // Calc reprojection error

            std::cout << "Avg. Reprojection error: "
                      << avg_reprojection_error(ref_points, mark_points, Rvec,
                                                Tvec, intrinsics, distortion)
                      << " px" << std::endl;
        }
        // END REFINE
    }
