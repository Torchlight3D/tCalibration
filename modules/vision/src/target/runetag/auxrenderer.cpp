                 ...
                 const cv::Mat& intrinsics)
{
    try {
        cv::ellipse(
            frame,
            cv::Point2f(
                static_cast<float>(el.center.x + intrinsics.at<double>(0, 2)),
               ...
               cv::Scalar color)
{
    ...
    frame.at<unsigned char>(i, j * 3 + 1) =
        static_cast<unsigned char>(color[1]);
    frame.at<unsigned char>(i, j * 3 + 2) =
...
void drawPoint(cv::Mat& frame, const cv::Point2f p, const cv::Mat& intrinsics)
{
    cv::circle(
        frame,
        cv::Point2f(static_cast<float>(p.x + intrinsics.at<double>(0, 2)),
                    static_cast<float>(p.y + intrinsics.at<double>(1, 2))),
        1, CV_RGB(255, 0, 255), 1);
}
...
{
    /*
    cv::Point2f pos( static_cast<float>(slot0.getCenter().x +
    intrinsics.at<double>(0,2)), static_cast<float>(slot0.getCenter().y +
    intrinsics.at<double>(1,2)) ); cvExt::its( frame ) << cvExt::its::position(
    pos ) << m.associatedModel()->name;
    ...
    */
    ...
    for( unsigned int i=0; i<m.getNumSlots(); i++ )
    {
        ...
        cv::Point2f pos( static_cast<float>( m.getSlot(i).getCenter().x +
    intrinsics.at<double>(0,2)), static_cast<float>( m.getSlot(i).getCenter().y
    ...
    */
    ...
    for (unsigned int i = 0; i < m.getNumSlots(); i++) {
        // if( m.getSlot(i).value() )
        {
            ...
            cv::Point2f pos( static_cast<float>(c.x +
            intrinsics.at<double>(0,2)), static_cast<float>(c.y +
            intrinsics.at<double>(1,2)) );*/
            cv::Point2f c = m.getSlot(i).getCenter();
            cv::Point2f pos(
                static_cast<float>(c.x + intrinsics.at<double>(0, 2)),
                          ...
                          const cv::Mat& distortion, cv::Scalar color)
{
    ...
    points.at<float>(0, 0) = -model_rad;
    points.at<float>(0, 1) = -model_rad;
    points.at<float>(0, 2) = 0.0;
    points.at<float>(1, 0) = model_rad;
    points.at<float>(1, 1) = -model_rad;
    points.at<float>(1, 2) = 0.0;
    points.at<float>(2, 0) = model_rad;
    points.at<float>(2, 1) = model_rad;
    points.at<float>(2, 2) = 0.0;
    points.at<float>(3, 0) = -model_rad;
    points.at<float>(3, 1) = model_rad;
    points.at<float>(3, 2) = 0.0;

    points.at<float>(4, 0) = -model_rad;
    points.at<float>(4, 1) = -model_rad;
    points.at<float>(4, 2) = -model_rad;
    points.at<float>(5, 0) = model_rad;
    points.at<float>(5, 1) = -model_rad;
    points.at<float>(5, 2) = -model_rad;
    points.at<float>(6, 0) = model_rad;
    points.at<float>(6, 1) = model_rad;
    points.at<float>(6, 2) = -model_rad;
    points.at<float>(7, 0) = -model_rad;
    points.at<float>(7, 1) = model_rad;
    points.at<float>(7, 2) = -model_rad;

    points.at<float>(8, 0) = 0.0f;
    points.at<float>(8, 1) = 0.0f;
    points.at<float>(8, 2) = 0.0f;
    ...
    its stream = its(frame);
    stream << its::position(out_points[8].x, out_points[8].y)
                               ...
                               const cv::Mat& distortion, cv::Scalar color)
{
    ...
    const unsigned int num_slots = m.associatedModel()->getNumSlots() /
                                   m.associatedModel()->getNumLayers();
    double angle = 6.283185 / (double)num_slots;

    std::vector<cv::Point2f> out_points;
    cv::Mat points(num_slots + 1, 3, CV_32F);
    points.at<float>(0, 0) = 0.0;
    points.at<float>(0, 1) = 0.0;
    points.at<float>(0, 2) = 0.0;
    ...
    for (unsigned int i = 1; i < num_slots + 1; i++) {
        ...
        points.at<float>(i, 0) =
            (float)(model_rad * cos(curr_angle + angle / 2.0));
        points.at<float>(i, 1) =
            (float)(model_rad * sin(curr_angle + angle / 2.0));
    ...
    // Project points
    cv::projectPoints(points, p.R, p.t, intrinsics, distortion, out_points);
    cv::projectPoints(points_outer, p.R, p.t, intrinsics, distortion,
                      out_points_outer);
...
{
    const double radius =
        1.0 * ((m.associatedModel()->getNumLayers() + 1) /
               (double)(m.associatedModel()->getNumLayers() * 2.0));

    float model_rad =
        static_cast<float>(m.associatedModel()->getWorldSize() * radius);
    const unsigned int num_slots = m.associatedModel()->getNumSlots();
    ...
    for (unsigned int i = 0; i < num_slots; i++) {
        ...
        if (m.associatedModel()->isInternal(k) &&
            m.associatedModel()->valueAtSlot(k)) {
                                   ...
                                   const cv::Mat& distortion)
{
    ...
    cv::projectPoints(bottom_vertices, p.R, p.t, intrinsics, distortion,
                      bottom_vertices_img);
    cv::projectPoints(top_vertices, p.R, p.t, intrinsics, distortion,
                      top_vertices_img);
    cv::projectPoints(marker_dots, p.R, p.t, intrinsics, distortion,
                      marker_dots_img);
    cv::projectPoints(marker_center, p.R, p.t, intrinsics, distortion,
                      marker_center_img);
                              ...
                              const cv::Mat& distortion)
{
    ...
    for (unsigned int i = 0; i < num_slots; i++) {
        ...
        // std::cout << k << std::endl;
        // cv::line( frame, cv::Point2f( C[0], C[1] ), cv::Point2f( C1[0], C1[1]
        // ), CV_RGB(0,0,0),1,CV_AA); cv::circle( frame, cv::Point2f(O[0],
        // O[1]), 2, CV_RGB(0,0,0),2 ); cv::circle( frame, cv::Point2f(C1[0],
        // C1[1]), 2, CV_RGB(0,0,0),2 );
    ...
    // Project points
    cv::projectPoints(points, p.R, p.t, intrinsics, distortion, out_points);
    cv::projectPoints(points_reflected, p.R, p.t, intrinsics, distortion,
                      out_points_reflected);
                ...
                const cv::Mat& intrinsics)
{
    cv::line(
        frame,
        cv::Point2f(static_cast<float>(center.x + intrinsics.at<double>(0, 2)),
                    static_cast<float>(center.y + intrinsics.at<double>(1, 2))),
        cv::Point2f(static_cast<float>(vector.x + center.x +
                                       intrinsics.at<double>(0, 2)),
                    static_cast<float>(vector.y + center.y +
                                       intrinsics.at<double>(1, 2))),
        color, 1);
}
              ...
              cv::Scalar color, const cv::Mat& intrinsics)
{
    cv::line(
        frame,
        cv::Point2f(static_cast<float>(p1.x + intrinsics.at<double>(0, 2)),
                    static_cast<float>(p1.y + intrinsics.at<double>(1, 2))),
        cv::Point2f(static_cast<float>(p2.x + intrinsics.at<double>(0, 2)),
                    static_cast<float>(p2.y + intrinsics.at<double>(1, 2))),
        color, 1);
}
