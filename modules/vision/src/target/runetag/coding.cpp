...
static long generator[43] = {1, 1, 6, 4, 6, 0, 3, 1, 5, 3, 5, 4, 0, 4, 6,
...
void Coding::Euclidean(NTL::zz_pEX& key, NTL::zz_pEX& locator, NTL::zz_pEX zeta,
                       long max_deg)
{
...
int Coding::decode(std::vector<long>& code_vec)
{
    ...
    long erasure_num = 0;
    for (long i = 0;
         i != std::min(code_length, static_cast<long>(code_vec.size())); ++i) {
    ...
    if (!NTL::IsZero(Sx)) {
        ...
        // Euclidean algorithm
        Euclidean(O, Lx, NTL::MulTrunc(Ex, Sx, end_range - start_range),
                  t + erasure_num);
        ...
        for (long pos = 0; pos != code_length; ++pos) {
            if (NTL::IsZero(eval(Lx, alpha[code_length - pos]))) {
                // error or erasure
                NTL::zz_pE e = alpha[code_length -
                                     (pos * (start_range - 1)) % code_length] *
                               eval(O, alpha[code_length - pos]) /
                               eval(DLx, alpha[code_length - pos]);
...
long Coding::generate(std::vector<long>& code, long index)
{
    ...
    for (long i = 0; i != code_length; ++i) code[i] = 0L;
    long start = 0L;
    while (index) {
    ...
    for (long i = 0; i != code_length; ++i) code[i] %= 7;
