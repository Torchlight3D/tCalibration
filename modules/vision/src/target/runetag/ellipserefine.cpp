                                ...
                                cv::Matx33d& out, cv::Mat dbg)
{
    ...
    for (int i = topleft.y; i < bottomright.y; i++) {
        for (int j = topleft.x; j < bottomright.x; j++) {
            // if( !point_inside( cv::Point2f( (float)j, (float)i ), min_points
            // ) && point_inside( cv::Point2f( (float)j, (float)i ), max_points
            // ) ) {

            double dk1 =
                std::sqrt((f1_x - j) * (f1_x - j) + (f1_y - i) * (f1_y - i));
            double dk2 =
                std::sqrt((f2_x - j) * (f2_x - j) + (f2_y - i) * (f2_y - i));
            if ((dk1 + dk2 < min_fsum) || (dk1 + dk2 > max_fsum))
                continue;

            // cv::circle( dbg, cv::Point2f( (float)j, (float)i ), 1,
            // CV_RGB(0,255,0) );
            ...
            const double Iui =
                ((double)gradient_x.at<signed short>(i, j) / 32768.0);
            const double Ivi =
                ((double)gradient_y.at<signed short>(i, j) / 32768.0);
