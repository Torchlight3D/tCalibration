    ...
    do {
        ...
        if (!models.empty()) {
            if (!models.begin()->second.compatible(newmodel)) {
                throw MarkerDetector::MarkerDetectorException(
                    "Trying to load a MarkerModel witch is not compatible with "
...
bool MarkerDetector::tryFit(SlotFitter& fitter,
...
int MarkerDetector::detectMarkers(const std::vector<cv::RotatedRect>& ellipses,
                                  std::vector<MarkerDetected>& markers_detected)
{
    ...
    for (unsigned int i = 0; i < ellipsePoints.size(); i++) {
        for (unsigned int j = i + 1; j < ellipsePoints.size(); j++) {
            ...
            SlotFitter sf_fit1(
                fitter.getVR(), ellipsePoints, fitter.getFit1WithOffset(-2.3),
                fitter.getFit1WithOffset(2.3), intrinsics, min_pts_for_level);
            if (!sf_fit1.isValid()) {
                continue;
            }
            ...
            SlotFitter sf_fit2(
                fitter.getVR(), ellipsePoints, fitter.getFit2WithOffset(-2.3),
                fitter.getFit2WithOffset(2.3), intrinsics, min_pts_for_level);
            if (!sf_fit2.isValid()) {
                continue;
            }
    ...
    std::vector<MarkerDetected>& markers_detected)
{
    ...
    bool found = false;
    for (unsigned int layer_guess = 0; layer_guess < possible_markers.size();
         layer_guess++) {
        ...
        try {
            ...
            if (coding.decode(detected_code) == 0) {
                ...
                if (models.find(idx_detected) != models.end()) {
                    ...
                    // Apply rotation
                    curr_det.associateModel(&(models[idx_detected]),
                                            curr_det.offset + rotation * 3);

                    // Remove errors (this is needed to avoid problems later on
