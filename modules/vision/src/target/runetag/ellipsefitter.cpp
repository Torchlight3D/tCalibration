...
EllipseFitter::CandidateVR::CandidateVR(const cv::Matx33d& _VR1,
                                        ...
                                        const cv::Matx33d& _VR2,
                                        const cv::Matx31d& _normal2)
    : VR1(&_VR1), normal1(&_normal1), VR2(&_VR2), normal2(&_normal2)
{
}
...
bool EllipseFitter::chooseAvgVR(const EllipsePoint& ep1,
                                const EllipsePoint& ep2, double& r,
                                double& r_other, cv::Matx33d& Qc,
                                cv::Matx33d& Qc_other)
{
    ...
    CandidateVR q1vr1_q2vr1(ep1.getVR1(), Q1_VR1_normal, ep2.getVR1(),
                            Q2_VR1_normal);
    CandidateVR q1vr1_q2vr2(ep1.getVR1(), Q1_VR1_normal, ep2.getVR2(),
                            Q2_VR2_normal);
    CandidateVR q1vr2_q2vr1(ep1.getVR2(), Q1_VR2_normal, ep2.getVR1(),
                            Q2_VR1_normal);
...
cv::Matx33d EllipseFitter::getFitWithOffset(double rad_mul, cv::Matx33d& who,
                                            double k) const
{
    const double f =
...
cv::Matx33d& EllipseFitter::transformToCircle(const cv::Matx33d& Q,
                                              const cv::Matx33d& VR,
...
double EllipseFitter::circleRonZ0(const cv::Matx33d& Qc)
{
    ...
    double denum =
        (b * b - a * c) * ((a + c) * (a + c) - ((a - c) * (a - c) + 4 * b * b));
    return -2 * num * (a + c) / denum;
}

bool EllipseFitter::fitCircles(const cv::Point2d& center1,
                               ...
                               cv::Matx33d& circle1, cv::Matx33d& circle2)
{
    ...
    const double asqrt =
        (-(x1 - x2) * (x1 - x2) *
         (x1 * x1 - 2 * x1 * x2 + x2 * x2 + (y1 - y2) * (y1 - y2)) *
         (-4 * radius2 + x1 * x1 - 2 * x1 * x2 + x2 * x2 + y1 * y1 -
          2 * y1 * y2 + y2 * y2));
    if (asqrt < 0.0) {
        return false;
    }
    ...
    const double anum =
        (-x1 * x1 * x1 * x1 + 2 * x1 * x1 * x1 * x2 - 2 * x1 * x2 * x2 * x2 +
         x2 * x2 * x2 * x2 - x1 * x1 * (y1 - y2) * (y1 - y2) +
         x2 * x2 * (y1 - y2) * (y1 - y2));
    const double adenum =
        (2 * (x1 - x2) *
         (x1 * x1 - 2 * x1 * x2 + x2 * x2 + (y1 - y2) * (y1 - y2)));
    ...
    const double bnum =
        -(y1 * y1 * y1 - y1 * y1 * y2 - y1 * y2 * y2 + y2 * y2 * y2 +
          x1 * x1 * (y1 + y2) - 2 * x1 * x2 * (y1 + y2) + x2 * x2 * (y1 + y2));
    const double bdenum =
        (2 * (x1 * x1 - 2 * x1 * x2 + x2 * x2 + (y1 - y2) * (y1 - y2)));
...
void EllipseFitter::buildCircle(cv::Matx33d& m, double a, double b,
                                double radius)
{
    m = cv::Matx33d();
...
{
    double magnitude =
        sqrt(vector(0, 0) * vector(0, 0) + vector(1, 0) * vector(1, 0) +
