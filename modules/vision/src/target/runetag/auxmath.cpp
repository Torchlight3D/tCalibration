...
double RDelta(const cv::Mat& R1, const cv::Mat& R2)
{
    ...
    return acos(rv1(0, 0) * rv2(0, 0) + rv1(1, 0) * rv2(1, 0) +
                rv1(2, 0) * rv2(2, 0));
}
...
cv::RotatedRect ellipseToRotatedRect(const cv::Matx33d& ellipse)
{
    ...
    if (b * b - 4.0 * a * c >= 0.0) {
        return cv::RotatedRect(cv::Point2f(0.0f, 0.0f), cv::Size2f(-1.0, -1.0),
                               0.0f);
    }
    cv::RotatedRect rr;
    ...
    const double num =
        2.0 * (a * f * f + c * d * d + g * b * b - 2 * b * d * f - a * c * g);
    double a1 =
        sqrt(num / (b2minac * (sqrt((a - c) * (a - c) + 4 * b * b) - (a + c))));
    double b1 = sqrt(
        num / (b2minac * (-sqrt((a - c) * (a - c) + 4 * b * b) - (a + c))));
...
double ellipseRadius(const cv::Matx33d& Qc)
{
    ...
    const double denum = (b * b - a * c) * (b * b - a * c) *
                         ((a + c) * (a + c) - ((a - c) * (a - c) + 4 * b * b));
