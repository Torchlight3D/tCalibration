    ...
    const cv::Mat& frame, std::vector<cv::RotatedRect>& detected)
{
    ...
    for (std::vector<std::vector<cv::Point>>::const_iterator it =
        ...
        if (cont.size() < min_ellipse_contour_points ||
            cont.size() > max_ellipse_contour_points) {
        ...
        if (ratio < min_required_roundness ||
            ratio > 1.0 / min_required_roundness) {
        ...
        for (unsigned int i = 0; i < cont.size(); i++) {
            ...
            dk1 = std::sqrt((f1_x - contour_pt_x) * (f1_x - contour_pt_x) +
                            (f1_y - contour_pt_y) * (f1_y - contour_pt_y));
            dk2 = std::sqrt((f2_x - contour_pt_x) * (f2_x - contour_pt_x) +
                            (f2_y - contour_pt_y) * (f2_y - contour_pt_y));
        ...
        // This partially compensates the fact that adaptiveThreshold
        // overestimate the size of the ellipses.
        ellipse.size.width += size_compensation;
        ellipse.size.height += size_compensation;

        detected.push_back(ellipse);
        ++num_detected;
        // cv::ellipse( cv::Mat(frame), ellipse.center, cv::Size(
        // ellipse.size.width/2.0, ellipse.size.height/2.0 ), ellipse.angle, 0,
        // 360, CV_RGB(255,0,0), 1 );

        /*
        for( unsigned int k=0; k<contour_points.size(); k++ ) {
            cv::circle( cv::Mat(frame), contour_points[k], 1, CV_RGB(0,255,0),
        1);
        }*/
    }
