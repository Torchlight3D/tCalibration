...
SlotFitter::SlotFitter(const cv::Matx33d& _VR,
                       ...
                       const cv::Mat& _intrinsics, int min_support,
                       int min_symbols_for_layer)
    : ellipses(_ellipses), _min_symbols_for_layer(min_symbols_for_layer)
{
    VR = cv::Matx33d(_VR);
    intrinsics = _intrinsics.clone();
    refit = cv::Matx33d();

    for (std::vector<EllipsePoint>::const_iterator it = ellipses.begin();
         it < ellipses.end(); it++) {
        ...
        if (A_min * x * x + 2.0 * B_min * x * y + C_min * y * y +
            ...
            if (A_max * x * x + 2.0 * B_max * x * y + C_max * y * y +
...
void SlotFitter::fit(std::vector<EllipsePoint>& ring_ellipses,
                     double radius_ratio, double gap_factor, int num_layers,
                     std::vector<MarkerDetected>& possible_markers)
{
    ...
    double f =
        (intrinsics.at<double>(0, 0) + intrinsics.at<double>(1, 1)) / 2.0;
    ...
    for (int i = 0; i < num_slots_for_layer; i++) {
        ...
        cv::Point2d curr_slot_p(refit_center.x + r * cos(angle) * 2.0,
                                refit_center.y + r * sin(angle) * 2.0);
        fit_slots.push_back(
            transformPointFromCircleToEllipse(curr_slot_p, VR, f));

        std::vector<cv::Point2f> level_slots;
        for (int j = 0; j < num_layers; j++) {
            const double factor =
                (((num_layers + j + 1) * 2.0) / (double)(num_layers * 4.0));
            cv::Point2f curr_slot_centerp(
                refit_center.x + r * factor * cos(angle - alpha * 0.5),
                refit_center.y + r * factor * sin(angle - alpha * 0.5));
            level_slots.push_back(
                transformPointFromCircleToEllipse(curr_slot_centerp, VR, f));
        }
        fit_slots_centers.push_back(level_slots);
    }

    for (int i = 0; i < num_layers; i++) {
        MarkerDetected newmarker =
            MarkerDetected::createDetectedMarker(num_layers);
        ...
        for (size_t h = 0; h < points_coords_size; h++) {
            ...
            if (ellipseContains(qmax, p)) {
                ...
                if (!ellipseContains(qmin, p)) {
                    // This ellipse is outside qmin and hence is a good
                    ...
                    for (unsigned int i = 0; i < fit_slots_size; i++) {
                        ...
                        const cv::Point2d& v1 = cv::Point2d(
                            fit_slots[i].x - c.x, fit_slots[i].y - c.y);
                        const cv::Point2d& v2 = cv::Point2d(
                            fit_slots[k].x - c.x, fit_slots[k].y - c.y);
                        ...
                        curr_slot.v2 = v2;
                        curr_slot.c = c;
                        curr_slot.slot_center =
                            (fit_slots_centers[k])[f_level - 1];
...
cv::Matx33d SlotFitter::getFitWithOffset(int currLayer, unsigned int num_layers,
                                         double radiusSqr) const
{
    ...
    const double factor =
        (((num_layers + currLayer + 1) * 2.0 + 1) / (double)(num_layers * 4.0));
    double rz0sqr = radiusSqr * factor * factor;

    refit(2, 2) =
        -rz0sqr + refit(0, 2) * refit(0, 2) + refit(1, 2) * refit(1, 2);
...
bool SlotFitter::ellipseContains(const cv::Matx33d& ellipse,
                                 const cv::Point2d& p)
{
    // Layer boundary
    const double& x = p.x;
...
cv::Point2d SlotFitter::transformPointFromCircleToEllipse(cv::Point2d p,
                                                          const cv::Matx33d& VR,
                                                          double f)
{
