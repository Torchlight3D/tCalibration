...
DigitalMarkerModel::DigitalMarkerModel(std::ifstream& ifs)
{
    ...
    if (startString.compare(std::string("RUNE_direct")) != 0) {
        throw MarkerModelLoadException(
            "Unable to read a MarkerModel descriptor.");
    }
    ...
    if (num_layers != 3) {
        throw MarkerModelLoadException(
            "Invalid marker model, 3 layers expected");
    }
    ...
    for (unsigned int i = 0; i < num_slots; i++) {
        ...
        if (b) {
            ...
            if (ifs.fail()) {
                throw MarkerModelLoadException(
                    "Error while reading ellipse model data");
            }
            model_ellipses[i] = me;
        }
    }
    ...
    if (coding.decode(decoded) != 0) {
        throw MarkerModelLoadException(
            "Invalid code. (Coder was unable to decode the given code)");
    }
    for (size_t i = 0; i < decoded.size(); ++i) {
        if (decoded[i] != code[i])
            throw MarkerModelLoadException(
                "Invalid code. (A correction is required)");
    }
...
DigitalMarkerModel& DigitalMarkerModel::operator=(
    const DigitalMarkerModel& other)
{
    if (this == &other) {
        return *this;
    }
...
bool DigitalMarkerModel::alignAndCheck(MarkerDetected& d) const
{
    throw MarkerModelOperationException(
        "Direct RUNE tag must not be aligned like this");
    return false;
#if 0
