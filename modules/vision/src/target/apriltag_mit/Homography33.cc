...
#ifdef STABLE_H
void Homography33::setCorrespondences(const std::vector<cv::Point2f> &sPts,
                                      const std::vector<cv::Point2f> &dPts)
{
    valid = false;
    srcPts = sPts;
    dstPts = dPts;
...
void Homography33::compute()
{
    ...
    for (int i = 0; i < 4; i++) {
        srcPoints.push_back(srcPts[i]);
    }
    for (int i = 0; i < 4; i++) {
        dstPoints.push_back(dstPts[i] - cxy);
    }

    const auto homography = cv::findHomography(srcPoints, dstPoints);
...
cv::Point2f Homography33::project(float worldx, float worldy)
{
    compute();

    cv::Point2f ixy;
    ixy.x = H(0, 0) * worldx + H(0, 1) * worldy + H(0, 2);
    ixy.y = H(1, 0) * worldx + H(1, 1) * worldy + H(1, 2);
