...
#pragma warning(default : 4996)
    ...
    while (i < lines->noLines) {
        ...
        for (int j = firstLineIndex; j <= lastLineIndex; j++) {
            ...
            fprintf(fp,
                    "|   %3d |    %3d    "
                    "|%7.1lf|%9.5lf|%6d|%8.1lf|%8.1lf|%8.1lf|%8.1lf|%8.1lf|%8."
                    "1lf|%8.1lf|\n",
                    j, lines->lines[j].segmentNo, lines->lines[j].a,
                    lines->lines[j].b, lines->lines[j].invert,
                    lines->lines[j].sx, lines->lines[j].sy, lines->lines[j].ex,
                    lines->lines[j].ey, angle, dist, len);
        } // end-for
    }     // end-for
    fprintf(fp, "+=======+===========+=======+=========+===============+======="
                "=+========+========+========+========+========+\n");
                        ...
                        double MAX_ERROR)
{
    ...
    while (i < lines->noLines) {
        ...
        if (firstLineIndex != lastLineIndex) {
            if (TryToJoinTwoLineSegments(
                    &lines->lines[firstLineIndex], &lines->lines[lastLineIndex],
                        ...
                        EDLines *lines)
{
    ...
    while (noPixels >= MIN_LINE_LEN) {
        ...
        while (index < noPixels) {
            ...
            int badPixelCount = 0;
            while (index < noPixels) {
                double d = ComputeMinDistance(x[index], y[index], lastA, lastB,
                                              lastInvert);
                ...
                if (goodPixelCount % 10 == 0)
                    LineFit(x, y, lastGoodIndex - startIndex + len + 1, &lastA,
                            &lastB, lastInvert);
                // Burak - Fits a new line every 10 good pixel
                index++;
            } // end-while
            ...
            if (goodPixelCount >= 2) {
                ...
                LineFit(x, y, len, &lastA, &lastB,
                        lastInvert); // faster LineFit
            ...
            if (goodPixelCount < 2 || index >= noPixels) {
                ...
                while (ComputeMinDistance(x[index], y[index], lastA, lastB,
                    ...
                    index++;
                ComputeClosestPoint(x[index], y[index], lastA, lastB,
                                    lastInvert, &sx, &sy);
...
#undef PRECISON_ANGLE
    ...
    for (int i = 0; i < lines->noLines; i++) {
        ...
        Pixel *pixels =
            &map->segments[ls->segmentNo].pixels[ls->firstPixelIndex];
        int noPixels = ls->len;
        ...
        if (ls->len >= 80) {
            ...
            // Validate short line segments by a line support region rectangle
            // having width=2
        }
        else if (ls->len <= 25) {
            valid =
                ValidateLineSegmentRect(srcImg, width, height, x, y, ls, LUT);
        }
        else {
            ...
            // invalid, then a line support region rectangle having width=2 is
            // tried If the line segment fails both tests, it is discarded
            int aligned = 0;
            int count = 0;
            for (int j = 0; j < noPixels; j++) {
                ...
                // com1 = (H-A)
                // com2 = (C-F)
                // Then: gx = com1 + com2 + (E-D) = (H-A) + (C-F) + (E-D) =
                // (C-A) + (E-D) + (H-F)
                //       gy = com2 - com1 + (G-B) = (H-A) - (C-F) + (G-B) =
                //       (F-A) + (G-B) + (H-C)
                //
                int com1 = srcImg[(r + 1) * width + c + 1] -
                           srcImg[(r - 1) * width + c - 1];
                int com2 = srcImg[(r - 1) * width + c + 1] -
                           srcImg[(r + 1) * width + c - 1];

                int gx = com1 + com2 + srcImg[r * width + c + 1] -
                         srcImg[r * width + c - 1];
            ...
            if (valid == false)
                valid = ValidateLineSegmentRect(srcImg, width, height, x, y, ls,
                                                LUT);
        } // end-else
            ...
            if ((double)x < vx[3]) {
                ...
                if (fabs(vx[0] - vx[3]) <= 0.01) {
                    if (vy[0] < vy[3])
                        ...
                        ys = vy[0] +
                             (x - vx[0]) * (vy[3] - vy[0]) / (vx[3] - vx[0]);
                }
                else
                    ys =
                        vy[0] + (x - vx[0]) * (vy[3] - vy[0]) / (vx[3] - vx[0]);
            }
            else {
                /* interpolation */
                if (fabs(vx[3] - vx[2]) <= 0.01) {
                    if (vy[3] < vy[2])
                        ...
                        ys = vy[3] +
                             (x - vx[3]) * (y2 - vy[3]) / (vx[2] - vx[3]);
                }
                else
                    ys =
                        vy[3] + (x - vx[3]) * (vy[2] - vy[3]) / (vx[2] - vx[3]);
            } // end-else
            ...
            if ((double)x < vx[1]) {
                ...
                if (fabs(vx[0] - vx[1]) <= 0.01) {
                    if (vy[0] < vy[1])
                        ...
                        ye = vy[0] +
                             (x - vx[0]) * (vy[1] - vy[0]) / (vx[1] - vx[0]);
                }
                else
                    ye =
                        vy[0] + (x - vx[0]) * (vy[1] - vy[0]) / (vx[1] - vx[0]);
            }
            else {
                /* interpolation */
                if (fabs(vx[1] - vx[2]) <= 0.01) {
                    if (vy[1] < vy[2])
                        ...
                        ye = vy[1] +
                             (x - vx[1]) * (vy[2] - vy[1]) / (vx[2] - vx[1]);
                }
                else
                    ye =
                        vy[1] + (x - vx[1]) * (vy[2] - vy[1]) / (vx[2] - vx[1]);
            } // end-else
...
int ComputeMinLineLength(int width, int height)
{
    ...
    // because we now test short line segments by a line support region
    // rectangle having width=2. This means that within a line support region
    // rectangle for a line segment of length "l" there are "2*l" many pixels.
    // Thus, a line segment of length "l" has a chance of getting validated by
    // NFA.
...
EDLines *DetectLinesByED(EdgeMap *&map, unsigned char *srcImg, int width,
                         ...
                         EDLines *invalidLines)
{
    // Make sure, smoothingSigma <= 2.0. Higher values do not produce good
    // results
...
#define USE_GX_GY_PLUS
    ...
    // we are computing the gradient over the smoothed image, a smaller value
    // must be used. 8 seems to give results closest to the theoretical
    // threshold value, and thus results closest to LSD.
  ...
  ValidateLineSegments(map, srcImg, lines, invalidLines);
    ...
    double validationSigma =
        smoothingSigma / 2.5; // Gives one false detection in noisy images, but
