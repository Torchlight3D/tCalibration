...
void StdRGB2Lab(unsigned char *redImg, unsigned char *greenImg,
                unsigned char *blueImg, unsigned char *LImg,
                unsigned char *aImg, unsigned char *bImg, int width, int height)
{
    // First RGB 2 XYZ
    double red, green, blue;
    double x, y, z;
...
#define YELLOW 70.0
    ...
    for (int i = 0; i < width * height; i++) {
        ...
        X = 0.412453 * redImg[i] + 0.357580 * greenImg[i] +
            0.180423 * blueImg[i];
        Y = 0.212671 * redImg[i] + 0.715160 * greenImg[i] +
            0.072169 * blueImg[i];
        Z = 0.019334 * redImg[i] + 0.119193 * greenImg[i] +
            0.950227 * blueImg[i];
