                                ...
                                double *H)
{
    ...
    for (int i = 1; i < height - 1; i++) {
        for (int j = 1; j < width - 1; j++) {
            ...
            //       (G-B) + (H-C)
            //
            int com1 = srcImg[(i + 1) * width + j + 1] -
                       srcImg[(i - 1) * width + j - 1];
            int com2 = srcImg[(i - 1) * width + j + 1] -
                       srcImg[(i + 1) * width + j - 1];
...
#define MIN_PATH_LEN 10
    ...
    if (chainLen < MIN_PATH_LEN)
        ...
        /// Test from index1 to index2. If OK, then we are done. Otherwise,
        /// split into two and recursively test the left & right halves
  ...
  for (int k=index1; k<=index2; k++){
    ...
    for (int k = index1; k <= index2; k++) {
        ...
        int dx = abs(map->segments[i].pixels[k].c -
                     map->segments[i].pixels[k - 1].c);
        int dy = abs(map->segments[i].pixels[k].r -
                     map->segments[i].pixels[k - 1].r);
        if (dx == 1 && dy == 1)
            count2++; // diagonal
        else
...
//  np *= 32;
    ...
    for (int i = 0; i < map->noSegments; i++) {
        TestSegment(map, gradImg, i, 0, map->segments[i].noPixels - 1, np, H,
                    divForTestSegment);
        //    TestSegment2(map, gradImg, i, 0, map->segments[i].noPixels-1, np,
        //    H, divForTestSegment);
    } // end-for
                           ...
                           double divForTestSegment)
{
    ...
    for (int i = 1; i < height - 1; i++) {
      ...
      int g = gx + gy;
            ...
            // 45 degrees
            G = abs(
                srcImg[i * width + j + 1] + srcImg[(i + 1) * width + j + 1] +
            ...
            // 135 degrees
            G = abs(
                srcImg[(i - 1) * width + j] + srcImg[(i - 1) * width + j + 1] +
            ...
            // 0 degrees
            G = abs(srcImg[(i - 1) * width + j - 1] +
                    2 * srcImg[(i - 1) * width + j] +
                    srcImg[(i - 1) * width + j + 1] -
                    srcImg[(i + 1) * width + j - 1] -
                    2 * srcImg[(i + 1) * width + j] -
                    srcImg[(i + 1) * width + j + 1]);
                                    ...
                                    unsigned char *maps[], int noMaps)
{
    ...
    for (double validationDiv = 1; validationDiv <= 8.5;
        ...
        // Validate segments
        for (int i = 0; i < map->noSegments; i++) {
            TestSegment(map, gradImg, i, 0, map->segments[i].noPixels - 1, np,
                        H, validationDiv);
        } // end-for
                          ...
                          double divForTestSegment)
{
    ...
    for (int i = 1; i < height - 1; i++) {
        for (int j = 1; j < width - 1; j++) {
            ...
            int com1 = ch1Img[(i + 1) * width + j + 1] -
                       ch1Img[(i - 1) * width + j - 1];
            int com2 = ch1Img[(i - 1) * width + j + 1] -
                       ch1Img[(i + 1) * width + j - 1];
            ...
            com1 = ch2Img[(i + 1) * width + j + 1] -
                   ch2Img[(i - 1) * width + j - 1];
            com2 = ch2Img[(i - 1) * width + j + 1] -
                   ch2Img[(i + 1) * width + j - 1];
            ...
            com1 = ch3Img[(i + 1) * width + j + 1] -
                   ch3Img[(i - 1) * width + j - 1];
            com2 = ch3Img[(i - 1) * width + j + 1] -
                   ch3Img[(i + 1) * width + j - 1];
                           ...
                           double divForTestSegment)
{
    ...
    for (int i = 1; i < height - 1; i++) {
      ...
      int grad = (ch1Grad + ch2Grad + ch3Grad + 2)/3;
            ...
            int gyCh1 =
                ch1Img[(i - 1) * width + j - 1] -
                ch1Img[(i + 1) * width + j - 1] + ch1Img[(i - 1) * width + j] -
                ch1Img[(i + 1) * width + j] + ch1Img[(i - 1) * width + j + 1] -
                ch1Img[(i + 1) * width + j + 1];
            ...
            int gyCh2 =
                ch2Img[(i - 1) * width + j - 1] -
                ch2Img[(i + 1) * width + j - 1] + ch2Img[(i - 1) * width + j] -
                ch2Img[(i + 1) * width + j] + ch2Img[(i - 1) * width + j + 1] -
                ch2Img[(i + 1) * width + j + 1];
            ...
            int gyCh3 =
                ch3Img[(i - 1) * width + j - 1] -
                ch3Img[(i + 1) * width + j - 1] + ch3Img[(i - 1) * width + j] -
                ch3Img[(i + 1) * width + j] + ch3Img[(i - 1) * width + j + 1] -
                ch3Img[(i + 1) * width + j + 1];
            ...
            //      2*gxy*sin(theta)*cos(theta) + gyy*sin(theta)*sin(theta)) +
            //      0.5);  // Where did I get this from?
    ...
    if (op == 1) {
        ...
        for (int i = 1; i < height - 1; i++) {
            for (int j = 1; j < width - 1; j++) {
                ...
                int com1 =
                    ch1Img[(i + 1) * width + j + 1] - ch1Img[i * width + j];
                int com2 =
                    ch1Img[i * width + j + 1] - ch1Img[(i + 1) * width + j];
