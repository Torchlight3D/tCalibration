    ...
    for (int i = 1; i < height - 1; i++) {
        for (int j = 1; j < width - 1; j++) {
            ...
            gx = (int)(sqrt((double)gxCh1 * gxCh1 + gxCh2 * gxCh2 +
                            gxCh3 * gxCh3) +
                       0.5);
            gy = (int)(sqrt((double)gyCh1 * gyCh1 + gyCh2 * gyCh2 +
                            gyCh3 * gyCh3) +
                       0.5);
                                    ...
                                    int height)
{
    ...
    for (int i = 2; i < height - 2; i++) {
        for (int j = 2; j < width - 2; j++) {
            ...
            // Prewitt for channel2
            int gxCh2 = abs((smoothCh2Img[(i - 2) * width + j + 1] -
                                    ...
                                    int height)
{
    ...
    for (int i = 3; i < height - 3; i++) {
        for (int j = 3; j < width - 3; j++) {
            // Prewitt for channel1
            int gxCh1 = abs((smoothCh1Img[(i - 3) * width + j + 1] -
            ...
            // Prewitt for channel2
            int gxCh2 = abs((smoothCh2Img[(i - 3) * width + j + 1] -
            ...
            // Prewitt for channel3
            int gxCh3 = abs((smoothCh3Img[(i - 3) * width + j + 1] -
                                 ...
                                 unsigned char *dirImg, int GRADIENT_THRESH)
{
    ...
    for (int i = 1; i < height - 1; i++) {
        unsigned char *prevLine =
            (unsigned char *)(smoothImg->imageData +
                                 ...
                                 unsigned char *dirImg, int width, int height)
{
    ...
    for (int i = 1; i < height - 1; i++) {
        for (int j = 1; j < width - 1; j++) {
            ...
            int grad = (int)(sqrt(gxx * cosTheta * cosTheta +
                                  2 * gxy * sinTheta * cosTheta +
...
#define MYPI 3.14159
            ...
            if (theta <= (-3 * MYPI) / 8 || theta >= (3 * MYPI) / 8)
                ...
                dirImg[i * width + j] =
                    EDGE_135; // 135 degree gradient, 45 degree edge  (due to y
                              // axis being in the opposite direction)
            else if (theta >= MYPI / 8)
                dirImg[i * width + j] =
                    EDGE_45; // 45 degree gradient, 135 degree edge (due to y
                             // axis being in the opposite direction)
            else
                dirImg[i * width + j] =
