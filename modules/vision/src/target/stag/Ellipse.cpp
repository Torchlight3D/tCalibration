...
customEllipse::customEllipse(cv::Point *points, int noPnts)
{
    ...
    fitPoints = (cv::Point *)malloc(noPoints * sizeof(cv::Point));
    memcpy(fitPoints, points, noPoints * sizeof(cv::Point));
...
cv::Point customEllipse::GetCenter()
{
    cv::Point tmp;
    tmp.x = (int)cX;
    tmp.y = (int)-cY;
...
cv::Point *customEllipse::DrawEllipse(int resolution)
{
    if (drawPoints != NULL)
        return drawPoints;

    drawPoints = (cv::Point *)malloc(resolution * sizeof(cv::Point));
...
inline double customEllipse::DistanceBwPoints(cv::Point p1, cv::Point p2)
{
    int xDist, yDist;
    double distance;
...
inline double customEllipse::SquaredDistanceBwPoints(cv::Point p1, cv::Point p2)
{
    int xDist, yDist;
    double distance;
...
double customEllipse::GetDistance(cv::Point point, double &estimation)
{
    ...
    cv::Point transPnt; // translated and rotated point
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (distances[i] < distances[minDistPnt])
            minDistPnt = i;
    }
...
double customEllipse::GetDistance(double pX, double pY, double &estimation)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        distances[i] = DistanceBwPoints(transPntX, transPntY, candidatesX[i],
                                        candidatesY[i]);
...
double customEllipse::GetSquaredDistance(cv::Point point, double &estimation)
{
    // for different initial values for 4 estimations (in radians)
    double th0[4], estimations[4];
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (sqDistances[i] < sqDistances[minDistPnt])
            minDistPnt = i;
    }
    estimation = estimations[minDistPnt];
                                         ...
                                         double &estimation)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        distances[i] = SquaredDistanceBwPoints(transPntX, transPntY,
                                               candidatesX[i], candidatesY[i]);
...
cv::Point *customEllipse::GetClosestPoints()
{
    if (closePoints != NULL)
        return closePoints;

    closePoints = (cv::Point *)malloc(noPoints * sizeof(cv::Point));
...
double customEllipse::GetClosestPointAndDistance(cv::Point test,
                                                 cv::Point &closest)
{
               ...
               double &centerX, double &centerY, double &radius)
{
    ...
    // radius is 0.4, if we tolerate 5% deviation, MSE must be at most 0.4 *
    // 0.05 = 0.02 NOTE: I changed this 0.02 -> 0.0002, because observed
...
void customEllipse::getEllipseSamples(int noOfSamples,
