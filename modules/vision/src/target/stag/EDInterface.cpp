...
void EDInterface::correctLineDirection(const cv::Mat& image, LineSegment& ls)
{
    ...
    if (ls.invert == 0) {
        minX = (int)std::min(ls.sx, ls.ex);
        maxX = (int)(std::max(ls.sx, ls.ex) + 0.5);
        pointsToSample = maxX - minX + 1;
    }
    else {
        minY = (int)std::min(ls.sy, ls.ey);
        maxY = (int)(std::max(ls.sy, ls.ey) + 0.5);
    ...
    std::vector<cv::Point> right(pointsToSample);
    std::vector<cv::Point> left(pointsToSample);
    ...
    if (ls.invert == 0) // if the line is horizontal
    {
        if (ls.sx < ls.ex) // if the line goes left to right
        {
            for (int i = 0; i < pointsToSample; i++) {
                ...
                right[i] = cv::Point((int)neutX, (int)round(neutY + offset));
                left[i] = cv::Point((int)neutX, (int)round(neutY - offset));
            }
        }
    ...
    else // if the line is vertical
    {
        if (ls.sy < ls.ey) // if the line goes up to down
        {
            for (int i = 0; i < pointsToSample; i++) {
                ...
                right[i] = cv::Point((int)round(neutX - offset), (int)neutY);
                left[i] = cv::Point((int)round(neutX + offset), (int)neutY);
            }
        }
    ...
    // if so, use safe read
    minX = std::min(std::min(right[0].x, right.back().x),
                    std::min(left[0].x, left.back().x));
    maxX = std::max(std::max(right[0].x, right.back().x),
                    std::max(left[0].x, left.back().x));
    minY = std::min(std::min(right[0].y, right.back().y),
                    std::min(left[0].y, left.back().y));
    maxY = std::max(std::max(right[0].y, right.back().y),
                    std::max(left[0].y, left.back().y));
...
cv::Point2d EDInterface::intersectionOfLineSegments(const LineSegment& line1,
