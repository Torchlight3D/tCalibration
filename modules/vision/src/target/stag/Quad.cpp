...
Quad::Quad(std::vector<cv::Point2d> inCorners)
{
    corners = inCorners;
...
void Quad::estimateHomography()
{
    ...
    std::vector<cv::Point2d> affineCorners = std::vector<cv::Point2d>(4);
...
void Quad::calculateProjectiveDistortion()
{
    ...
    for (int i = 1; i < 4; i++) {
        curDist = abs(lineInf.x * corners[i].x + lineInf.y * corners[i].y +
                      lineInf.z);
        if (curDist < minDist)
            minDist = curDist;
        if (curDist > maxDist)
