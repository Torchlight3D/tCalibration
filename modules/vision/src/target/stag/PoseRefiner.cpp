...
void PoseRefiner::refineMarkerPose(EDInterface* edInterface, Marker& marker)
{
    ...
    cv::Ptr<cv::MinProblemSolver::Function> ptr_F =
        cv::makePtr<Refine>().staticCast<cv::MinProblemSolver::Function>();
...
cv::Point2d PoseRefiner::projectPoint(cv::Point2d p, cv::Mat H)
{
...
bool PoseRefiner::checkIfPointInQuad(const Marker& marker, const cv::Point2d& p)
{
    // check if point p is in the fan of each corner
    cv::Point2d c1c2, c1c4, c3c2, c3c4, c1p, c3p;

    c1c2 = cv::Point2d(marker.corners[1].x - marker.corners[0].x,
                       marker.corners[1].y - marker.corners[0].y);
    c1c4 = cv::Point2d(marker.corners[3].x - marker.corners[0].x,
                       marker.corners[3].y - marker.corners[0].y);
    c3c2 = cv::Point2d(marker.corners[1].x - marker.corners[2].x,
                       marker.corners[1].y - marker.corners[2].y);
    c3c4 = cv::Point2d(marker.corners[3].x - marker.corners[2].x,
                       marker.corners[3].y - marker.corners[2].y);

    c1p = cv::Point2d(p.x - marker.corners[0].x, p.y - marker.corners[0].y);
    c3p = cv::Point2d(p.x - marker.corners[2].x, p.y - marker.corners[2].y);
