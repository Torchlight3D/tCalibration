                                 ...
                                 const Job_metadata& metadata)
{
    ...
    if (nwritten != buffer.size()) {
        LOG(ERROR) << std::format(
            "Could not write header to edge info serialization file, tried {} "
            "bytes, only wrote {}",
                                   ...
                                   Job_metadata& metadata)
{
    ...
    if (nread != buffer.size()) {
        LOG(INFO) << std::format(
            "Could not read header from edge info serialization file, tried {} "
            "bytes, only wrote {}",
            buffer.size(), nread);
        return false;
    }
    ...
    const std::vector<double>& in_geometric_length)
{
    ...
    for (size_t k = 0; k < in_centroid.size(); k++) {
        ...
        if (nwritten != size_in_bytes) {
            LOG(ERROR) << std::format(
                "Could not write to edge info serialization file, tried {} "
                "bytes, only wrote {}",
...
Edge_info Edge_info::deserialize(FILE* fin, bool& valid)
{
    ...
    if (nread != 10 * sizeof(double)) {
        LOG(ERROR) << std::format("Could not read from edge info serialization "
                                  "file, tried {} bytes, only read {}",
    ...
    if (nread != sizeof(uint32_t)) {
        LOG(ERROR) << "Could not read from edge info serialization file, SFR "
                      "size read failed";
        return b;
    }
    ...
    if (nsfr > 128) {
        LOG(ERROR) << std::format(
            "Unexpected SFR length ({}) in edge deserialization, aborting",
            nsfr);
        return b;
    }
    ...
    nread = fread(ibuf, 1, sizeof(uint32_t), fin);
    if (nread != sizeof(uint32_t)) {
        LOG(ERROR) << "Could not read from edge info serialization "
    ...
    if (nsfr > 256) {
        LOG(ERROR) << "Unexpected ESF length in edge deserialization, aborting";
        return b;
    }

    nread = fread(ibuf, 1, sizeof(float) * nesf, fin);
    if (nread != sizeof(float) * nesf) {
        LOG(ERROR) << "Could not read from edge info serialization file, ESF "
                      "read failed";
        return b;
    }
