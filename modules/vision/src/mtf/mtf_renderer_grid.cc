...
public:
    ...
    {
        return block.get_quality(edge) > very_poor_quality
                   ? block.get_mtf50_value(edge)
                   : nodata();
    }

    virtual bool in_range(double val) const { return val > 0 && val < 1.0; }
    ...
    virtual double nodata() const { return 2.0; }

    virtual double smoothing_factor() const
    {
        return sparse_mode ? 1e-1 : 1e-3;
    }

    virtual int pruning_threshold() const { return sparse_mode ? 2 : 1; }
...
Mtf_renderer_grid::Mtf_renderer_grid(
    ...
    const std::string& fname, const std::string& gnuplot_binary,
    const cv::Mat& img, int gnuplot_width, bool lpmm_mode, double pixel_size,
    double in_zscale, double surface_max, int mtf_contrast)
...
void Mtf_renderer_grid::set_gnuplot_warning(bool gnuplot)
{
...
void Mtf_renderer_grid::render(const std::vector<Block>& blocks)
{
    ...
    interpolate_grid(mtf_ftor, MERIDIONAL, grid_mer_coarse, grid_mer_fine,
                     img_dims, blocks, m_upper, mtf_ftor.smoothing_factor(),
                     mtf_ftor.pruning_threshold());
    interpolate_grid(mtf_ftor, SAGITTAL, grid_sag_coarse, grid_sag_fine,
                     img_dims, blocks, m_upper, mtf_ftor.smoothing_factor(),
                     mtf_ftor.pruning_threshold());
    ...
    for (int y = 0; y < grid_mer_coarse.rows; y++) {
        for (int x = 0; x < grid_mer_coarse.cols; x++) {
            ...
            zmax = std::max(
                zmax, (double)grid_mer_coarse.at<float>(y, x) * pixel_size);
            zmin = std::min(
                zmin, (double)grid_mer_coarse.at<float>(y, x) * pixel_size);
        }
        fprintf(file, "\n");
    }
    fprintf(file, "\n\n");
    ...
    for (int y = 0; y < grid_sag_coarse.rows; y++) {
        for (int x = 0; x < grid_sag_coarse.cols; x++) {
            ...
            zmax = std::max(
                zmax, (double)grid_sag_coarse.at<float>(y, x) * pixel_size);
            zmin = std::min(
                zmin, (double)grid_sag_coarse.at<float>(y, x) * pixel_size);
        }
        fprintf(file, "\n");
    }
    fprintf(file, "\n\n");
    ...
    if (surface_max > 0 &&
        surface_max > zmax) { // valid user override on maximum value specified
        zmax = surface_max;
    }
    ...
    double linewidth =
        std::max(double(0.5), double(0.5) * gnuplot_width / 1024.0);
    ...
    fprintf(gpf,
            ...
            width_in_pixels,
            (int)lrint(width_in_pixels * 2.3 * grid_mer_fine.rows /
                       double(grid_mer_fine.cols)),
#ifdef _WIN32
            "Verdana",
#else
            "Arial",
    ...
    fprintf(gpf,
            "set term pngcairo dashed transparent enhanced size %d, %d font "
            "'%s,%d'  background rgb \"white\"\n",
            (int)lrint(width_in_pixels * 2 * grid_mer_fine.rows /
                       double(grid_mer_fine.cols)),
            height_in_pixels_3d,
#ifdef _WIN32
            "Verdana",
...
#ifdef _WIN32
    sprintf(buffer, "\"\"%s\" \"%sgrid.gnuplot\"\"", gnuplot_binary.c_str(),
            wdir.c_str());
#else
    sprintf(buffer, "\"%s\" \"%sgrid.gnuplot\"", gnuplot_binary.c_str(),
            wdir.c_str());
    ...
    if (rval != 0) {
        ...
        logger.info(
            "You can try to execute [%s] to render the plots manually\n",
            buffer);
        gnuplot_failure = true;
    }
    else {
        logger.debug("%s\n", "Gnuplot plot completed successfully. Look for "
