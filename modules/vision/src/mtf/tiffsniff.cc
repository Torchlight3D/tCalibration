...
Tiffsniff::Tiffsniff(const std::string& fname, bool is_8bit)
{
    ...
    if (fin->good()) {
        ...
        try {
            ...
            logger.error("%s\n", "Error while parsing input image, unable to "
                                 "extract colourspace information.");
        }
    }

    if (inferred_profile == profile_t::UNKNOWN) {
        logger.debug("No ICC profile found, attempting to infer profile from "
                     "Exif information, Exif CS: %d\n",
                     exif_cs);
        if (exif_cs < 0) {
...
void Tiffsniff::read_icc_profile(off_t offset)
{
    ...
    if (fin->seekg(offset).good()) {
        ...
        logger.debug("Device: %c%c%c%c\n", icc_header[12], icc_header[13],
                     icc_header[14], icc_header[15]);
        logger.debug("Colour space: %c%c%c%c\n", icc_header[16], icc_header[17],
                     icc_header[18], icc_header[19]);
