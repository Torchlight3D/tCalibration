                          ...
                          const std::vector<std::pair<double, double>>& lut)
{
    if (cnr < lut.back().first) {
        if (cnr > lut.front().first) {
            ...
            if (i < lut.size() - 1) {
                return lut[i].second + (lut[i + 1].second - lut[i].second) *
                                           (cnr - lut[i].first) /
                                           (lut[i + 1].first - lut[i].first);
            }
                                          ...
                                          int width)
{
    ...
    double delta = sampled[left_trans] - (smoothed[left_trans + bhw] -
                                          smoothed[left_trans - bhw - 1]) /
                                             double(2 * bhw + 1);
    ...
    delta = sampled[right_trans] -
            (smoothed[right_trans + bhw] - smoothed[right_trans - bhw - 1]) /
                double(2 * bhw + 1);
    for (int idx = right_trans; idx < std::min(fft_right, fft_size - bhw - 1);
         idx++) {
        sampled[idx] = (smoothed[idx + bhw] - smoothed[idx - bhw - 1]) /
                       double(2 * bhw + 1);
    }
                         ...
                         double edge_value = 0.5)
{
    ...
    for (int x = -sgh; x <= sgh; x++) {
        w[x + sgh] =
            exp(-c * (double(x) / double(sgh)) * (double(x) / double(sgh)));
    }
...
int EsfModelLoss::build_esf(std::vector<Ordered_point>& ordered,
                            ...
                            bool allow_peak_shift)
{
    ...
    std::vector<std::pair<double, double>> ridge_lut = {
        {4, 95},  {5, 92},  {6, 89},     {7, 86},  {8, 83},  {9, 79},  {10, 76},
        {12, 70}, {14, 63}, {16, 58},    {18, 53}, {20, 49}, {22, 46}, {24, 43},
        {26, 42}, {28, 40}, {30, 39},    {35, 38}, {40, 38}, {45, 36}, {50, 34},
        {60, 30}, {70, 27}, {80, 22},    {90, 14}, {100, 7}, {124, 2}, {149, 2},
    ...
    for (int b = min_left_bin; b < max_right_bin; b++) {
        ...
        if (npts < (order + 1)) {
            ...
            if (fabs(mid) < 0.125 * twidth + model_switch_bias) {
                ...
                // blend in the box filter, otherwise f/16 LSFs lose some detail
                // right at the foot
    ...
    // the ESF is split into segments, with different types and levels of
    // smoothing applied to different segments
    const std::vector<std::pair<double, double>> extreme_tails_lut = {
        {4.012, 27},  {5.006, 26},  {6.001, 25},  {6.996, 25},  {7.990, 24},
        {8.985, 23},  {9.979, 22},  {11.968, 22}, {13.958, 21}, {15.947, 20},
        {17.937, 20}, {19.926, 19}, {21.915, 19}, {23.904, 19}, {25.894, 18},
        {27.884, 18}, {29.874, 17}, {34.846, 16}, {39.822, 16}, {44.795, 15},
        {49.770, 14}, {59.715, 12}, {69.671, 12}, {79.614, 11}, {89.571, 10},
        {99.506, 9},  {124.374, 8}, {149.222, 8}, {174.085, 7}, {198.973, 7},
        {298.480, 7}, {397.781, 6}, {497.074, 6}, {595.965, 5}, {695.846, 4},
        {794.242, 4}};
    ...
    for (int rep = 0; rep < 1; rep++) {
        local_moving_average_smoother(smoothed, sampled, fft_size, 0, fft_size,
                                      lt, rt, sw_width);
    }

    const std::vector<std::pair<double, double>> transition_lut = {
        {99.5, 8},  {124.4, 8}, {149.2, 7}, {174.1, 7}, {199.0, 6},
        {298.5, 5}, {397.8, 4}, {497.1, 3}, {596.0, 3}, {695.8, 3},
        {794.2, 2}, {893.9, 2}, {1000, 0},  {1100, 0}};
    ...
    const std::vector<std::pair<double, double>> core_lut = {
        {4.0, 7},   {5.0, 6},  {6.0, 6},   {7.0, 5},   {8.0, 5},   {9.0, 5},
        {10.0, 5},  {12.0, 4}, {14.0, 4},  {15.9, 4},  {17.9, 3},  {19.9, 3},
        {21.9, 3},  {23.9, 3}, {25.9, 3},  {27.9, 3},  {29.9, 3},  {34.8, 2},
        {39.8, 2},  {44.8, 2}, {49.8, 2},  {59.7, 2},  {69.7, 1},  {79.6, 1},
        {89.6, 1},  {99.5, 1}, {124.4, 1}, {149.2, 1}, {174.1, 1}, {199.0, 1},
        {298.5, 0}, {397.8, 0}};
