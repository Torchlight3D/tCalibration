                 ...
                 cv::Point2d& best_sol)
{
    ...
    LOG(INFO) << std::format("Imatest_crop: line fit after {} tries ({} "
                             "workable), score={}, got line {}*x + {}",
                             tries, workable, best_score, best_sol.x,
                             best_sol.y);
...
Imatest_cropper::Imatest_cropper(cv::Mat& in_img) : Cropper(in_img)
{
    ...
    LOG(INFO) << "Imatest_crop: Otsu threshold = " << threshold;
    ...
    LOG(INFO) << std::format(
        "Imatest_crop: proposed cuts between rows {} and {}", (int)top_extreme,
        (int)bot_extreme);
