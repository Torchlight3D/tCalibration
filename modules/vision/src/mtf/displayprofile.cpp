...
cv::Mat Display_profile::to_luminance(const cv::Mat& img)
{
    if (!is_linear) {
        ...
        if (img.elemSize1() == 1) {
            ...
            if (img.channels() == 3) {
                LOG(INFO) << std::format(
                    "8-bit nonlinear RGB, luma={}R + {}G + {}B",
                ...
                LOG(INFO) << "8-bit nonlinear grayscale";
            ...
            if (img.channels() == 3) {
                LOG(INFO) << std::format(
                    "16-bit nonlinear RGB, luma={}R + {}G + {}B",
                    luminance_weights[0], luminance_weights[1],
                    luminance_weights[2]);
                cv::Mat newmat = cv::Mat(img.rows, img.cols, CV_16UC1);
                uint16_t* dptr = (uint16_t*)newmat.data;
                ...
                LOG(INFO) << "16-bit nonlinear grayscale";
        ...
        if (img.elemSize1() == 1) {
            ...
            if (img.channels() == 3) {
                ...
                LOG(INFO) << "8-bit linear grayscale";
                uint8_t* sentinel = sptr + img.rows * img.cols;
                while (sptr < sentinel) {
            ...
            if (img.channels() == 3) {
                ...
                LOG(INFO) << "16-bit linear grayscale";
                return img;
            }
        }
    }
...
std::vector<cv::Mat> Display_profile::to_linear_rgb(const cv::Mat& img)
{
    ...
    if (!is_linear) {
        ...
        if (img.elemSize1() == 1) {
            uint8_t* sptr = (uint8_t*)img.data;
            if (img.channels() == 3) {
                LOG(INFO) << "8-bit nonlinear RGB -> 16-bit linear RGB";
                uint8_t* sentinel = sptr + 3 * img.rows * img.cols;
                while (sptr < sentinel) {
            ...
            if (img.channels() == 3) {
                LOG(INFO) << "16-bit nonlinear RGB -> 16-bit linear RGB";
                uint16_t* sptr = (uint16_t*)img.data;
                uint16_t* sentinel = sptr + 3 * img.rows * img.cols;
                while (sptr < sentinel) {
        ...
        if (img.elemSize1() == 1) {
            uint8_t* sptr = (uint8_t*)img.data;
            if (img.channels() == 3) {
                LOG(INFO) << "8-bit linear RGB -> 16-bit linear RGB";
                uint8_t* sentinel = sptr + 3 * img.rows * img.cols;
                while (sptr < sentinel) {
                ...
                LOG(INFO) << "8-bit linear grayscale -> 16-bit linear gray RGB";
                uint8_t* sentinel = sptr + img.rows * img.cols;
                while (sptr < sentinel) {
                    *dptr_b++ = uint16_t(*sptr << 8);
                    *dptr_g++ = uint16_t(*sptr << 8);
            ...
            if (img.channels() == 3) {
                LOG(INFO) << "16-bit linear RGB -> 16-bit linear RGB (separate "
                             "channels)";
                uint16_t* sptr = (uint16_t*)img.data;
                uint16_t* sentinel = sptr + 3 * img.rows * img.cols;
                while (sptr < sentinel) {
