...
Mtf_renderer_focus::Mtf_renderer_focus(
    ...
    const std::string& wdir, const std::string& prof_fname, const cv::Mat& img,
...
void Mtf_renderer_focus::render(const std::vector<Block>&)
{
    LOG(FATAL) << "This function should not be used. Aborting";
    return;
}

void Mtf_renderer_focus::render(const std::vector<Mtf_profile_sample>& samples,
                                Bayer::bayer_t bayer,
                                ...
                                cv::Rect* dimension_correction)
{
    ...
    if (samples.size() < 100) {
        draw.fail_with_message(wdir + prname,
                               std::string("Insufficient targets detected"));
        return;
    }
    ...
    for (size_t i = 0; i < samples.size(); i++) {
        if (samples[i].quality > very_poor_quality) {
            ...
            if (std::abs(wc.y) < 20 * psf && std::abs(wc.x) < 180 * psf) {
                ...
                double depth = 0;
                distance_scale.estimate_depth_img_coords(samples[i].p.x,
                                                         samples[i].p.y, depth);
                coord.x = depth;
            ...
            // merged.at<cv::Vec3b>(lrint(samples[i].p.y),
            // lrint(samples[i].p.x)) = cv::Vec3b(200, 200, 0);
        }
    }
    mean_y /= double(data.size());
    std::vector<cv::Point2d> img_cents;
    auto sliding(sliding_edges);
    for (size_t k = 0; k < sliding.size(); k += 2) {
        cv::Scalar edge_col(200, 200, 0);
        const std::pair<cv::Point2d, cv::Point2d>& ec = sliding[k];
        const std::pair<cv::Point2d, cv::Point2d>& en = sliding[k + 1];
        cv::Point2d cent = 0.5 * (en.first + ec.first + ec.second);
        // cv::line(merged, cent - transverse, cent + transverse, edge_col, 2,
        // cv::LINE_AA);
    ...
    if (distance_scale.fiducial_code_set >= 0 &&
        distance_scale.fiducial_code_set <= 4) {
        std::vector<std::vector<cv::Point2d>> central_block_centroids{
            /*A0 */ {{411.913, 983.093}, {411.844, 969.094}, {411.773, 954.867},
                     {411.702, 940.407}, {411.629, 925.707}, {411.555, 910.761},
                     {411.479, 895.564}, {411.403, 880.109}, {411.325, 864.389},
                     {411.246, 848.398}, {411.165, 832.128}, {411.083, 815.572},
                     {411, 798.722},     {410.914, 781.571}, {410.828, 764.11},
                     {410.74, 746.331},  {410.65, 728.225},  {410.559, 709.784},
                     {410.466, 690.996}, {410.371, 671.854}, {410.274, 652.346},
                     {410.176, 632.462}, {410.075, 612.192}, {409.973, 591.522},
                     {409.868, 570.443}, {409.762, 548.941}, {409.653, 527.004},
                     {409.542, 504.618}, {409.429, 481.77},  {409.313, 458.444},
                     {409.195, 434.626}, {409.075, 410.3},   {408.952, 385.45},
                     {408.826, 360.058}, {408.697, 334.107}, {408.566, 307.578},
                     {408.431, 280.451}, {408.294, 252.706}, {408.153, 224.321},
                     {408.009, 195.275}, {407.862, 165.543}, {407.711, 135.102},
                     {407.556, 103.925}, {407.398, 71.9857}},
            /*A1 */ {{290.59, 710.57},   {290.552, 699.549}, {290.513, 688.394},
                     {290.473, 677.104}, {290.433, 665.675}, {290.393, 654.105},
                     {290.352, 642.391}, {290.31, 630.532},  {290.268, 618.523},
                     {290.226, 606.362}, {290.182, 594.047}, {290.139, 581.573},
                     {290.094, 568.939}, {290.05, 556.142},  {290.004, 543.177},
                     {289.958, 530.041}, {289.912, 516.732}, {289.865, 503.246},
                     {289.817, 489.578}, {289.768, 475.727}, {289.719, 461.687},
                     {289.669, 447.454}, {289.619, 433.026}, {289.568, 418.398},
                     {289.516, 403.565}, {289.463, 388.524}, {289.41, 373.269},
                     {289.356, 357.797}, {289.3, 342.102},   {289.245, 326.18},
                     {289.188, 310.026}, {289.131, 293.635}, {289.073, 277.001},
                     {289.014, 260.119}, {288.954, 242.984}, {288.893, 225.589},
                     {288.831, 207.929}, {288.768, 189.998}, {288.704, 171.789},
                     {288.64, 153.296},  {288.574, 134.512}, {288.507, 115.43},
                     {288.439, 96.0432}, {288.37, 76.3441}},
            /*A2 */ {{205.279, 510.658}, {205.258, 502.215}, {205.237, 493.697},
                     {205.215, 485.102}, {205.194, 476.431}, {205.172, 467.681},
                     {205.15, 458.852},  {205.129, 449.943}, {205.106, 440.952},
                     {205.084, 431.879}, {205.061, 422.723}, {205.038, 413.481},
                     {205.015, 404.154}, {204.992, 394.739}, {204.968, 385.236},
                     {204.945, 375.643}, {204.921, 365.959}, {204.896, 356.183},
                     {204.872, 346.314}, {204.847, 336.349}, {204.822, 326.288},
                     {204.797, 316.13},  {204.772, 305.872}, {204.746, 295.514},
                     {204.72, 285.054},  {204.694, 274.49},  {204.668, 263.821},
                     {204.641, 253.045}, {204.614, 242.161}, {204.587, 231.167},
                     {204.56, 220.061},  {204.532, 208.842}, {204.504, 197.507},
                     {204.475, 186.056}, {204.447, 174.487}, {204.418, 162.796},
                     {204.389, 150.983}, {204.359, 139.046}, {204.329, 126.983},
                     {204.299, 114.791}, {204.268, 102.468}, {204.238, 90.0135},
                     {204.207, 77.4239}, {204.175, 64.6974}},
            /*A3 */ {{145.06, 365.679},  {145.049, 359.347}, {145.038, 352.975},
                     {145.026, 346.561}, {145.015, 340.106}, {145.004, 333.608},
                     {144.992, 327.068}, {144.981, 320.485}, {144.969, 313.859},
                     {144.958, 307.189}, {144.946, 300.474}, {144.934, 293.715},
                     {144.922, 286.911}, {144.91, 280.061},  {144.898, 273.165},
                     {144.886, 266.222}, {144.874, 259.233}, {144.861, 252.196},
                     {144.849, 245.111}, {144.836, 237.977}, {144.824, 230.795},
                     {144.811, 223.563}, {144.798, 216.282}, {144.786, 208.949},
                     {144.773, 201.566}, {144.76, 194.131},  {144.747, 186.644},
                     {144.733, 179.105}, {144.72, 171.512},  {144.707, 163.866},
                     {144.693, 156.165}, {144.68, 148.409},  {144.666, 140.598},
                     {144.652, 132.731}, {144.638, 124.806}, {144.624, 116.825},
                     {144.61, 108.786},  {144.596, 100.688}, {144.582, 92.5307},
                     {144.567, 84.3136}, {144.553, 76.0361}, {144.538, 67.6973},
                     {144.524, 59.2966}, {144.509, 50.8334}},
            /*A4 */ {{102.514, 260.994}, {102.508, 256.323}, {102.503, 251.63},
                     {102.497, 246.914}, {102.491, 242.177}, {102.485, 237.418},
                     {102.479, 232.636}, {102.473, 227.831}, {102.467, 223.004},
                     {102.461, 218.154}, {102.455, 213.281}, {102.449, 208.385},
                     {102.443, 203.465}, {102.437, 198.522}, {102.431, 193.555},
                     {102.424, 188.564}, {102.418, 183.549}, {102.412, 178.51},
                     {102.406, 173.447}, {102.4, 168.359},   {102.393, 163.246},
                     {102.387, 158.108}, {102.38, 152.945},  {102.374, 147.757},
                     {102.368, 142.544}, {102.361, 137.304}, {102.355, 132.039},
                     {102.348, 126.748}, {102.341, 121.43},  {102.335, 116.086},
                     {102.328, 110.716}, {102.322, 105.318}, {102.315, 99.8936},
                     {102.308, 94.4418}, {102.301, 88.9625}, {102.294, 83.4556},
                     {102.288, 77.9208}, {102.281, 72.3579}, {102.274, 66.7667},
                     {102.267, 61.1469}, {102.26, 55.4984},  {102.253, 49.821},
                     {102.246, 44.1143}, {102.239, 38.3783}}};
        std::vector<cv::Point2d>& block_cents =
            central_block_centroids[distance_scale.fiducial_code_set];
        for (size_t k = 0; k < block_cents.size(); k++) {
            cv::Scalar edge_col(20, 20, 200);
            double world_y =
                block_cents[k].x -
                distance_scale.page_scale_factor * 420 / sqrt(2.0) * 0.5;
            double world_x =
                block_cents[k].y - distance_scale.page_scale_factor * 420 * 0.5;
            cv::Point2d cent = distance_scale.world_to_image(world_x, world_y);

            double min_dist = 1e50;
            for (auto& ic : img_cents) {
                min_dist = std::min(cv::norm(ic - cent), min_dist);
            }
            const double cross_w =
                0.5 * min_ic_dist - std::max(0.1 * min_ic_dist, 2.0);
            const double cross_l = min_ic_dist;
            if (min_dist > 1.01 * min_ic_dist) {
                cv::line(merged,
                         cent - cross_l * transverse - cross_w * longitudinal,
                         cent + cross_l * transverse + cross_w * longitudinal,
                         edge_col, 2, cv::LINE_AA);
                cv::line(merged,
                         cent - cross_l * transverse + cross_w * longitudinal,
                         cent + cross_l * transverse - cross_w * longitudinal,
                         edge_col, 2, cv::LINE_AA);
        ...
        LOG(ERROR) << "mtf_renderer_focus: encountered unknown fiducial code "
    ...
    LOG(INFO) << std::format("dropped {} samples, {} remain",
    ...
    errsum /= wsum;
    LOG(INFO) << "final model fit error (weighted): " << errsum;

    // do a quick bootstrap to estimate some bounds on the focus peak
    std::vector<double> mc_peaks;
    const int n_mc = 30;
    for (int i = 0; i < n_mc; i++) {
        std::vector<Sample> mc_data;
    ...
    std::string rp_name = std::format("{}{}_profile_points.txt", wdir,
                                      magic_enum::enum_name(bayer));
    ...
    double peak_y = 0;
    std::string cp_name = std::format("{}{}_profile_curve.txt", wdir,
                                      magic_enum::enum_name(bayer));
    ...
    if (ellipses) {
        ...
        for (const auto& e : *ellipses) {
            ...
            for (size_t li = 0; li < distance_scale.fid_img_points.size();
                 li++) {
                if (cv::norm(distance_scale.fid_img_points[li] -
                             cv::Point2d(e.centroid_x, e.centroid_y)) < 0.001) {
                    fid_idx = li;
                }
            }
            ...
            if (fid_idx >= 0) {
                ...
                cv::Point2d backproj_pt =
                    distance_scale.world_to_image(p3d.x, p3d.y, p3d.z);
                fid_offset =
            ...
            for (double theta = 0; theta < 2 * M_PI; theta += M_PI / 64.0) {
                ...
                double rot_x = cos(e.angle) * synth_x - sin(e.angle) * synth_y +
                               e.centroid_x;
                double rot_y = sin(e.angle) * synth_x + cos(e.angle) * synth_y +
                               e.centroid_y;
                ...
                cv::Point2d current(rot_x, rot_y);
                if (theta > 0) {
                    cv::line(merged, prev, current, ellipse_col, 1,
                             cv::LINE_AA);

                    if (fid_idx >= 0) {
                        cv::line(merged, prev + fid_offset,
                                 current + fid_offset, vec_col, 1, cv::LINE_AA);
                    }
                }
    ...
    for (double x = min_pix_long; x < max_pix_long; x += 1) {
        ...
        if (depth > min_long && depth < max_long) {
            ...
            cv::Point2d wc = distance_scale.estimate_world_coords(px, py);
            ...
            cv::Point2d proj_p = distance_scale.world_to_image(wc.x, world_y);
    ...
    for (double wy = -130 * psf; wy < 130 * psf; wy += 1) {
        cv::Point2d proj_p = distance_scale.world_to_image(peak_wx, wy);
        curve.push_back(proj_p);
    }
    ...
    for (; zmax > -200; zmax -= 2) {
        ...
        if (p.x < border || p.y < border || p.x > img.cols - 1 - border ||
            p.y > initial_rows - 1 - border)
            break;
    }
    ...
    if (distance_scale.bundle_rmse < 1.5) {
        ...
        draw.checkmark(cv::Point2d(25, initial_rows + ts.height * 3 * 1.75),
                       rmse_col);
    }
    else {
        draw.crossmark(cv::Point2d(35, initial_rows + ts.height * 2.75 * 1.75),
                       red);
    }
    ...
    if (std::abs(distance_scale.get_normal_angle_z() - 45) < 10) {
        ...
        draw.crossmark(cv::Point2d(35, rpy - 0.25 * 1.75 * ts.height), red);
    }
    ...
    exposure_checks(cv::Point2d(180 * psf, 135 * psf), white_clip, black_clip,
                    overexposure);
    ...
    if (white_clip < 10) {
        draw.checkmark(cv::Point2d(rpx - 25, rpy), wc_col);
    }
    else {
    ...
    if (black_clip < 10) {
        draw.checkmark(cv::Point2d(rpx - 25, rpy), bc_col);
    }
    else {
        draw.crossmark(cv::Point2d(rpx - 15, rpy - 0.25 * 1.75 * ts.height),
                       red);
...
Eigen::VectorXd Mtf_renderer_focus::rpfit(Ratpoly_fit& cf, bool scale,
                                          bool refine)
{
    ...
    while (!done) {
        ...
        for (int iter = 0; iter < 1; iter++) {
            ...
            sol = cov.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV)
                      .solve(b);
        }
        ...
        if (cf.has_poles(sol)) {
            cf.order_m--;
            if (!cf.silent) {
                LOG(INFO) << std::format("reduced order: ({}, {})", cf.order_n,
                                         cf.order_m);
            }
            done = false;
        }
...
void Mtf_renderer_focus::exposure_checks(const cv::Point2d& dims,
                                         double& white_clip, double& black_clip,
                                         double& overexposure)
{
    std::vector<cv::Point2d> corners{{-dims.x, -dims.y},
                                     {dims.x, -dims.y},
                                     {dims.x, dims.y},
                                     {-dims.x, dims.y}};
    std::vector<cv::Point> roi;
    for (const auto& p : corners) {
        cv::Point2d v = distance_scale.world_to_image(p.x, p.y);
    ...
    bounds.width = std::min((img.cols - 1 - bounds.x), bounds.width);
    bounds.height = std::min((img.rows - 1 - bounds.y), bounds.height);

    // cv::Mat masked;
    // img.copyTo(masked, mask);

    // calculate histogram
    std::vector<uint32_t> histo(65536 + 1, 0);
    ...
    std::vector<double> chisto(histo.size(), 0.0);
    chisto[0] = histo[0];
    ...
    if (chisto.back() - chisto[last_nz - 1] > 0.05 * chisto.back()) {
        white_clip =
            (chisto[last_nz] - chisto[last_nz - 1]) / chisto.back() * 100;
    }

    // catch a histogram with a bin within 5% of end with an abnormally large
    // count
    int wc_sentinel = std::max(2.0, last_nz * 0.95);
    int wc_peak = last_nz;
    while (wc_peak > wc_sentinel) {
        if (chisto[wc_peak] - chisto[wc_peak - 1] > 0.05 * chisto.back()) {
            white_clip =
                (chisto[wc_peak] - chisto[wc_peak - 1]) / chisto.back() * 100;
        }
        wc_peak--;
    }
    ...
    if ((chisto.back() - chisto[65535 * 0.9]) > 0.7 * histo.back()) {
        overexposure =
            (chisto.back() - chisto[65535 * 0.9]) / (0.5 * chisto.back()) * 100;
        overexposure = std::min(overexposure, 100.0);
    }

    int first_nz = 0;
    while (first_nz < (int)histo.size() - 1 && histo[first_nz] == 0) first_nz++;

    std::vector<double> rhisto(histo.size(), 0.0);
    ...
    int p5 = first_nz + 1;
    while (p5 < (int)histo.size() - 1 && rhisto[p5] > 0.05 * rhisto.front())
        p5++;

    black_clip = 0;
    if (rhisto[first_nz] - rhisto[first_nz + 1] > 0.0001 * rhisto.front()) {
        black_clip = (rhisto[first_nz] - rhisto[first_nz + 1]) /
                     (0.05 * rhisto.front()) * 100;
        black_clip = std::min(black_clip, 100.0);
    }
