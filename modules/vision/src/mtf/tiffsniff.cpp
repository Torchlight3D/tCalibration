...
Tiffsniff::Tiffsniff(const std::string& fname, bool is_8bit)
{
    fin = std::shared_ptr<std::iostream>(
        new std::fstream(fname, std::ios_base::binary | std::ios_base::in));
    if (fin->good()) {
        ...
        try {
            ...
            if (magic[0] == 0xff && magic[1] == 0xd8) { // possible JPEG file
                // we could check the end of the file for 0xff 0xf9, but Sony
                // pads the back of the file with zeros, so we'll just wing it
                has_profile = true;
                ...
                if (blocks.size() > 0) {
                    ...
                    if (!icc_found) { // no ICC block, settle for EXIF
                        for (size_t i = 0; i < blocks.size() && !exif_found;
                             i++) {
            ...
            LOG(ERROR) << "Error while parsing input image, unable to extract "
                          "colourspace information.";
        }
    }

    if (inferred_profile == profile_t::UNKNOWN) {
        LOG(INFO) << "No ICC profile found, attempting to infer profile from "
                     "Exif information, Exif CS: "
                  << exif_cs;
        if (exif_cs < 0) {
            // for 16-bit images, force linear colour space if no Exif Colour
            // Space tag was found for 8-bit images, assume sRGB
...
uint32_t Tiffsniff::read_uint32()
{
    if (big_endian) {
...
uint16_t Tiffsniff::read_uint16()
{
    if (big_endian) {
...
Display_profile Tiffsniff::profile()
{
    if (gtable.empty()) {
        return {gparm, luminance_weights};
    }

    return {gtable, luminance_weights};
}
    ...
    if (fin->seekg(offset).good()) {
        ...
        if (fin->read((char*)magic, 4).good()) {
            ...
            if (memcmp(le_id, magic, 4) == 0) {
                if (offset == 0) {
                    LOG(INFO) << "Little endian TIFF detected.";
                }
                else {
                    LOG(INFO) << "EXIF / Little endian TIFF detected.";
                }
                is_valid_tiff = true;
            }
            if (memcmp(be_id, magic, 4) == 0) {
                if (offset == 0) {
                    LOG(INFO) << "Big endian TIFF detected.";
                }
                else {
                    LOG(INFO) << "EXIF / Big endian TIFF detected.";
                }
                is_valid_tiff = true;
                big_endian = true;
            }
...
void Tiffsniff::read_ifd(off_t offset, off_t base_offset, ifd_t ifd_type)
{
    if (fin->seekg(offset).good()) {
        ...
        // check for weird values; realistic tiff files will not have thousands
        // of entries
        ...
        for (uint16_t i = 0; i < cur_ifd_entries; i++) {
            ...
            // ICC profile
            if (ifd_type == ifd_t::TIFF && field.tag_id == 0x8773) {
                read_icc_profile(field.data_offset);
            }

            // EXIF field
            if (ifd_type == ifd_t::TIFF && field.tag_id == 0x8769) {
                read_ifd(base_offset + field.data_offset, base_offset,
                         ifd_t::EXIF);
            }

            // EXIF colourspace tag
            if (ifd_type == ifd_t::EXIF && field.tag_id == 0xa001) {
            ...
            // EXIF gamma tag
            if (ifd_type == ifd_t::EXIF && field.tag_id == 0xa500) {
            ...
            if (ifd_type == ifd_t::EXIF && field.tag_id == 0xa005) {
                read_ifd(base_offset + field.data_offset, base_offset,
                         ifd_t::EXIF_INTEROP);
            }

            // EXIF interoperability tag
            if (ifd_type == ifd_t::EXIF_INTEROP && field.tag_id == 0x0001) {
                char ids[4];
                if (fin->seekg(-4, fin->cur).good() &&
                    fin->read((char*)ids, 4).good()) {
...
void Tiffsniff::read_icc_profile(off_t offset)
{
    ...
    if (fin->seekg(offset).good()) {
        ...
        LOG(INFO) << std::format("Device: %c%c%c%c", icc_header[12],
                                 icc_header[13], icc_header[14],
                                 icc_header[15]);
        LOG(INFO) << std::format("Colour space: %c%c%c%c", icc_header[16],
                                 icc_header[17], icc_header[18],
                                 icc_header[19]);
        ...
        for (uint32_t i = 0; i < icc_entries; i++) {
            ...
            // Just grab the first TRC get find, since they are probably all the
            // same anyway
            if (!found_trc && (tag.tag_signature == 0x67545243 ||
                               tag.tag_signature == 0x6B545243 ||
                               tag.tag_signature == 0x62545243 ||
                               tag.tag_signature == 0x72545243)) {
            ...
            if (tag.tag_signature == 0x77747074) { // media white point
                ...
                if (std::abs(wp[1] - 1.0) <
                    ...
                    LOG(INFO) << std::format("white point is %lg, %lg, %lg",
                                             wp[0], wp[1], wp[2]);
                }
            }
        ...
        if (!found_trc) {
            LOG(ERROR) << "Warning: image contains ICC profile, but no TRC "
                          "curve was found.";
            inferred_profile = profile_t::sRGB;
        }
        else {
    ...
    if (fin->seekg(offset + 8).good()) {
        ...
        switch (ftype) {
            ...
            default: // treat this as linear
                LOG(ERROR) << "unknown ICC parametricCurveType " << ftype;
                throw -1;
                break;
        }
    }
...
std::vector<double> Tiffsniff::read_xyztype_entry(off_t offset, uint32_t)
{
    if (fin->seekg(offset).good()) {
        ...
        if (sig != 0x58595A20) {
            LOG(INFO) << std::format(
                "Expected XYZ signature, found {} instead.", sig);
            throw -1;
        }
...
std::vector<std::pair<jpeg_app_t, off_t>> Tiffsniff::scan_jpeg_app_blocks()
{
    std::vector<std::pair<jpeg_app_t, off_t>> blocks;
    fin->seekg(2);
    bool done = false;

    while (fin->good() && !done) {
        uint16_t app_id = icc_tag::read_uint16(fin);
        uint16_t bsize = icc_tag::read_uint16(
            fin); // this size excludes the size bytes themselves
        ...
        if (!done) {
            ...
            if (strncasecmp((char*)sig, "ICC_PROFILE", 11) == 0) {
                blocks.push_back(std::make_pair(
                    jpeg_app_t::ICC,
                    off_t(fin->tellg()) + 2)); // skip over chunk numbers
            }
            if (strncasecmp((char*)sig, "EXIF", 4) == 0) {
                int i;
                for (i = 0; i < 4 && fin->get() == 0; i++);
                blocks.push_back(
                    std::make_pair(jpeg_app_t::EXIF, off_t(fin->tellg()) - 1));
            }
    ...
    if (fin->seekg(offset).good()) {
        ...
        do {
            ...
            if (!fin->good() || !fin->read((char*)magic, 4).good()) {
                // not necessarily an error, maybe we already grabbed the
                // profile ...
                return;
            }
            ...
            if (memcmp(magic, "sRGB", 4) == 0 &&
                !has_icc_profile) { // not tested yet
                int psrgb = fin->get();
                if (psrgb >= 0 && psrgb <= 3) {
                    inferred_profile = profile_t::sRGB;
                    has_profile = true;
            ...
            if (memcmp(magic, "iCCP", 4) == 0) {
                ...
                if (fin->get() != 0)
                    throw -1; // PNG compression method not deflate!

                size_t c_size = chunk_size - (name_len + 1);

                std::vector<unsigned char> c_chunk(chunk_size, 0);
                std::vector<unsigned char> u_chunk(chunk_size * 10, 0);
                if (!fin->read((char*)c_chunk.data(), c_size).good())
                    throw -1;
                ...
                std::shared_ptr<std::iostream> fout(new std::stringstream());
                if (fout->good()) {
                    fout->write((char*)u_chunk.data(), u_size);

                    std::shared_ptr<std::iostream> img_file = fin;
                    fin = fout;
                    read_icc_profile(0);
...
#else
            LOG(INFO) << "No PNG/ICC support because zlib not found.";
#endif
            fin->seekg(chunk_size + 4, fin->cur);
