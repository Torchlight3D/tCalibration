    ...
    for (size_t i = 0; i < blocks.size(); i++) {
        ...
        for (size_t k = 0; k < 4; k++) {
            ...
            if (!blocks[i].get_edge_valid(k))
                continue;
            if (std::abs(blocks[i].get_ca(k).x - Edge_info::nodata) < 1e-6 ||
                std::abs(blocks[i].get_ca(k).y - Edge_info::nodata) < 1e-6)
                continue;

            if (std::abs(delta) >= std::cos(math::degToRad(45.)) ||
                blocks.size() == 1 || allow_all_edges) {
