...
int Ellipse_detector::fit(const Component_labeller &cl,
                          const Gradient &gradient, const Pointlist &raw_points,
                          int tl_x, int tl_y, int dilate)
{
    ...
    for (size_t i = 0; i < raw_points.size(); i++) {
        ...
        if (raw_points[i].x <= border ||
            raw_points[i].x >= width - 1 - border ||
            raw_points[i].y <= border ||
            raw_points[i].y >= height - 1 - border) {
            edge_touched = true;
        }

        circ += sqrt(SQR(raw_points[i].x - prev_point.x) +
                     SQR(raw_points[i].y - prev_point.y));
    ...
    if (edge_touched)
        return 0; // skip objects that touch the edge of the image (ellipses not
    ...
    for (auto it = boundary.begin(); it != boundary.end(); it++) {
        ...
        if (gradient.grad_magnitude(x_pos, y_pos) > 1e-7) {
            ...
            l[2] = -(grad_x.at<float>(y_pos, x_pos) * (x_pos - mx) * iso_scale +
                     grad_y.at<float>(y_pos, x_pos) * (y_pos - my) * iso_scale);
    ...
    cov2(1, 0) = cov(4, 3);
    cov2(1, 1) = cov(4, 4);
    Eigen::JacobiSVD<Matrix2d> svd{cov2,
    ...
    if (result == 0 && major_axis >= min_major_axis &&
        minor_axis >= min_minor_axis && gradient_ok) {
        is_circle = 1;
    }

    if (std::isnan(centroid_x) || std::isnan(centroid_y) ||
        std::isnan(major_axis) || std::isnan(minor_axis)) {
        is_circle = 0;
    }

    LOG(INFO) << std::format(
        "centre ({}, {}), major = {}, minor = {}, angle = {}, is_circle = {}",
        centroid_x, centroid_y, major_axis, minor_axis, angle / M_PI * 180.0,
        is_circle);
    ...
    if (is_circle) {
        ...
        LOG(INFO) << "label used for scanset: " << clabel;
        int total = 0;
        int foreground = 0;
...
void Ellipse_detector::_dilate(std::set<iPoint> &s, int width, int height,
                               int iters)
{
    const int border = 1;

    if (iters > 0) {
        std::set<iPoint> gen_s;
...
void Ellipse_detector::_dilate_outer_only(std::set<iPoint> &s, int width,
                                          int height)
{
    const int border = 1;
    ...
    for (auto it = s.begin(); it != s.end(); it++) {
        ...
        cv::Point2d dir(it->first - cx,
                        it->second - cy); // current radial direction
        ...
        if ((right - it->first) * dir.x >= 0)
            gen_s.insert(iPoint(right, it->second));
        if ((right - it->first) * dir.x >= 0 &&
            (bottom - it->second) * dir.y >= 0)
            gen_s.insert(iPoint(right, bottom));
        if ((bottom - it->second) * dir.y >= 0)
            gen_s.insert(iPoint(it->first, bottom));
        if ((left - it->first) * dir.x >= 0 &&
            (bottom - it->second) * dir.y >= 0)
            gen_s.insert(iPoint(left, bottom));
        if ((left - it->first) * dir.x >= 0)
            gen_s.insert(iPoint(left, it->second));
        if ((left - it->first) * dir.x >= 0 && (top - it->second) * dir.y >= 0)
            gen_s.insert(iPoint(left, top));
        if ((top - it->second) * dir.y >= 0)
            gen_s.insert(iPoint(it->first, top));
        if ((right - it->first) * dir.x >= 0 && (top - it->second) * dir.y >= 0)
            gen_s.insert(iPoint(right, top));
    }
    s = gen_s; // copy it back
}
...
bool Ellipse_detector::gradient_check(
    [[maybe_unused]] const Component_labeller &cl, const Gradient &gradient,
    const Pointlist &raw_points)
{
    // gradient just on ellipse perimeter must be perpendicular to ellipse
    // tangent
    ...
    for (size_t i = 0; i < raw_points.size(); i++) {
        ...
        cv::Point2d d(raw_points[i].x - centroid_x,
                      raw_points[i].y - centroid_y);
        double rx = cosa * d.x - sina * d.y;
        double ry = sina * d.x + cosa * d.y;
        double theta = atan2(ry, rx); // ellipse curve parameter theta
        cv::Point2d tangent = normalize(
            cv::Point2d(-major_axis * sin(theta), minor_axis * cos(theta)));
        ...
        cv::Point2d grad =
            normalize(cv::Point2d(gradient.grad_x().at<float>(py, px),
                                  gradient.grad_y().at<float>(py, px)));
