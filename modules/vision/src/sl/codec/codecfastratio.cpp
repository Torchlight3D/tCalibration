...
// Decoder
DecoderFastRatio::DecoderFastRatio(unsigned int _screenCols,
                                   unsigned int _screenRows, CodecDir _dir)
...
void DecoderFastRatio::decodeFrames(cv::Mat &up, cv::Mat &vp, cv::Mat &mask,
                                    cv::Mat &shading) const
{
    ...
    //    mask.setTo(true);
    mask =
        (shading > 10000) & (shading < 65000) & (up <= screenCols) & (up >= 0);
