    ...
    for (int r = 1; r < phase.rows - 1; r++) {
        for (int c = 1; c < phase.cols - 1; c++) {
            ...
            if (mask.at<bool>(r, c) == true) {
                ...
                quality.at<float>(r, c) =
                    std::max(std::max(std::max(up, down), left), right);
            }
        }
    }
...
std::vector<float> computethresholds(cv::Mat quality, const cv::Mat mask)
{
    ...
    for (int r = 0; r < quality.rows; r++) {
        for (int c = 0; c < quality.cols; c++) {
            if (mask.at<bool>(r, c) == true)
                stdDev += (quality.at<float>(r, c) - meanValue) *
                          (quality.at<float>(r, c) - meanValue);
        }
    }
...
void unwrappatch(cv::Mat phase, cv::Mat quality, cv::Mat mask, cv::Mat phOffset,
                 float threshold, cv::Point ptStart, int borderX, int borderY,
                 int stepX, int stepY)
{
    // Prepare array for possible remainder pixels that has been left over for
    // further processing
    int numRem = std::fabs((float)ptStart.x - borderX) *
                 std::fabs((float)ptStart.y - borderY);
    ...
    for (int r = ptStart.y; r != borderY - 1; r += stepY) {
        for (int c = ptStart.x; c != borderX - 1; c += stepX) {
            ...
            if ((mask.at<bool>(r, c)) &&
                (quality.at<float>(r, c) <= threshold)) {
                ...
                if (quality.at<float>(r, c - stepX) == VAL_UNWRAPPED) {
                    phOffset.at<float>(r, c) =
                        ...
                        unwrappix(phase.at<float>(r, c),
                                  phase.at<float>(r, c - stepX));
                    ...
                    if (quality.at<float>(r - stepY, c) == VAL_UNWRAPPED) {
                        phOffset.at<float>(r, c) =
                            ...
                            unwrappix(phase.at<float>(r, c),
                                      phase.at<float>(r - stepY, c));
                        quality.at<float>(r, c) = VAL_UNWRAPPED;
                    }
                    else {
                        // If it wasn't able to unwrap from any start facing
                        // pixel, check border facing pixels and if there's
                        // valid one, add current pixel to the leftovers
                        if ((mask.at<bool>(r, c + stepX)) ||
                            (mask.at<bool>(r + stepY, c)))
            ...
            const std::vector<float> thresholds)
{
    ...
    cv::Mat centerRegion =
        searchMask(cv::Range(ptStart.y - 10, ptStart.y + 10),
    ...
    for (unsigned int i = 0; i < thresholds.size(); i++) {
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart, 0,
                    0, -1, -1);
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart, 0,
                    phase.rows + 1, -1, 1);
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart,
                    phase.cols + 1, 0, 1, -1);
        unwrappatch(phase, quality, mask, phOffset, thresholds[i], ptStart,
                    phase.cols + 1, phase.rows + 1, 1, 1);
    }
    ...
    for (int r = 0; r < phase.rows; r++) {
        for (int c = 0; c < phase.cols; c++) {
            // If the pixel has been unwrapped, actualize it's phase and also
