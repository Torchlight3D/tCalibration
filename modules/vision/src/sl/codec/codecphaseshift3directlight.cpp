    ...
    : Encoder(_screenCols, _screenRows, _dir)
{
    ...
    for (unsigned int i = 0; i < N; i++) {
        float phase = 2.0 * pi_f / float(N) * (1.0 - (float)i);
...
cv::Mat EncoderPhaseShift3DirectLight::getEncodingPattern(
...
// Decoder
DecoderPhaseShift3DirectLight::DecoderPhaseShift3DirectLight(
    unsigned int _screenCols, unsigned int _screenRows, CodecDir _dir)
...
void DecoderPhaseShift3DirectLight::decodeFrames(cv::Mat &up, cv::Mat &vp,
                                                 cv::Mat &mask,
                                                 cv::Mat &shading) const
{
    ...
    modulation = 3.0 * (I2 - I3).mul(I2 - I3) +
                 (2.0 * I1 - I2 - I3).mul(2.0 * I1 - I2 - I3);
