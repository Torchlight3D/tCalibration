 ...
 */
constexpr unsigned int binaryToGray(unsigned int num)
{
    return (num >> 1) ^ num;
}
 ...
 */
constexpr unsigned grayToBinary(unsigned num, unsigned numBits)
{
 ...
 * Source:
 * http://icfun.blogspot.com/2009/04/get-n-th-bit-value-of-any-integer.html
 */
constexpr int get_bit(int decimal, int N)
{
...
constexpr int powi(int num, unsigned int exponent)
{
...
// Encoder
EncoderGrayCode::EncoderGrayCode(unsigned int _screenCols,
...
// Decoder
DecoderGrayCode::DecoderGrayCode(unsigned int _screenCols,
                                 unsigned int _screenRows, CodecDir _dir)
                                   ...
                                   cv::Mat &shading) const
{
    ...
    std::vector<cv::Mat> framesBinary(frames.size());
    for (unsigned int i = 0; i < frames.size(); i++) {
        ...
        framesBinary[i] =
            cv::abs(shading - frames[i]) < cv::abs(frames[i] - minImage);
        cv::threshold(framesBinary[i], framesBinary[i], 1, 1,
                      cv::THRESH_BINARY);
    }

    // Encode every pixel column
    int NbitsHorz = std::ceil(std::log2((float)screenCols));

    // Number of vertical encoding patterns
    int NbitsVert = std::ceil(std::log2((float)screenRows));

    if (dir & CodecDirHorizontal) {
        std::vector<cv::Mat> framesHorz(framesBinary.begin() + 2,
                                        framesBinary.begin() + Nhorz + 2);
        ...
        for (int i = 0; i < up.rows; i++) {
            for (int j = 0; j < up.cols; j++) {
                ...
                for (unsigned int f = 0; f < framesHorz.size(); f++) {
                    // Gray decimal
                    enc += powi(2, NbitsHorz - f - 1) *
                           framesHorz[f].at<unsigned char>(i, j);
                }
                // Standard decimal
        ...
        for (int i = 0; i < vp.rows; i++) {
            for (int j = 0; j < vp.cols; j++) {
                ...
                for (unsigned int f = 0; f < framesVert.size(); f++) {
                    // Gray decimal
                    enc += powi(2, NbitsVert - f - 1) *
                           framesVert[f].at<unsigned char>(i, j);
                }
                // Standard decimal
