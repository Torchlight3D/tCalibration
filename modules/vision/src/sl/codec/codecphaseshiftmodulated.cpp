...
constexpr unsigned int nPhaseY = 16;
constexpr unsigned int Nx = 3;
constexpr unsigned int Ny = 3;
constexpr unsigned int Ncue = 3;
} // namespace
    ...
    : Encoder(_screenCols, _screenRows, _dir)
{
    ...
    for (uint x = 0; x < Nx; x++) {
        ...
        for (uint r = 0; r < screenRows; r++) {
            for (uint c = 0; c < screenCols; c++) {
                float phase = 2 * pi_f * c / pitch - phaseX;
    ...
    for (uint y = 0; y < Ny; y++) {
        ...
        for (uint r = 0; r < screenRows; r++) {
            for (uint c = 0; c < screenCols; c++) {
                float phase = 2 * pi_f * r / pitch - phaseY;
                                              ...
                                              cv::Mat &shading) const
{
    ...
    for (int x = Ny; x <= frames.size() - Ncue; x += Ny) {
        std::vector<cv::Mat> framesY(frames.begin() + (x - Ny),
                                     frames.begin() + x);
