...
Vocabulary::Vocabulary(std::istream &stream) { load(stream); }

Vocabulary::Vocabulary(const Vocabulary &voc) { *this = voc; }
...
void Vocabulary::getFeatures(
    const std::vector<std::vector<cv::Mat>> &featuresList,
    std::vector<cv::Mat> &_features) const
{
        ...
        while (goon) {
            ...
            if (first_time) {
                ...
                for (size_t c{0}; c < clusters.size(); ++c) {
                    ...
                    for (const auto &index : groups[c]) {
                        cluster_descriptors.push_back(descriptors[index]);
                    }

                    desc::meanValue(cluster_descriptors, clusters[c]);
                }
            ...
            for (auto fit = descriptors.begin(); fit != descriptors.end();
                 ++fit) //, ++d)
            {
                double best_dist = desc::distance((*fit), clusters[0]);
                unsigned int icluster = 0;

                for (unsigned int c = 1; c < clusters.size(); ++c) {
                    double dist = desc::distance((*fit), clusters[c]);
                    if (dist < best_dist) {
                        best_dist = dist;
                        icluster = c;
                    }
    ...
    if (current_level < m_L) {
        ...
        for (unsigned int i = 0; i < clusters.size(); ++i) {
            ...
            for (const auto &index : groups[i]) {
                child_features.push_back(descriptors[index]);
            }
                                      ...
                                      std::vector<cv::Mat> &clusters) const
{
    ...
    for (auto fit = pfeatures.begin(); fit != pfeatures.end(); ++fit, ++dit) {
        *dit = desc::distance((*fit), clusters.back());
    }
    ...
    while ((int)clusters.size() < m_k) {
        ...
        for (auto fit = pfeatures.begin(); fit != pfeatures.end();
             ++fit, ++dit) {
            if (*dit > 0) {
                double dist = desc::distance((*fit), clusters.back());
                if (dist < *dit)
                    *dit = dist;
            }
        }
                           ...
                           WordValue &weight, NodeId *nid, int levelsup) const
{
    ...
    do {
        ...
        // desc::distance(feature, m_nodes[final_id].descriptor);

        for (const auto &id : nodes) {
            double d = desc::distance(feature, m_nodes[id].descriptor);
            if (d < best_d) {
                best_d = d;
                final_id = id;
            }
    ...
    if (feature.type() == CV_8U) {
        do {
            ...
            for (const auto &id : nodes) {
                ...
                uint64_t dist =
                    desc::distance_8uc1(feature, m_nodes[id].descriptor);
                if (dist < best_d) {
                    best_d = dist;
                    final_id = id;
                    bestidx = idx;
        ...
        do {
            ...
            for (const auto &id : nodes) {
                ...
                //   std::cout<<idx<< " "<<id<<" "<<
                //   m_nodes[id].descriptor<<std::endl;
                uint64_t dist = desc::distance(feature, m_nodes[id].descriptor);
...
void Vocabulary::getWordsFromNode(NodeId nid, std::vector<WordId> &words) const
{
    ...
    if (m_nodes[nid].isLeaf()) {
        ...
        while (!parents.empty()) {
            ...
            for (const auto &childId : m_nodes[parentid].children) {
                const Node &child_node = m_nodes[childId];
...
void Vocabulary::save(cv::FileStorage &f, const std::string &name) const
{
    ...
    while (!parents.empty()) {
        ...
        for (const auto &childId : children) {
            const Node &child = m_nodes[childId];
            ...
            f << "descriptor" << desc::toString(child.descriptor);
            f << "}";
    ...
    noexcept(false)
{
    ...
    while (!parents.empty()) {
        ...
        for (auto pit : parent.children) {
            ...
            desc::toStream(child.descriptor, aux_stream);
            // add to parent list
            if (!child.isLeaf())
...
void Vocabulary::fromStream(std::istream &str) noexcept(false)
{
    ...
    if (compressed) {
        ...
        for (int i = 0; i < nChunks; i++) {
            ...
            size_t d =
                qlz_decompress(&input[0], &decompressed[0], &state_decompress);
    ...
    for (size_t i = 1; i < m_nodes.size(); ++i) {
        ...
        desc::fromStream(child.descriptor, *_used_str);
...
void Vocabulary::load(const cv::FileStorage &fs, const std::string &name)
{
    ...
    for (unsigned int i = 0; i < fn.size(); ++i) {
        ...
        desc::fromString(m_nodes[nid].descriptor, d);
    }
...
{
    return m_words.empty() ? -1 : m_words[0]->descriptor.cols;
}

int Vocabulary::getDescritorType() const
{
    return m_words.empty() ? -1 : m_words[0]->descriptor.type();
}
