...
Homography33::Homography33(const cv::Point2f &opticalCenter)
    : _cxy(opticalCenter), fA(), H(), valid(false)
{
    fA.setZero();
    H.setZero();
}
...
#ifdef STABLE_H
void Homography33::setCorrespondences(const cv::Point2f *srcPts, int srcCount,
                                      const cv::Point2f *dstPts, int dstCount)
{
    ...
    valid = false;
    _srcPts = {srcPts, srcPts + srcCount};
    _dstPts = {dstPts, dstPts + dstCount};
}
...
void Homography33::compute()
{
    ...
    std::transform(_srcPts.begin(), _srcPts.end(), std::back_inserter(sPts),
                   [](const auto &pt) {
                       return cv::Point2f{pt.x, pt.y};
                   });

    std::vector<cv::Point2f> dPts;
    std::transform(_dstPts.begin(), _dstPts.end(), std::back_inserter(dPts),
                   [&](const auto &pt) {
                       return cv::Point2f{pt.x - _cxy.x, pt.y - _cxy.y};
                   });
...
std::pair<float, float> Homography33::project(float worldx, float worldy)
{
    ...
    ixy.first = ixy.first / z + _cxy.x;
    ixy.second = ixy.second / z + _cxy.y;
    return ixy;
}
