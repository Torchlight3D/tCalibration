...
public:
    Upnp(bool use_minimal_template = false)
    ...
    //     corresponding ray origin and direction at the i-th entry in
    //     ray_origins and ray_directions.
    //   solution_rotations:  The computed and candidate quaternions or
    //   rotations. solution_translations:  The estimated and candidate
    //   translations. solution_costs:  The costs of the solutions.
    ...
    // Evaluates the Upnp cost function given a rotation and the cost
    // parameters. The evaluated cost is the following:
    //
    // J(R) = vec(R)' * quadratic_penatly_mat * vec(R) +
    //        2 * linear_penalty_vector' * vec(R) + gamma.
    //
    ...
    //
    // Params:
    //   parameters:  The cost parameters of the function (i.e., quadratic
    //   penalty
...
private:
    ...
    // Flag that controls which polynomial solver to use. If set true, the
    // minimal samples will use the SolveForRotationsFromMinimalSample(), which
    // is slow; and uses SolveForRotationsFromNonMinimalSample() when set true,
    // which is faster than the former.
