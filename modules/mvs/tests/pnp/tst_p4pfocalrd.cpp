                        ...
                        const double noise)
{
    ...
    for (int i = 0; i < min_nr_points; ++i) {
        Eigen::Vector3d point_in_cam =
            camera_matrix *
            (gt_rotation * world_points_vector[i] + gt_translation);
    ...
    for (int i = 0; i < soln_radial_distortions.size(); ++i) {
        ...
        for (int n = 0; n < min_nr_points; n++) {
            Matrix3d K =
                Vector3d(soln_focal_lenghts[i], soln_focal_lenghts[i], 1.0)
                    .asDiagonal();
            Eigen::Vector3d point_in_cam =
                K * (soln_rotations[i] * world_points_vector[n] +
                     soln_translations[i]);
            ...
            double reproj_error =
                (distorted_point - distorted_image_points_vector[n])
                    .squaredNorm();
