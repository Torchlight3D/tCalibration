...
AxisAlignedBox3D Point::bbox() const
{
    return AxisAlignedBox3D(_position_and_color.pos(),
...
AxisAlignedBox3D Segment::bbox() const
{
    AxisAlignedBox3D B(P1.pos());
    B.include(P2.pos());
...
Polygone::Polygone(const std::vector<Feedback3DColor>& fc) : _vertices(fc)
{
    initNormal();

    for (size_t i = 0; i < fc.size(); i++) _bbox.include(fc[i].pos());
}

AxisAlignedBox3D Polygone::bbox() const { return _bbox; }
