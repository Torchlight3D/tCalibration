...
constexpr double EPS_GOURAUD_THRESHOLD = 0.05;
constexpr char CREATOR[]{"VRender library - (c) Cyril Soler 2005"};
const char* GOURAUD_TRIANGLE_EPS[] = {
...
void EPSExporter::writeHeader(QTextStream& out) const
{
    ...
    for (int i = 0; GOURAUD_TRIANGLE_EPS[i]; i++)
...
void EPSExporter::spewPolygone(const Polygone* P, QTextStream& out)
{
    ...
    if (nvertices > 0) {
        ...
        if (smooth && !_blackAndWhite) {
            ...
            /* Flat shaded polygon and white polygons; all vertex colors the
             * same. */
...
void EPSExporter::spewSegment(const Segment* S, QTextStream& out)
{
    ...
    if ((!_blackAndWhite) && (dr != 0 || dg != 0 || db != 0)) {
        ...
        steps = int(
            0.5f + std::max(1.0, colormax * distance * EPS_SMOOTH_LINE_FACTOR));
