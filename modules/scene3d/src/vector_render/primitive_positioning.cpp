...
int PrimitivePositioning::computeRelativePosition(const Primitive *p1,
                                                  const Primitive *p2)
{
    AxisAlignedBox3D bb1(p1->bbox());
    AxisAlignedBox3D bb2(p2->bbox());
    ...
    if (p1->nbVertices() >= 3) {
        ...
        if (p2->nbVertices() == 2) // Case of a segment versus a polygon
            return computeRelativePosition(dynamic_cast<const Polygone *>(p1),
                                                  ...
                                                  const Segment *S)
{
    ...
    for (size_t i = 0; i < P->nbVertices(); ++i)
        if (intersectSegments_XY(Vector2(S->vertex(0)), Vector2(S->vertex(1)),
                                 Vector2(P->vertex(i)),
                                 Vector2(P->vertex(i + 1)), _EPS, t1, t2))
    ...
    for (const auto &intersection : intersections) {
        tmin = std::min(tmin, intersection);
        tmax = std::max(tmax, intersection);
    }
    ...
    for (const auto &intersection : intersections) {
        Vector3 v((1 - intersection) * S->vertex(0) +
                  intersection * S->vertex(1));
                                                  ...
                                                  const Polygone *P2)
{
        ...
        1) // There is some numerical error in gpc. Let's skip.
    {
        ...
        return res;
        // throw runtime_error("Intersection with more than 1 contour ! Non
        // convex polygons ?") ;
    }
    ...
    for (long i = 0;
        ...
        Vector3 v1(gpc_int.contour[0].vertex[i].x,
                   gpc_int.contour[0].vertex[i].y, -f1 / P1->normal().z());
        Vector3 v2(gpc_int.contour[0].vertex[i].x,
                   gpc_int.contour[0].vertex[i].y, -f2 / P2->normal().z());
...
void PrimitivePositioning::getsigns(const Primitive *P, const NVector3 &v,
                                    double C, std::vector<int> &signs,
                                    std::vector<double> &zvals, int &Smin,
                                    int &Smax, double I_EPS)
{
    if (!P) {
        throw std::runtime_error("Null primitive in getsigns !");
    }
...
void PrimitivePositioning::splitPrimitive(Primitive *P, const NVector3 &v,
                                          ...
                                          Primitive *&prim_lo)
{
    if (auto *p1 = dynamic_cast<Polygone *>(P))
        PrimitivePositioning::split(p1, v, c, prim_up, prim_lo);
    if (auto *p2 = dynamic_cast<Segment *>(P))
        PrimitivePositioning::split(p2, v, c, prim_up, prim_lo);
    if (auto *p3 = dynamic_cast<Point *>(P))
