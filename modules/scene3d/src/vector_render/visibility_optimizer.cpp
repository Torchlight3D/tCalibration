...
#ifdef A_FAIRE
void VisibilityOptimizer::optimize(std::vector<PtrPrimitive>& primitives,
                                   float& percentage_finished, string& message)
#else
void VisibilityOptimizer::optimize(std::vector<PrimitivePtr>& primitives,
    ...
    for (size_t pindex = primitives.size() - 1; long(pindex) >= 0;
        ...
        if (primitives[pindex]) {
            ...
            if (primitives[pindex]->nbVertices() > 1) {
                ...
                try {
                    PrimitivePtr p(primitives[pindex]);
                    ...
                    // 1 - creates a gpc_polygon corresponding to the current
                    // primitive

                    gpc_vertex_list* new_poly_verts = new gpc_vertex_list;
                    gpc_vertex_list* new_poly_reduced_verts =
                        new gpc_vertex_list;
                    ...
                    if (p->nbVertices() == 2) {
                        ...
                        new_poly_reduced_verts->vertex[0].x =
                            p->vertex(0).x() + du;
                        new_poly_reduced_verts->vertex[0].y =
                            p->vertex(0).y() + dv;
                        new_poly_reduced_verts->vertex[1].x =
                            p->vertex(1).x() + du;
                        new_poly_reduced_verts->vertex[1].y =
                            p->vertex(1).y() + dv;
                        new_poly_reduced_verts->vertex[2].x =
                            p->vertex(1).x() - du;
                        new_poly_reduced_verts->vertex[2].y =
                            p->vertex(1).y() - dv;
                        new_poly_reduced_verts->vertex[3].x =
                            p->vertex(0).x() - du;
                        new_poly_reduced_verts->vertex[3].y =
                            p->vertex(0).y() - dv;
                    }
                    else {
                        new_poly_verts->num_vertices = p->nbVertices();
                        new_poly_verts->vertex =
                    ...
                    gpc_add_contour(&new_poly_reduced, new_poly_reduced_verts,
                                    false);

                    // 2 - computes the difference between this polygon, and the
                    // union of the 	preceeding ones.

                    gpc_polygon_clip(GPC_DIFF, &new_poly_reduced,
                                     &cumulated_union, &difference);

                    // 3 - checks the difference. If void, the primitive is not
                    // visible: skip it 	and go to next primitive.
                    ...
                    // 4 - The primitive is visible. Let's add it to the
                    // cumulated union of 	primitives.
                    ...
                    for (unsigned long i = 0; i < cumulated_union.num_contours;
                         ++i) {
