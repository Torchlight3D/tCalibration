...
constexpr double EGALITY_EPS = 0.0001;
constexpr double LINE_EGALITY_EPS = 0.0001;
...
public:
    ...
    void recursFillPrimitiveArray(std::vector<PrimitivePtr> &) const;

private:
    BSPNode *_root;
    std::vector<Segment *> _segments; // these are for storing segments and
                                      // points when _root is null
    std::vector<Point *> _points;
};

void BSPSortMethod::sortPrimitives(std::vector<PrimitivePtr> &primitive_tab,
                                   VRenderParams &vparams)
{
    ...
    // Store segments and points for pass 2, because
    // polygons are deleted by the insertion and can
    // not be dynamic_casted anymore.
    std::vector<PrimitivePtr> segments_and_points;
    for (size_t i{0}; i < primitive_tab.size(); ++i, ++nbinserted) {
        if ((P = dynamic_cast<Polygone *>(primitive_tab[i]))) {
            tree.insert(P);
        }
        else {
    ...
    Segment *S;
    Point *p;
    for (size_t j{0}; j < segments_and_points.size(); ++j, ++nbinserted) {
        if ((S = dynamic_cast<Segment *>(segments_and_points[j]))) {
            tree.insert(S);
        }
        else if ((p = dynamic_cast<Point *>(segments_and_points[j]))) {
            tree.insert(p);
        }
...
private:
    ...
    std::vector<Segment *> seg_plus;
    std::vector<Segment *> seg_moins;

    std::vector<Point *> pts_plus;
    std::vector<Point *> pts_moins;
...
void BSPTree::recursFillPrimitiveArray(std::vector<PrimitivePtr> &tab) const
{
    if (_root) {
...
{
    const double Z = P->sommet3DColor(0).x() * a + P->sommet3DColor(0).y() * b +
                     P->sommet3DColor(0).z() * c - d;

    return (Z > EGALITY_EPS) ? 1 : -1;
}

void BSPNode::Classify(Segment *S, Segment *&moins_, Segment *&plus_)
{
    const double Z1 = S->sommet3DColor(0).x() * a +
                      S->sommet3DColor(0).y() * b +
                      S->sommet3DColor(0).z() * c - d;
    const double Z2 = S->sommet3DColor(1).x() * a +
                      S->sommet3DColor(1).y() * b +
...
void BSPNode::Classify(Polygone *P, Polygone *&moins_, Polygone *&plus_)
{
    ...
    std::vector<Feedback3DColor> Ps;
    std::vector<Feedback3DColor> Ms;
    ...
    if ((nZero > 2) || (nconsZero > 0)) {
        // Ils y a des imprecisions numeriques dues au fait que le poly estpres
        // du plan.
    ...
    for (int k = 1; k <= n; k++) {
        ...
        if (sign == prev_sign) {
            ...
            //  Il faut effectuer le calcul en utilisant les memes valeurs que
            //  pour le calcul des signes,
            // sinon on risque des incoherences dues aux imprecisions
            // numeriques.
            ...
            prev_sign = sign;
        } // prev_sign != 0 donc necessairement sign = 0. Le sommet tombe dans
          // le plan
        else {
...
void BSPNode::recursFillPrimitiveArray(
    std::vector<PrimitivePtr> &primitive_tab) const
{
    if (fils_plus) {
