                            ...
                            gpc_polygon *p, int type, gpc_op op)
{
    ...
    for (int k = 0; k < total_vertices; ++k) edge_table[k] = edge_node();
    ...
    for (size_t c = 0; c < p->num_contours; c++) {
        if (p->contour[c].num_vertices < 0) {
            ...
            for (i = 0; i < p->contour[c].num_vertices; i++)
                if (OPTIMAL(p->contour[c].vertex, i,
                            p->contour[c].num_vertices)) {
                    edge_table[num_vertices].vertex.x =
                        p->contour[c].vertex[i].x;
                    edge_table[num_vertices].vertex.y =
                        p->contour[c].vertex[i].y;
                ...
                if (FWD_MIN(edge_table, min, num_vertices)) {
                    ...
                    for (i = 0; i < num_edges; i++) {
                        ...
                        e[i].succ = ((num_edges > 1) && (i < (num_edges - 1)))
                                        ? &(e[i + 1])
                                        : nullptr;
                        e[i].pred = ((num_edges > 1) && (i > 0)) ? &(e[i - 1])
                                                                 : nullptr;
...
{
    if (!p)
        throw std::runtime_error("GPC: Something's wrong.");
...
{
    for (size_t c = 0; c < p->num_contours; c++) FREE(p->contour[c].vertex);
    FREE(p->hole);
    FREE(p->contour);
    p->num_contours = 0;
}
    ...
    while (scanbeam < sbt_entries) {
        ...
        if (local_min) {
            if (local_min->y == yb) {
                ...
                for (edge = local_min->first_bound; edge;
        ...
        if (scanbeam < sbt_entries) {
            ...
            for (intersect = it; intersect; intersect = intersect->next) {
                ...
                if (!e0->next)
                    throw std::runtime_error("GPC internal error.");
                if (!e1->next)
                    throw std::runtime_error("GPC internal error.");
...
{
    for (size_t s = 0; s < t->num_strips; s++) FREE(t->strip[s].vertex);
    FREE(t->strip);
    t->num_strips = 0;
}
