      ...
      mBoard(dashboard)
{
    ...
    QSettings settings(QApplication::applicationDirPath() + QDir::separator() +
                           QApplication::applicationName() + QString(".ini"),
                       QSettings::IniFormat);
...
BoardLayout::~BoardLayout() { clearLayout(); }
...
void BoardLayout::clearLayout()
{
    ...
    for (int i = 0; i < size; i++) {
        if (QCPLayoutElement *el = takeAt(0)) {
            if (auto boardElement = qobject_cast<BoardElement *>(el))
...
void BoardLayout::updateLayout()
{
    ...
    for (int i = 0; i < mElements.size(); ++i) {
        auto rect = mElementsGridRect.at(i);
        QRect elementRect(
            mMargins.left() + rect.x() * mColumnWidth + mColumnSpacing / 2,
            mMargins.top() + rect.y() * mRowHeight + mRowSpacing / 2,
...
int BoardLayout::elementCount() const { return mElements.size(); }
...
QCPLayoutElement *BoardLayout::elementAt(const QPointF &pos) const
{
    ...
    if (indexRow >= 0 && indexCol >= 0 && indexRow < mRowCount &&
...
QCPLayoutElement *BoardLayout::elementAt(int col, int row) const
{
    for (int i = 0; i < mElements.size(); ++i) {
        auto rect = mElementsGridRect.at(i);
        if ((rect.x() <= col) && ((rect.x() + rect.width() - 1) >= col) &&
            (rect.y() <= row) && ((rect.y() + rect.height() - 1) >= row))
...
QCPLayoutElement *BoardLayout::elementAt(int col, int row, int width,
                                         int height) const
{
...
int BoardLayout::elementIndex(QCPLayoutElement *element) const
{
    if (element) {
        ...
        //        logger()->debug() << Q_FUNC_INFO << "Element not in this
        //        layout";
    }
    else {
...
bool BoardLayout::take(QCPLayoutElement *element)
{
    if (element) {
        ...
        //        logger()->debug() << Q_FUNC_INFO
        //                          << "Element not in this layout, couldn't
        //                          take";
    }
    else {
...
double BoardLayout::selectTest(const QPointF &pos, bool onlySelectable,
                               QVariant *details) const
{
    ...
    if (indexRow >= 0 && indexCol >= 0 && indexRow < mRowCount &&
        indexCol < mColumnCount) {
        if (QCPLayoutElement *el = mElementsMap.at(indexCol).at(indexRow))
            if (el->realVisibility() &&
                el->selectTest(pos, onlySelectable) >= 0)
...
void BoardLayout::saveElementRectSettings(QCPLayoutElement *element,
                                          QSettings *settings)
{
...
//        int indexCol = int((event->posF().x() - mMargins.left()) /
//        mColumnWidth); int indexRow = int((event->posF().y() - mMargins.top())
//        / mRowHeight);
...
//                    QRectF elementRect = QRectF(mMargins.left() + indexCol *
//                    mColumnWidth + double(mColumnSpacing / 2.0),
//                                                mMargins.top() + indexRow *
//                                                mRowHeight +
//                                                double(mRowSpacing / 2.0),
...
//                if(selectTest(event->posF(), false) >= 0) {
//                    int indexElement = -1;
//                    if(QCPLayoutElement *el =
//                    mElementsMap.at(indexCol).at(indexRow)) {
//                        indexElement = mElements.indexOf(el);
//                    }

//                    if(indexElement >= 0) {
//                        auto gridRect = mElementsGridRect.at(indexElement);
//                        QRectF elementRect = QRectF(mMargins.left() +
//                        gridRect.x() * mColumnWidth + double(mColumnSpacing
//                        / 2.0),
//                                                    mMargins.top() +
//                                                    gridRect.y() * mRowHeight
//                                                    + double(mRowSpacing
//                                                    / 2.0), gridRect.width() *
//                                                    mColumnWidth -
//                                                    mColumnSpacing,
...
QPen BoardLayout::backgroundPen() const { return mBackgroundPen; }
...
void BoardLayout::draw(QCPPainter *painter) { drawBackground(painter); }
...
void BoardLayout::drawBackground(QCPPainter *painter)
{
    ...
    if (!mBackgroundPixmap.isNull()) {
        if (mBackgroundScaled) {
            ...
            painter->drawPixmap(mRect.topLeft() + QPointF(0, -1),
                                mScaledBackgroundPixmap,
                                QRectF(0, 0, mRect.width(), mRect.height()) &
...
bool BoardLayout::addElement(QCPLayoutElement *element, QPoint pos, bool replot)
{
...
bool BoardLayout::setElementGeometry(int index, int col, int row, int width,
                                     int height)
{
    if (index >= 0 && index < mElementsGridRect.size()) {
...
bool BoardLayout::addElement(QCPLayoutElement *element, bool replot, int col,
                             int row, int width, int height)
{
    if (element) {
        ...
        while ((adjustedWidth < width) &&
               (col + adjustedWidth < mColumnCount)) {
            adjustedPoint.setX(mMargins.left() +
                               (adjustedWidth + col + 0.5) * mColumnWidth);
        ...
        while ((adjustedHeight < height) &&
               (row + adjustedHeight < mRowCount)) {
            bool fit = true;
            for (int i = 0; i < width; i++) {
                adjustedPoint.setX(mMargins.left() +
                                   (col + i + 0.5) * mColumnWidth);
                adjustedPoint.setY(mMargins.top() +
                                   (adjustedHeight + row + 0.5) * mRowHeight);
                if (selectTest(adjustedPoint, false) >= 0) {
                    fit = false;
                    break;
                }
        ...
        if ((col < 0) || (col + width > mColumnCount)) {
            //            logger()->debug() << QString("Requested column/width
            //            (%1/%2) "
            //                                         "exceeds layout dimension
            //                                         (%3/%4)")
            //                                     .arg(col)
            //                                     .arg(width)
            //                                     .arg(mColumnCount)
        ...
        if ((row < 0) || (row + height > mRowCount)) {
            //            logger()->debug() << QString("Requested row/height
            //            (%1/%2) exceeds "
            //                                         "layout dimension
            //                                         (%3/%4)")
            //                                     .arg(row)
...
{
    return addElement(element, replot, rect.x(), rect.y(), rect.width(),
                      rect.height());
}
...
void BoardLayout::updateElementMap()
{
    ...
    for (int i = 0; i < mElements.size(); i++) {
        ...
        for (int j = gridRect.x(); j < gridRect.x() + gridRect.width(); j++)
            for (int k = gridRect.y(); k < gridRect.y() + gridRect.height();
                 k++)
                mElementsMap[j][k] = mElements.at(i);
    }
}

int BoardLayout::rowCount() const { return mRowCount; }
...
int BoardLayout::columnCount() const { return mColumnCount; }
...
int BoardLayout::columnSpacing() const { return mColumnSpacing; }
...
int BoardLayout::rowSpacing() const { return mRowSpacing; }
...
QSizeF BoardLayout::singleElementSize() const { return mSingleElementSize; }
...
void BoardLayout::updateSingleElementSize()
{
    ...
    mSingleElementSize.setWidth(mSingleElementColumnCount * mColumnWidth -
                                mColumnSpacing);
    mSingleElementSize.setHeight(mSingleElementRowCount * mRowHeight -
                                 mRowSpacing);
}

double BoardLayout::columnWidth() const { return mColumnWidth; }

double BoardLayout::rowHeight() const { return mRowHeight; }
...
void BoardLayout::setBackground(const QPixmap &pm, bool scaled,
