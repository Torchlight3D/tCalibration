      ...
      mBoard(dashboard)
{
    ...
    setMinimumMargins({1, 1, 1, 1});
    setMargins({1, 1, 1, 1});
    setLayer(QLatin1String("main"));
    setSizeConstraintRect(scrOuterRect);
}

int AdaptiveTextElement::maxPointSize() const { return mMaxPointSize; }
...
int AdaptiveTextElement::minPointSize() const { return mMinPointSize; }

int AdaptiveTextElement::pointSize() const { return mFont.pointSize(); }
    ...
    if (pointSize != mFont.pointSize()) {
        ...
        if (mCachedPixmap) {
            if (mRect.size().width() > 0 && mRect.size().height() > 0) {
                mTextPixmap = QPixmap(mRect.size());
                ...
                if (cachePainter.isActive()) {
                    ...
                    cachePainter.drawText(0, 0, int(mRect.width()),
                                          int(mRect.height()), textFlags(),
                                          mTextDisplayed);
...
QBrush AdaptiveTextElement::backgroundBrush() const { return mBackgroundBrush; }
...
{
    if (phase == upLayout) {
        if (mNeedUpdate || (mPreviousText != mText) ||
            (mPreviousSize != mRect.size()) || !mCachedPixmapRendered) {
            ...
            if (!mText.isEmpty()) {
                ...
                switch (mAdjustStrategy) {
                    case asAdjustPointSize: {
                        ...
                        int adjustPoint = -1;
                        std::function<bool(int, int)> compareIntFunc =
                            std::less_equal<int>();
                        std::function<bool(bool, bool)> compareRectFunc =
                            andFunc;

                        QFontMetrics fontMetrics(font);
                        QRect boundingRect = fontMetrics.boundingRect(
                            int(mRect.x()), int(mRect.y()), int(mRect.width()),
                            int(mRect.height()), textFlags(), mText);

                        if (compareRectFunc(
                                compareIntFunc(boundingRect.height(),
                                               int(mRect.height())),
                                compareIntFunc(boundingRect.width(),
                                               int(mRect.width())))) {
                            adjustPoint = 1;
                        ...
                        for (;;) {
                            ...
                            boundingRect = fm.boundingRect(
                                int(mRect.x()), int(mRect.y()),
                                int(mRect.width()), int(mRect.height()),
                                textFlags(), mText);
                    ...
                    case asAdjustAndElide: {
                        ...
                        if (visible()) {
                            ...
                            if (boundingRect.width() > mRect.width()) {
                                mTextDisplayed = fontMetrics.elidedText(
                                    mText, Qt::ElideMiddle, int(mRect.width()));
                            }
                        }
...
QSize AdaptiveTextElement::minimumOuterSizeHint() const
{
    if (visible()) {
        QFont font = mFont;
        QFontMetrics metrics(font);
        auto result =
            metrics.boundingRect(0, 0, 0, 0, mTextFlags, mText).size();
...
QSize AdaptiveTextElement::maximumOuterSizeHint() const
{
    return {QWIDGETSIZE_MAX, QWIDGETSIZE_MAX};
}

void AdaptiveTextElement::mouseDoubleClickEvent(QMouseEvent *event,
...
void AdaptiveTextElement::mousePressEvent(QMouseEvent *event,
                                          const QVariant &details)
{
    Q_UNUSED(details)
    event->ignore();
...
void AdaptiveTextElement::mouseMoveEvent(QMouseEvent *event,
...
bool AdaptiveTextElement::needUpdate() const { return mNeedUpdate; }
...
void AdaptiveTextElement::setBoldText(bool bold) { mFont.setBold(bold); }
...
void AdaptiveTextElement::setAdjustStrategy(AdjustStrategy strategy)
{
    if (mAdjustStrategy != strategy) {
        mAdjustStrategy = strategy;
        mNeedUpdate = true;
    }
}
...
void AdaptiveTextElement::draw(QCPPainter *painter)
{
    ...
    if (!mText.isEmpty()) {
        ...
        if (mCachedPixmap) {
            ...
            painter->drawText(mRect, mTextFlags, mTextDisplayed,
                              &mTextBoundingRect);
        }
    }
}
