...
Project::Project(QObject* parent) : QObject(parent), mPagesDirWrittable(true)
{
    ...
    //    connect(mAlarmsWatcher, &QFileSystemWatcher::fileChanged, this,
    //    &Project::alarmFilesChanged); connect(mAlarmsWatcher,
    //    &QFileSystemWatcher::directoryChanged, this,
    //    &Project::alarmFilesChanged);
}
...
bool Project::generate(const QString& workingDirectory,
                       const QString& projectName)
{
    ...
    mProjectPath = QDir::toNativeSeparators(workingDirectory +
    ...
    if (dirWorking.mkdir(projectName)) {
        ...
        if (dirInfo.isWritable()) {
            mSettingsPath = mProjectPath + QDir::separator() + projectName +
                            QString(".project");
            ...
            mParamSettingsPath =
                mProjectPath + QDir::separator() + QString("Parameters");
            ...
            mAlarmsConfigPath =
                mProjectPath + QDir::separator() + QString("Alarms");
            ...
            mResourcesPath =
                mProjectPath + QDir::separator() + QString("Resources");

            QDir dirResources(mResourcesPath);
            if (!dirResources.exists())
                dirResources.mkpath(".");

            mResourcesImagesPath =
                mResourcesPath + QDir::separator() + QString("Images");
...
bool Project::load(const QString& workingDirectory, const QString& projectName,
                   bool fullLoad)
{
    QApplication::setOverrideCursor(Qt::WaitCursor);
    QApplication::processEvents();
    ...
    mProjectPath = QDir::toNativeSeparators(workingDirectory +
                                            QDir::separator() + projectName);
    mSettingsPath =
        mProjectPath + QDir::separator() + projectName + QString(".project");
    ...
    if (fileInfo.exists()) {
        ...
        if (settings.contains(PRO_SETTINGS_KEY_CREATIONDATE))
            mCreationDate =
                settings.value(PRO_SETTINGS_KEY_CREATIONDATE).toDateTime();

        if (settings.contains(PRO_SETTINGS_KEY_MODIFICATIONDATE))
            mModificationDate =
                settings.value(PRO_SETTINGS_KEY_MODIFICATIONDATE).toDateTime();
        ...
        QDir dirPages(mPagesPath);
        if (!dirPages.exists()) {
            mPagesDirWrittable = dirPages.mkpath(".");
        }
        else {
            QFileInfo dirInfo(mPagesPath);
            mPagesDirWrittable = dirInfo.isWritable();
        }
        ...
        if (!dirVisuParam.exists()) {
            mParamSettingsDirWrittable = dirVisuParam.mkpath(".");
        }
        else {
            QFileInfo dirInfo(mParamSettingsPath);
            mParamSettingsDirWrittable = dirInfo.isWritable();
        }
...
void Project::loadPages()
{
    ...
    QStringList pages = directory.entryList(
        QStringList() << "*.page",
    ...
    foreach (QString filename, pages) {
        ...
        auto page =
            QExplicitlySharedDataPointer<Page>(new Page(base, mPagesPath));
...
QString Project::resourcesPath() const { return mResourcesPath; }

QString Project::resourcesImagesPath() const { return mResourcesImagesPath; }

QString Project::alarmsConfigPath() const { return mAlarmsConfigPath; }

QMap<QString, QExplicitlySharedDataPointer<Alarms>>
Project::alarmsConfigurations() const
{
    return mAlarmsConfigurations;
}

QExplicitlySharedDataPointer<Alarms> Project::alarmConfiguration(
    const QString& name)
{
    ...
    //    logger()->warn()
    //        << QString("Alarm configuration does not exist - %1").arg(name);
    return QExplicitlySharedDataPointer<Alarms>();
}

QString Project::requestedPageName() const { return mRequestedPageName; }
...
QList<ElementDecl> Project::findParameter(const QString& parameterName)
{
    ...
    std::sort(pagesNames.begin(), pagesNames.end(), collator);

    for (const auto& pageName : pagesNames) {
        auto path =
            mPagesPath + QDir::separator() + pageName + QString(".page");
        ...
        for (int i = 0; i < elementsCount; i++) {
            ...
            for (int i = 0; i < paramCount; i++) {
                ...
                if (shared) {
                    paramlabel =
                        settings.value("ConnectedParamLabel").toString();
                }
                else {
...
QString Project::pagesPath() const { return mPagesPath; }
...
void Project::loadParametersSettings()
{
    ...
    foreach (QString filename, files) {
        QString parameterSettingsPath =
            mParamSettingsPath + QDir::separator() + filename;
        ...
        if (!parameterSettings->label().isEmpty() &&
            ...
            if (mParametersSettings.value(parameterSettings->label())
                    .contains(parameterSettings->description())) {
                auto conf =
                    mParametersSettings.value(parameterSettings->label())
                        .value(parameterSettings->description());
                //                logger()->warn()
                //                    << QString("Parameter configuration
                //                    conflict  : File %1 - "
                //                               "File : %2")
                //                           .arg(QFileInfo(conf->saveFilePath()).fileName())
                //                           .arg(QFileInfo(parameterSettings->saveFilePath())
                //                                    .fileName());
            }
            else {
                mParametersSettings[parameterSettings->label()].insert(
                    parameterSettings->description(), parameterSettings);
            }
        }
        else {
            //            logger()->warn()
            //                << QString("Parameter configuration is not valid
            //                (empty "
            //                           "string) - Label  : %1 - Descr : %2 -
            //                           File : %3")
            //                       .arg(parameterSettings->label())
            //                       .arg(parameterSettings->description())
            //                       .arg(filename);
        }
    }

    //    logger()->info() << QString("%1 loaded parameter configurations")
    //                            .arg(mParametersSettings.count());
}

void Project::addParameterSettings(
    ParameterConfiguration::Ptr parameterSettings)
{
    mParametersSettings[parameterSettings->label()].insert(
        parameterSettings->description(), parameterSettings);
...
ParameterConfiguration::Ptr Project::parameterSettings(
    const QString& name, const QString& description)
{
    ...
    //    logger()->warn() << QString(
    //                            "Parameter configuration does not exist - %1 -
    //                            %2") .arg(name) .arg(description);

    return ParameterConfiguration::Ptr();
}

QDateTime Project::creationDate() const { return mCreationDate; }
...
QDateTime Project::modificationDate() const { return mModificationDate; }
...
QString Project::name() const { return mName; }
...
QString Project::paramSettingsPath() const { return mParamSettingsPath; }

QString Project::currentPageName() const { return mCurrentPageName; }

QMap<QString,
     QMap<QString, ParameterConfiguration::Ptr>>
Project::parametersSettings() const
{
...
void Project::loadAlarmsConfigurations()
{
    ...
    foreach (QString filename, files) {
        QString alarmConfigPath =
            mAlarmsConfigPath + QDir::separator() + filename;
        //        mAlarmsWatcher->addPath(alarmConfigPath);
        QExplicitlySharedDataPointer<Alarms> alarmConfig =
            QExplicitlySharedDataPointer<Alarms>(new Alarms());

        alarmConfig->loadFromFile(alarmConfigPath);
        mAlarmsConfigurations.insert(alarmConfig->name(), alarmConfig);
    }

    //    logger()->info() << QString("%1 loaded alarm configurations")
    //                            .arg(mAlarmsConfigurations.count());
}

void Project::addAlarmConfiguration(
    QExplicitlySharedDataPointer<Alarms> alarmConfig)
{
    mAlarmsConfigurations.insert(alarmConfig->name(), alarmConfig);
    setModificationDate(QDateTime::currentDateTime());
...
bool Project::exportToCsv(const QString& filePath)
{
    QFile file(filePath);
