      ...
      mFullScreenParent(nullptr)
{
    ...
    addLayer("layout_background", layer("elements_background"),
    ...
    connect(mLiveRefreshTimer, SIGNAL(timeout()), this, SLOT(updateLive()),
            Qt::DirectConnection);

    mProject = QSharedPointer<Project>(new Project());

    setPlottingHints({
        QCP::phCacheLabels | QCP::phImmediateRefresh /*|QCP::phFastPolylines*/
    });
    plotLayout()->clear();
    ...
    connect(mClearElementsAction, &QAction::triggered, this,
            &Board::clearElements);

    auto clearElementsShortcut =
        new QShortcut(QKeySequence(QKeySequence::Back), this);
    connect(clearElementsShortcut, &QShortcut::activated, this,
            &Board::clearElements);

    mDeleteElementsAction = new QAction("Delete element(s)");
    mDeleteElementsAction->setShortcut(QKeySequence(tr("Del")));
    connect(mDeleteElementsAction, &QAction::triggered, this,
            &Board::deleteElements);
    mContextMenu->addAction(mDeleteElementsAction);

    auto deleteElementsShortcut =
        new QShortcut(QKeySequence(QKeySequence::Delete), this);
    connect(deleteElementsShortcut, &QShortcut::activated, this,
            &Board::deleteElements);
    ...
    connect(cutElementsShortcut, &QShortcut::activated, this,
            &Board::cutElements);

    mCopyElementsAction = new QAction("Copy");
    mCopyElementsAction->setShortcut(QKeySequence(tr("Ctrl+C")));
    connect(mCopyElementsAction, &QAction::triggered, this,
            &Board::copyElements);
    mContextMenu->addAction(mCopyElementsAction);

    auto copyElementsShortcut = new QShortcut(QKeySequence(tr("Ctrl+C")), this);
    connect(copyElementsShortcut, &QShortcut::activated, this,
            &Board::copyElements);

    mPasteElementsAction = new QAction("Paste");
    mPasteElementsAction->setShortcut(QKeySequence(tr("Ctrl+V")));
    connect(mPasteElementsAction, &QAction::triggered, this,
            &Board::pasteElements);
    mContextMenu->addAction(mPasteElementsAction);

    auto pasteElementsShortcut =
        new QShortcut(QKeySequence(tr("Ctrl+V")), this);
    connect(pasteElementsShortcut, &QShortcut::activated, this,
            &Board::pasteElements);

    clearPage();
    mColorIndex = 1;

    connect(mDashboardLayout, &BoardLayout::pageModified, this,
    ...
    connect(mDataManager.data(), &DataManager::parametersUpdated, this,
            &Board::checkParameters);
    //    connect(mDataManager.data(), &DataManager::reinitPlayback, this,
    //    &Board::resetPlayback);
}
...
BoardLayout *Board::dashboardLayout() const { return mDashboardLayout; }

QSharedPointer<DataManager> Board::dataManager() const { return mDataManager; }

void Board::checkModification()
{
    if (mPageModified && mProject && mProject->currentPage()) {
        QMessageBox msgBox;
        msgBox.setText(QString("The page %1\n has been modified.")
                           .arg(mProject->currentPage()->name()));
...
void Board::updateLive()
{
    ...
    if (ms > mRefreshTimerPeriod) {
        //        logger()->warn() << QString("Board replot too slow - %1
        //        ms").arg(ms);
    }
...
void Board::updateElements(bool forceUpdate)
{
    for (int i = 0; i < mDashboardLayout->elementCount(); i++) {
        if (auto *el =
                qobject_cast<BoardElement *>(mDashboardLayout->elementAt(i))) {
...
{
    if (mDataManager) {
        bool reload = mDataManager->setPlaybackDateTime(
            double(mReferencePlaybackTime) / 1000., mReferencePlaybackDate);
        if (reload) {
        ...
        for (int i = 0; i < mDashboardLayout->elementCount(); i++) {
            if (auto *el = qobject_cast<BoardElement *>(
                    mDashboardLayout->elementAt(i))) {
...
void Board::editElement()
{
    ...
    if (mSelectedIndexes.count() == 1) {
        auto *element = qobject_cast<BoardElement *>(
            mDashboardLayout->elementAt(mSelectedIndexes.first()));
        if (element) {
            element->edit();
...
void Board::clearElements()
{
    ...
    for (auto index : mSelectedIndexes) {
        auto *element =
            qobject_cast<BoardElement *>(mDashboardLayout->elementAt(index));
        if (element) {
...
void Board::deleteElements()
{
    ...
    int indexCol = int((mouseBoardPos.x() - layoutMargins.left()) /
                       mDashboardLayout->columnWidth());
    int indexRow = int((mouseBoardPos.y() - layoutMargins.top()) /
                       mDashboardLayout->rowHeight());
    ...
    if (indexElement > -1) {
        ...
        if (mHoverRect->geometry() != rect) {
            mHoverRect->hide();
            mHoverRect->setGeometry(rect);
        ...
        QRectF elementRect = QRectF(
            layoutMargins.left() + indexCol * mDashboardLayout->columnWidth() +
                double(mDashboardLayout->columnSpacing() / 2.0),
            layoutMargins.top() + indexRow * mDashboardLayout->rowHeight() +
                double(mDashboardLayout->rowSpacing() / 2.0),
...
void Board::copyElements()
{
    ...
    if (!tmpFileName.isEmpty()) {
        ...
        for (int i = 0; i < elementIds.count(); i++) {
            auto *element = qobject_cast<BoardElement *>(
                mDashboardLayout->elementAt(std::get<0>(elementIds.at(i))));
            if (element) {
        ...
        mimeData->setData(Board::elementCopyMimeType(), tmpFileName.toUtf8());
    ...
    if (mHoverRect->isVisible()) {
        ...
        if (!tmpFileName.isEmpty()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                settings.beginGroup("ElementGeometry");
                QRect elementRect =
                    mDashboardLayout->loadElementRectSettings(&settings);
                settings.endGroup();
                ...
                auto *element =
                    ElementFactory::Factory()->createElement(elementName);
                if (element) {
                    ...
                    if (mDashboardLayout->addElement(
                            element, true, indexCol + offsetCol,
                            indexRow + offsetRow, elementRect.width(),
                            elementRect.height())) {
                        settings.beginGroup("ElementConfigurations");
...
void Board::dropEvent(QDropEvent *event)
{
    if (!mLayoutLocked) {
        if (mProject->currentPage()) {
            auto mouseBoardPos = event->position().toPoint();
            auto layoutMargins = mDashboardLayout->margins();
            int indexCol = int((mouseBoardPos.x() - layoutMargins.left()) /
                               mDashboardLayout->columnWidth());
            int indexRow = int((mouseBoardPos.y() - layoutMargins.top()) /
            ...
            int indexElement =
                mDashboardLayout->elementIndex(indexCol, indexRow);

            if (event->mimeData()->hasFormat(Board::elementNameMimeType())) {
                if (indexElement == -1) {
                    const QMimeData *mimeData = event->mimeData();
                    QByteArray encoded =
                        mimeData->data(Board::elementNameMimeType());
                    QString elementName = QString::fromUtf8(encoded);

                    auto boardElement =
                        ElementFactory::Factory()->createElement(elementName);
                    if (boardElement) {
                        addElement(boardElement, event->position());
                        ...
                        mSelectedIndexes.clear();
                         ...
                         Board::propertyNameMimeType())) {
                const QMimeData *mimeData = event->mimeData();
                QByteArray encoded =
                    mimeData->data(Board::propertyNameMimeType());
                ...
                if (paramSettings) {
                    auto element = qobject_cast<BoardElement *>(
                        mDashboardLayout->elementAt(indexElement));
                    if (!element) {
                        QDialog dialog;
                        dialog.setWindowTitle("Element Picker");
                        dialog.setMinimumSize(300, 350);

                        auto *lay = new QVBoxLayout(&dialog);
                        auto *elemntPicker = new ElementPickerWidget(
                            &dialog, BoardElement::etTimeSeries);
                        ...
                        connect(
                            elemntPicker,
                            &ElementPickerWidget::elementDoubleClicked, this,
                            [&dialog, elemntPicker]() {
                                if (!elemntPicker->selectedElement().isEmpty())
                                    dialog.accept();
                            });
                        connect(
                            buttonBox, &QDialogButtonBox::accepted, this,
                            [&dialog, elemntPicker]() {
                                if (!elemntPicker->selectedElement().isEmpty())
                                    dialog.accept();
                            });
                        ...
                        if (res == QDialog::Accepted) {
                            QString elementName =
                                elemntPicker->selectedElement();
                            auto el = ElementFactory::Factory()->createElement(
                                elementName);
                            if (el) {
                                addElement(el, event->position());
                    ...
                    if (element) {
                        ...
                        event->accept();
                         ...
                         Board::parameterNameMimeType())) {
                const QMimeData *mimeData = event->mimeData();
                QByteArray encoded =
                    mimeData->data(Board::parameterNameMimeType());

                QDataStream stream(&encoded, QIODevice::ReadOnly);
                QStringList listLabels;
                stream >> listLabels;

                auto boardElement = qobject_cast<BoardElement *>(
                    mDashboardLayout->elementAt(indexElement));
                if (!boardElement) {
                    BoardElement::TimeSeriesType tsType =
                        BoardElement::tstUnknown;
                    for (const auto &label : listLabels) {
                        QSharedPointer<TimeSeries> param =
                            mDataManager->dataParameter(label);
                        if (param) {
                            if (tsType == BoardElement::tstUnknown) {
                                ...
                                if (param->size() > 1) {
                                    if (tsType == BoardElement::tstScalar) {
                                        tsType =
                                            BoardElement::tstScalarAndVector;
                                        break;
                                    }
                                }
                                else {
                    ...
                    auto *lay = new QVBoxLayout(&dial);
                    auto *elemntPicker = new ElementPickerWidget(
                        &dial, BoardElement::etTimeSeries, tsType, tsSize);

                    auto buttonBox = new QDialogButtonBox(
                        QDialogButtonBox::Ok | QDialogButtonBox::Cancel);

                    connect(elemntPicker,
                            &ElementPickerWidget::elementDoubleClicked, this,
                            [&dial, elemntPicker]() {
                                if (!elemntPicker->selectedElement().isEmpty())
                                    dial.accept();
                ...
                if (boardElement) {
                    if (mDataManager) {
                        for (const auto &label : listLabels) {
                            ...
                            if (param) {
                                bool replace = false;
                                if (boardElement->parametersCount() > 0 &&
                                    boardElement->timeSeriesSize() ==
                                        BoardElement::tssSingle) {
                                    replace = true;
                                    if (event->modifiers() &
                                        Qt::ControlModifier)
                        ...
                        event->accept();
                         ...
                         Board::alarmConfigMimeType())) {
                const QMimeData *mimeData = event->mimeData();
                QString label = QString::fromStdString(
                    mimeData->data(Board::alarmConfigMimeType()).toStdString());
                ...
                if (alarm) {
                    if (!element) {
                        ...
                        auto *elemntPicker =
                            new ElementPickerWidget(&dial, type);
                        ...
                        connect(
                            elemntPicker,
                            &ElementPickerWidget::elementDoubleClicked,
                            [&dial, elemntPicker]() {
                                if (!elemntPicker->selectedElement().isEmpty())
                                    dial.accept();
                            });
            ...
            if (event->mimeData()->hasFormat(Board::elementNameMimeType()) ||
                ...
                if (indexElement > -1) {
                    if (mSelectedIndexes.contains(indexElement)) {
                        ...
                        auto rect = mDashboardLayout->elementAt(indexElement)
                                        ->outerRect();
                        if (mHoverRect->geometry() != rect) {
                            mHoverRect->hide();
                            mHoverRect->setGeometry(rect);
                    ...
                    QRectF elementRect = QRectF(
                        layoutMargins.left() +
                            indexCol * mDashboardLayout->columnWidth() +
                            double(mDashboardLayout->columnSpacing() / 2.0),
                        layoutMargins.top() +
                            indexRow * mDashboardLayout->rowHeight() +
                            double(mDashboardLayout->rowSpacing() / 2.0),
                        mDashboardLayout->columnWidth() -
                            mDashboardLayout->columnSpacing(),
                        mDashboardLayout->rowHeight() -
                            mDashboardLayout->rowSpacing());
                ...
                if (event->mimeData()->hasFormat(
                    ...
                    if (indexElement > -1) {
                        auto boardElement = qobject_cast<BoardElement *>(
                            mDashboardLayout->elementAt(indexElement));
                        if (boardElement && boardElement->type() ==
                    ...
                    if (indexElement > -1) {
                        auto boardElement = qobject_cast<BoardElement *>(
                            mDashboardLayout->elementAt(indexElement));
                        if (boardElement &&
                            boardElement->type() == BoardElement::etAlarm) {
                            validDrag = true;
                        }
                    }
                    else {
...
void Board::mousePressEvent(QMouseEvent *event)
{
    ...
    if (!mLayoutLocked) {
        if (mAnimationGroup->state() == QAnimationGroup::Running)
            return;
        auto mouseBoardPos = mapFromGlobal(event->globalPosition().toPoint());
        auto layoutMargins = mDashboardLayout->margins();
        int indexCol = int((mouseBoardPos.x() - layoutMargins.left()) /
                           mDashboardLayout->columnWidth());
        int indexRow = int((mouseBoardPos.y() - layoutMargins.top()) /
        ...
        if (index > -1) {
            ...
            if (event->modifiers() & Qt::ControlModifier) {
                if (mSelectedIndexes.contains(index)) {
                    ...
                    auto selectionRect = QSharedPointer<SelectionRect>(
                        new SelectionRect(this), &QObject::deleteLater);
                    ...
                    if (selectionRect->geometry() != rect) {
                        selectionRect->hide();
                        selectionRect->setGeometry(rect);
                ...
                if (!mSelectedIndexes.contains(index)) {
                    ...
                    mResizeOrientation = selectionRect->hoverHandle(
                        event->globalPosition().toPoint());
                }
                else {
                    mResizeOrientation = mSelectedRects.first()->hoverHandle(
                        event->globalPosition().toPoint());
                }
            }
        }
        ...
        if (event->button() == Qt::RightButton) {
            ...
            mContextMenu->exec(event->globalPosition().toPoint() +
                               QPoint(3, 3));
        }
    }
}
        ...
        if (mMouseHasMoved) {
            if (event->buttons() & Qt::LeftButton) {
                ...
                if (mRubberBand->isVisible()) {
                    auto rect =
                        QRect(mRubberBandOrigin, event->pos()).normalized();
                    mRubberBand->setGeometry(rect);

                    auto layoutMargins = mDashboardLayout->margins();

                    int col = qRound((rect.x() - layoutMargins.left()) /
                                     mDashboardLayout->columnWidth());
                    int row = qRound((rect.y() - layoutMargins.top()) /
                                     mDashboardLayout->rowHeight());
                    int width =
                        qRound(rect.width() / mDashboardLayout->columnWidth());
                    int height =
                        qRound(rect.height() / mDashboardLayout->rowHeight());
                    ...
                    for (int i = 0; i < width; i++) {
                        for (int j = 0; j < height; j++) {
                            auto index = mDashboardLayout->elementIndex(
                                col + i, row + j);
                            if (index >= 0 &&
                                !mSelectedIndexes.contains(index)) {
                                auto selectionRect =
                                    QSharedPointer<SelectionRect>(
                                        new SelectionRect(this),
                                        &QObject::deleteLater);
                                selectionRect->setBrushActive(true);
                                ...
                                auto rect = mDashboardLayout->elementAt(index)
                                                ->outerRect();
                                if (selectionRect->geometry() != rect) {
                                    selectionRect->hide();
                                    selectionRect->setGeometry(rect);
                    ...
                    if (!mSelectedIndexes.empty()) {
                        ...
                        for (int k = 0; k < mSelectedIndexes.count(); k++) {
                            ...
                            switch (mResizeOrientation) {
                                case SelectionRect::hhNone: {
                                    ...
                                    int xShift =
                                        event->pos().x() - mMousePos.x();
                                    int yShift =
                                        event->pos().y() - mMousePos.y();
                            ...
                            if (validRect) {
                                ...
                                auto layoutMargins =
                                    mDashboardLayout->margins();

                                draggedElementNewColumn =
                                    qRound((rect.x() - layoutMargins.left()) /
                                           mDashboardLayout->columnWidth());
                                draggedElementNewRow =
                                    qRound((rect.y() - layoutMargins.top()) /
                                           mDashboardLayout->rowHeight());
                                newDragWidth =
                                    qRound(rect.width() /
                                           mDashboardLayout->columnWidth());
                                ...
                                auto gridRect =
                                    mDashboardLayout->elementGridRectAt(
                                        indexSelection);
                                ...
                                if (draggedElementNewColumn + newDragWidth >
                                    mDashboardLayout->columnCount()) {
                                    locked = true;
                                    draggedElementNewColumn =
                                        mDashboardLayout->columnCount() -
                                        gridRect.width();
                                ...
                                if (draggedElementNewRow + newDragHeight >
                                    mDashboardLayout->rowCount()) {
                                    locked = true;
                                    draggedElementNewRow =
                                        mDashboardLayout->rowCount() -
                                        gridRect.height();
                                ...
                                for (int i = 0; i < newDragWidth; i++) {
                                    for (int j = 0; j < newDragHeight; j++) {
                                        auto index =
                                            mDashboardLayout->elementIndex(
                                                draggedElementNewColumn + i,
                                                draggedElementNewRow + j);
                                        if (index >= 0 &&
                                            !mSelectedIndexes.contains(index)) {
                                            validPosition = false;
                                            break;
                        ...
                        bool canMove = true;
                        for (auto st : validRects) canMove &= st;
                        for (auto st : validPositions) canMove &= st;
                        for (auto st : lockedElements) canMove &= !st;

                        if (canMove) {
                            for (int k = 0; k < mSelectedIndexes.count(); k++) {
                                mDashboardLayout->setElementGeometry(
                                    mSelectedIndexes.at(k), newCols.at(k),
                                    newRows.at(k), newWidths.at(k),
                                    newHeights.at(k));
                            }
                            replot(QCustomPlot::rpQueuedReplot);
                        }
                ...
                if (indexElement > -1) {
                    if (mSelectedIndexes.contains(indexElement)) {
                        ...
                        if (mSelectedIndexes.count() > 1) {
                            ...
                            auto handle =
                                mSelectedRects
                                    .at(mSelectedIndexes.indexOf(indexElement))
                                    ->hoverHandle(
                                        event->globalPosition().toPoint());
                            switch (handle) {
                                case SelectionRect::hhNone:
                        ...
                        auto handle = mHoverRect->hoverHandle(
                            event->globalPosition().toPoint());
                        switch (handle) {
                            case SelectionRect::hhNone:
                                setCursor(QCursor(Qt::ArrowCursor));
    ...
    if (mRubberBand->isVisible()) {
        ...
        if (mSelectedIndexes.count() > 0) {
            ...
            for (int i = 0; i < mSelectedIndexes.count(); i++) {
                ...
                auto finalRect =
                    mDashboardLayout->elementAt(mSelectedIndexes.at(i))
                        ->outerRect();
                finalRect.moveTopLeft(
                    (mDashboardLayout->elementAt(mSelectedIndexes.at(i))
                         ->outerRect()
                         .topLeft()));
                ...
                connect(animation, &QPropertyAnimation::valueChanged,
                        [selRect]() { selRect->update(); });

                mAnimationGroup->addAnimation(animation);
            }
            connect(mAnimationGroup, &QAnimationGroup::finished, this,
                    [this]() { mAnimationGroup->clear(); });
    ...
    if (!mLayoutLocked) {
        if (event->button() == Qt::LeftButton) {
            ...
            if (index > -1) {
                ...
                auto *element = qobject_cast<BoardElement *>(
                    mDashboardLayout->elementAt(index));
                if (element) {
                    element->edit();
                    setPageModified(true);
                }
                ...
                QVBoxLayout *lay = new QVBoxLayout();
                auto *elemntPicker = new ElementPickerWidget(
                    &dial, BoardElement::etUnknown, BoardElement::tstUnknown,
            ...
            if (mouseInteractions()) {
                ...
                auto mouseBoardPos = mapFromGlobal(mouseGlobalPos);
                if (auto *el = qobject_cast<BoardElement *>(
                        mDashboardLayout->elementAt(mouseBoardPos))) {
                    event->accept();
                    el->loadData();
                }
            }
...
void Board::resizeEvent(QResizeEvent *event)
{
    ...
    for (int i = 0; i < mSelectedIndexes.count(); i++) {
        auto rect =
            mDashboardLayout->elementAt(mSelectedIndexes.at(i))->outerRect();
        mSelectedRects.at(i)->hide();
        mSelectedRects.at(i)->setGeometry(rect);
...
bool Board::layoutLocked() const { return mLayoutLocked; }
...
bool Board::mouseInteractions() const { return mMouseInteractions; }
...
bool Board::pageModified() const { return mPageModified; }

void Board::setPageModified(bool pageModified) { mPageModified = pageModified; }
...
bool Board::statMode() const { return mStatMode; }
...
bool Board::liveDataRefreshEnabled() const { return mLiveDataRefreshEnabled; }
...
void Board::loadPage()
{
    ...
    if (page) {
        //        logger()->info() << QString("Loading page
        //        %1").arg(page->name());
        ...
        mDashboardLayout->setSingleElementRowCount(
            page->singleElementRowCount());
        mDashboardLayout->setSingleElementColumnCount(
            page->singleElementColumnCount());

        QPixmap pic(mProject->resourcesImagesPath() + QDir::separator() +
                    page->background());
...
QSharedPointer<Project> Board::project() const { return mProject; }
    ...
    if (!mFullScreenParent) {
        ...
        if (!wparent->layout()) {
            if (qobject_cast<QSplitter *>(wparent)) {
                mFullScreenParentLayoutIndex =
                    qobject_cast<QSplitter *>(wparent)->indexOf(this);
            }
            else {
                return false;
            }
            ...
            for (int i = 0; i < lay->count(); i++) {
                ...
                if (lay->itemAtPosition(mFullScreenParentGridRow,
                                        mFullScreenParentGridCol)
                        ->widget() == this)
                    break;
            }
        }
        ...
        mFullScreenParent->setContentsMargins({});
        ...
        if (qobject_cast<QSplitter *>(mFullScreenEmbeddedParent)) {
            qobject_cast<QSplitter *>(mFullScreenEmbeddedParent)
                ->insertWidget(mFullScreenParentLayoutIndex, this);
        }
        else if (qobject_cast<QBoxLayout *>(
                     mFullScreenEmbeddedParent->layout())) {
            qobject_cast<QBoxLayout *>(mFullScreenEmbeddedParent->layout())
                ->insertWidget(mFullScreenParentLayoutIndex, this);
        }
        else if (qobject_cast<QGridLayout *>(
                     mFullScreenEmbeddedParent->layout())) {
            QGridLayout *gl = qobject_cast<QGridLayout *>(
                mFullScreenEmbeddedParent->layout());
            gl->addWidget(this, mFullScreenParentGridRow,
                          mFullScreenParentGridCol);
        }
        else if (qobject_cast<QStackedLayout *>(
                     mFullScreenEmbeddedParent->layout())) {
        }
        else {
        }
