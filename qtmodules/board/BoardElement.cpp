...
QSharedPointer<BoardParameter> BoardElement::addParameter(
    ParameterConfiguration::Ptr parameterSettings)
{
    ...
    if (!mParametersLabel.contains(parameterSettings->label())) {
        boardParam = QSharedPointer<BoardParameter>(
            new BoardParameter(std::move(parameterSettings), mBoard));
...
QSharedPointer<BoardParameter> BoardElement::addParameter(QString paramLabel)
{
    ...
    if (!mParametersLabel.contains(paramLabel)) {
        boardParam = QSharedPointer<BoardParameter>(
            new BoardParameter(paramLabel, mBoard));
        boardParam->parameterConfiguration()
            ->defaultColorSettingsRef()
            .setColor(mBoard->randomColor(mParameterColorIndex));
    ...
    QSharedPointer<TimeSeries> dataParameter)
{
    ...
    if (!mParametersLabel.contains(dataParameter->name())) {
        boardParam = QSharedPointer<BoardParameter>(
            new BoardParameter(dataParameter, mBoard));
        boardParam->parameterConfiguration()
            ->defaultColorSettingsRef()
            .setColor(mBoard->randomColor(mParameterColorIndex));
...
void BoardElement::addParameter(QSharedPointer<BoardParameter> boardParameter)
{
    if (mBoardParameters.contains(boardParameter)) {
        //        logger()->info() << name() << " "
        //                         << boardParameter->getTimeSeries().name()
        //                         << " already added";
        return;
    }
    ...
    if (mBoardParameters.count() < mParametersMaxCount) {
        ...
        mBoard->dataManager()->registerBoardParameterId(
            boardParameter->getTimeSeries().parameterId());
    }
        ...
        mBoard->dataManager()->unregisterBoardParameterId(
            toRemove->getTimeSeries().parameterId());
        mBoard->dataManager()->registerBoardParameterId(
            boardParameter->getTimeSeries().parameterId());
    }
}
...
bool BoardElement::statModeEnabled() const { return mStatModeEnabled; }
...
void BoardElement::setBoardLiveState(bool isLive) { Q_UNUSED(isLive) }
...
Board *BoardElement::board() const { return mBoard; }

QString BoardElement::name() const { return mName; }

void BoardElement::setName(const QString &name) { mName = name; }
...
void BoardElement::setTimeSeriesSize(TimeSeriesSize timeSeriesSize)
{
...
void BoardElement::setTimeSeriesType(TimeSeriesType timeSeriesType)
{
...
void BoardElement::setBackgroundMode(BackgroundMode backgroundMode)
{
    mBackgroundMode = backgroundMode;
}

BoardElement::ElementType BoardElement::type() const { return mType; }

void BoardElement::setType(ElementType type) { mType = type; }
...
ParameterConfiguration::ConfigurationMode BoardElement::configurationMode()
    const
{
    return mConfigurationMode;
}

void BoardElement::setConfigurationMode(
    ParameterConfiguration::ConfigurationMode mode)
{
    mConfigurationMode = mode;
}
...
void BoardElement::droppedParameter(
    ParameterConfiguration::Ptr parameterSettings)
{
...
void BoardElement::droppedParameter(QSharedPointer<TimeSeries> dataParameter,
                                    bool replace)
{
    if (replace)
...
int BoardElement::parametersMaxCount() const { return mParametersMaxCount; }
...
void BoardElement::saveConfigurations(QSettings *settings)
{
    ...
    for (int i = 0; i < mBoardParameters.count(); i++) {
        ...
        mBoardParameters.at(i)->saveParameterSettings(settings,
                                                      mConfigurationMode);
    }
    settings->endArray();
}
...
{
    if (mBoardParameters.count() > 0 && index < mBoardParameters.count()) {
...
void BoardElement::removeBoardParameter(int index, bool update)
{
    ...
    if (mBoard->dataManager())
        mBoard->dataManager()->unregisterBoardParameterId(
            boardParam->getTimeSeries().parameterId());
...
QSharedPointer<BoardParameter> BoardElement::replaceParameter(
    int index,
    ParameterConfiguration::Ptr parameterSettings)
{
    QSharedPointer<BoardParameter> boardParam;
    if (!mParametersLabel.contains(parameterSettings->label())) {
        if (index < mBoardParameters.count() && index >= 0) {
            boardParam = QSharedPointer<BoardParameter>(
                new BoardParameter(parameterSettings, mBoard));
...
void BoardElement::replaceParameter(
    int index, QSharedPointer<BoardParameter> boardParameter)
{
    if (mBoardParameters.contains(boardParameter)) {
        ...
        if (index < mBoardParameters.count() && index >= 0) {
            ...
            mParametersLabel.replace(index,
                                     boardParameter->getTimeSeries().name());
            mBoard->dataManager()->registerBoardParameterId(
                boardParameter->getTimeSeries().parameterId());
            mBoard->dataManager()->unregisterBoardParameterId(id);
...
double BoardElement::selectTest(const QPointF &pos, bool onlySelectable,
                                QVariant *details) const
{
    ...
    if (pos.x() >= left && pos.x() <= right && pos.y() >= bottom &&
        ...
        //            logger()->debug() << Q_FUNC_INFO << "parent plot not
        //            defined";
        return -1.;
    }
