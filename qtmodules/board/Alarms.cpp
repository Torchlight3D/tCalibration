...
void Alarms::saveToFile(const QString &saveDirectory)
{
    if (mSaveFilePath.isEmpty()) {
        mSaveFilePath =
            saveDirectory + QDir::separator() + mName + QString(".alrm");
    }

    //    logger()->info() << QString("Saving AlarmConfiguration [%1] to file
    //    [%2]")
    //                            .arg(mName)
    //                            .arg(mSaveFilePath);
...
QString Alarms::name() const { return mName; }

void Alarms::setName(const QString &name) { mName = name; }
...
void Alarms::addAlarm(bool active, QString watchedParam, QString message,
                      QString mOutputParam, Alarms::AlarmFunction function,
                      double arg, QColor color)
{
...
QList<bool> Alarms::active() const { return mActive; }

QStringList Alarms::watchedParameters() const { return mWatchedParameters; }

QStringList Alarms::messages() const { return mMessages; }

QStringList Alarms::outputParameters() const { return mOutputParameters; }

QList<Alarms::AlarmFunction> Alarms::functions() const { return mFunctions; }

QList<double> Alarms::functionArgs() const { return mFunctionArgs; }

QList<QColor> Alarms::colors() const { return mColors; }

bool Alarms::modified() const { return mModified; }

void Alarms::setModified(bool modified) { mModified = modified; }
