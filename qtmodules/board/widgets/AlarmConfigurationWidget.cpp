#include "AlarmConfigurationWidget.h"
#include "ui_AlarmConfigurationWidget.h"
...
{
    ui->setupUi(this);
    ui->alarmsTableWidget->horizontalHeader()->setSectionResizeMode(
        QHeaderView::Stretch);

    mModified = false;

    connect(ui->newPushButton, &QPushButton::clicked, this,
            &AlarmConfigurationWidget::newAlarm);
    connect(ui->downPushButton, &QPushButton::clicked, this,
            &AlarmConfigurationWidget::moveDown);
    connect(ui->upPushButton, &QPushButton::clicked, this,
            &AlarmConfigurationWidget::moveUp);
    connect(ui->delPushButton, &QPushButton::clicked, this,
            &AlarmConfigurationWidget::removeAlarm);
    connect(ui->filterPushButton, &QPushButton::clicked, this,
            &AlarmConfigurationWidget::filter);
    connect(ui->resetFilterPushButton, &QPushButton::clicked, this,
            &AlarmConfigurationWidget::clearFilter);
    connect(ui->alarmsTableWidget, &QTableWidget::itemDoubleClicked, this,
            &AlarmConfigurationWidget::doubleClickedItem);
    connect(ui->connectedCheckBox, &QCheckBox::stateChanged, this,
            &AlarmConfigurationWidget::stateChanged);
}

AlarmConfigurationWidget::~AlarmConfigurationWidget() { delete ui; }

void AlarmConfigurationWidget::setEditionMode(
    const AlarmConfigurationWidget::EditionMode &mode)
{
    mEditionMode = mode;
...
void AlarmConfigurationWidget::updateConfig()
{
    ...
    for (int i = 0; i < ui->alarmsTableWidget->rowCount(); i++) {
        ...
        if (ui->alarmsTableWidget->item(i, 2)->text() ==
            QString("Greater than X"))
            func = Alarms::afGreaterThan;
        if (ui->alarmsTableWidget->item(i, 2)->text() ==
            QString("Lower than X"))
            func = Alarms::afLowerThan;
        if (ui->alarmsTableWidget->item(i, 2)->text() ==
            QString("Parameter not alive"))
            func = Alarms::afParamDead;

        mAlarmConfig->addAlarm(
            active, ui->alarmsTableWidget->item(i, 1)->text(),
            ui->alarmsTableWidget->item(i, 4)->text(),
            ui->alarmsTableWidget->item(i, 5)->text(), func,
...
void AlarmConfigurationWidget::updateUi()
{
    ...
    for (int i = 0; i < mAlarmConfig->alarmsCount(); i++) {
        ...
        ui->alarmsTableWidget->setItem(
            ui->alarmsTableWidget->rowCount() - 1, 0,
            new QTableWidgetItem(mAlarmConfig->active().at(i) ? QString("Yes")
                                                              : QString("No")));

        ui->alarmsTableWidget->setItem(
            ui->alarmsTableWidget->rowCount() - 1, 1,
        ...
        switch (mAlarmConfig->functions().at(i)) {
            case Alarms::afBitSet:
                ...
                ui->alarmsTableWidget->setItem(
                    ui->alarmsTableWidget->rowCount() - 1, 2,
        ...
        ui->alarmsTableWidget->setItem(
            ui->alarmsTableWidget->rowCount() - 1, 3,
            new QTableWidgetItem(
                QString::number(mAlarmConfig->functionArgs().at(i))));

        ui->alarmsTableWidget->setItem(
            ui->alarmsTableWidget->rowCount() - 1, 4,
            new QTableWidgetItem(mAlarmConfig->messages().at(i)));

        ui->alarmsTableWidget->setItem(
            ui->alarmsTableWidget->rowCount() - 1, 5,
            new QTableWidgetItem(mAlarmConfig->outputParameters().at(i)));

        auto colorItem = new QTableWidgetItem(
            mAlarmConfig->colors().at(i).name(QColor::HexArgb));
        colorItem->setForeground(mAlarmConfig->colors().at(i));

        ui->alarmsTableWidget->setItem(ui->alarmsTableWidget->rowCount() - 1, 6,
                                       colorItem);
    }
}
...
QExplicitlySharedDataPointer<Alarms> AlarmConfigurationWidget::alarmConfig()
    const
{
    return mAlarmConfig;
}

void AlarmConfigurationWidget::setAlarmConfig(
...
void AlarmConfigurationWidget::filter()
{
    ...
    if (!ui->watchedParamLineEdit->text().isEmpty()) {
        ...
        auto items = ui->alarmsTableWidget->findItems(
            ui->watchedParamLineEdit->text(), Qt::MatchExactly);
        for (auto item : items)
            if (item->column() == 1)
    ...
    if (!ui->messageLineEdit->text().isEmpty()) {
        QList<int> rowsToShow;
        auto items = ui->alarmsTableWidget->findItems(
            ui->messageLineEdit->text(), Qt::MatchContains);
        for (auto item : items)
            if (item->column() == 4)
                rowsToShow.append(item->row());
    ...
    if (!ui->displayedParamLineEdit->text().isEmpty()) {
        QList<int> rowsToShow;
        auto items = ui->alarmsTableWidget->findItems(
            ui->displayedParamLineEdit->text(), Qt::MatchContains);
        for (auto item : items)
            if (item->column() == 5)
                rowsToShow.append(item->row());
...
void AlarmConfigurationWidget::newAlarm()
{
    ...
    connect(&functionCombo, &QComboBox::currentIndexChanged,
            [&argSpinBox](int index) {
                argSpinBox.setDecimals(index == 2 ? 0 : 4);
                argSpinBox.setEnabled(index != 3);
            });
    ...
    connect(&buttonBox, &QDialogButtonBox::accepted,
            [&dial, &messageLineEdit, &watchedParamLineEdit]() {
                if (!watchedParamLineEdit.text().isEmpty() &&
                    !messageLineEdit.text().isEmpty())
                    dial.accept();
                else
                    QMessageBox::warning(nullptr, "Alarm Creation",
    ...
    if (res == QDialog::Accepted) {
        ...
        ui->alarmsTableWidget->setItem(
            ui->alarmsTableWidget->rowCount() - 1, 0,
            new QTableWidgetItem(activeCheck.isChecked() ? QString("Yes")
                                                         : QString("No")));
        ...
        auto colorItem =
            new QTableWidgetItem(colorLineEdit.color().name(QColor::HexArgb));
    ...
    if (items.count() > 0) {
        ...
        if (row < ui->alarmsTableWidget->rowCount() - 1) {
            ...
            for (int i = 0; i < ui->alarmsTableWidget->columnCount(); i++) {
                ...
                itemsToMoveUp.append(
                    ui->alarmsTableWidget->takeItem(row + 1, i));
            }

            for (int i = 0; i < ui->alarmsTableWidget->columnCount(); i++) {
                ui->alarmsTableWidget->setItem(row, i, itemsToMoveUp.at(i));
                ui->alarmsTableWidget->setItem(row + 1, i,
                                               itemsToMoveDown.at(i));
            }
...
void AlarmConfigurationWidget::doubleClickedItem(QTableWidgetItem *item)
{
    if (item) {
        ...
        watchedParamLineEdit.setText(
            ui->alarmsTableWidget->item(row, 1)->text());
        functionCombo.setCurrentText(
            ui->alarmsTableWidget->item(row, 2)->text());
        argSpinBox.setValue(
            ui->alarmsTableWidget->item(row, 3)->text().toDouble());
        messageLineEdit.setText(ui->alarmsTableWidget->item(row, 4)->text());

        displayedParamLineEdit.setText(
            ui->alarmsTableWidget->item(row, 5)->text());

        colorLineEdit.setColor(
            QColor(ui->alarmsTableWidget->item(row, 6)->text()));
        ...
        if (res == QDialog::Accepted) {
            ...
            ui->alarmsTableWidget->setItem(
                row, 1, new QTableWidgetItem(watchedParamLineEdit.text()));

            ui->alarmsTableWidget->setItem(
                row, 2, new QTableWidgetItem(functionCombo.currentText()));
            ...
            ui->alarmsTableWidget->setItem(
                row, 4, new QTableWidgetItem(messageLineEdit.text()));

            ui->alarmsTableWidget->setItem(
                row, 5, new QTableWidgetItem(displayedParamLineEdit.text()));

            auto colorItem = new QTableWidgetItem(
                colorLineEdit.color().name(QColor::HexArgb));
            colorItem->setForeground(colorLineEdit.color());

            ui->alarmsTableWidget->setItem(row, 6, colorItem);
            mModified = true;
        }
    }
}
...
void AlarmConfigurationWidget::resizeColumns()
{
    ...
    if (width < ui->alarmsTableWidget->width()) {
        ...
        for (int i = 0; i < ui->alarmsTableWidget->columnCount(); i++)
            ui->alarmsTableWidget->setColumnWidth(
                i, ratio * ui->alarmsTableWidget->columnWidth(i) - 1);
    }
}
