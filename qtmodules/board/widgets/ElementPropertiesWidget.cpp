...
#include "ui_ElementPropertiesWidget.h"
...
ElementPropertiesWidget::ElementPropertiesWidget(QWidget *parent)
    : QWidget(parent),
      ui(new Ui::ElementPropertiesWidget),
      mProject(nullptr),
      ...
      mBitfieldsModified(false)
{
    ...
    connect(ui->createConfigurationPushButton, &QPushButton::clicked, this,
            &ElementPropertiesWidget::createSharedConfiguration);
}

ElementPropertiesWidget::~ElementPropertiesWidget() { delete ui; }

bool ElementPropertiesWidget::isConnected()
{
    ...
    ParameterConfiguration::Ptr parameterSettings)
{
    ...
    ui->colorLineEdit->setColor(
        parameterSettings->defaultColorSettingsRef().color());
    ui->colorLineEdit->setMode(
        parameterSettings->defaultColorSettingsRef().mode());
    ui->colorModeComboBox->setCurrentIndex(
        parameterSettings->defaultColorSettingsRef().mode() - 1);

    ui->itemsColorComboBox->setCurrentIndex(parameterSettings->itemColorMode());
    if (parameterSettings->itemColorMode() ==
        ParameterConfiguration::icCustom) {
        ui->itemsColorWidget->setVisible(true);
        ui->itemsColorLineEdit->setText(
            parameterSettings->itemStaticColor().name(QColor::HexArgb));
    }
    else {
    ...
    ui->outOfRangeCheckBox->setChecked(
        parameterSettings->outOfRangeColorEnabled());
    ui->outOfRangeColorLineEdit->setColor(
        parameterSettings->outOfRangeColorSettingsRef().color());
    ui->outOfRangeColorLineEdit->setMode(
        parameterSettings->outOfRangeColorSettingsRef().mode());
    ui->outOfRangeColorModeComboBox->setCurrentIndex(
        parameterSettings->outOfRangeColorSettingsRef().mode() - 1);

    ui->lowThresholdsTableWidget->clear();
    QMapIterator<double, ColorSettings> itLow(
        parameterSettings->thresholdsSettingsRef().lowThresholds());
    while (itLow.hasNext()) {
        itLow.next();

        auto *newLine = new ColorLineEdit();
    ...
    QMapIterator<double, ColorSettings> itHigh(
        parameterSettings->thresholdsSettingsRef().highThresholds());
    while (itHigh.hasNext()) {
        ...
        btn_edit->setIcon(QIcon(QPixmap(":/icons8_cancel_50px.png")));
        connect(btn_edit, &QToolButton::clicked, this,
                &ElementPropertiesWidget::removeHighThreshold_clicked);
    ...
    QMapIterator<qlonglong, ColorSettings> itStateColor(
        parameterSettings->statesSettingsRef().statesColor());
    while (itStateColor.hasNext()) {
        itStateColor.next();

        auto *item = new QTableWidgetItem(QString::number(itStateColor.key()));
        item->setTextAlignment(Qt::AlignCenter);

        auto *newLine2 = new ColorLineEdit();
        ...
        newLine2->setText(
            parameterSettings->statesSettingsRef().statesText().value(
                itStateColor.key()));

        auto *btn_edit = new QToolButton();
        btn_edit->setIcon(QIcon(QPixmap(":/icons8_cancel_50px.png")));
        connect(btn_edit, &QToolButton::clicked, this,
                &ElementPropertiesWidget::removeState_clicked);
    ...
    if (parameterSettings->graphLineStyle() == QCPGraph::lsImpulse)
        ...
        ui->graphLineStyleComboBox->setCurrentIndex(
            parameterSettings->graphLineStyle());
    ui->itemsThresholdsCheckBox->setChecked(
        parameterSettings->itemsThresholdsVisible());

    ui->curveTracerCheckBox->setChecked(
        parameterSettings->curveTracerVisible());
}

void ElementPropertiesWidget::updateParameterSettings(
    ParameterConfiguration::Ptr parameterSettings)
{
    ...
    parameterSettings->defaultColorSettingsRef().setColor(
        ui->colorLineEdit->color());
    parameterSettings->defaultColorSettingsRef().setMode(
        ui->colorLineEdit->mode());

    parameterSettings->setOutOfRangeColorEnabled(
        ui->outOfRangeCheckBox->isChecked());
    parameterSettings->outOfRangeColorSettingsRef().setColor(
        ui->outOfRangeColorLineEdit->color());
    parameterSettings->outOfRangeColorSettingsRef().setMode(
        ui->outOfRangeColorLineEdit->mode());
    ...
    if (mThresholdsModified) {
        ...
        for (int i = 0; i < ui->highThresholdsTableWidget->rowCount(); i++) {
            auto *w = qobject_cast<ColorLineEdit *>(
                ui->highThresholdsTableWidget->cellWidget(i, 1));
            if (w) {
                double val = w->text().toDouble();
                ColorSettings cs;
                cs.setMode(w->mode());
                cs.setColor(w->color());
                parameterSettings->thresholdsSettingsRef().insertHighThreshold(
                    val, cs);
            }
        }
        ...
        for (int i = 0; i < ui->lowThresholdsTableWidget->rowCount(); i++) {
            auto *w = qobject_cast<ColorLineEdit *>(
                ui->lowThresholdsTableWidget->cellWidget(i, 1));
            if (w) {
                double val = w->text().toDouble();
                ColorSettings cs;
                cs.setMode(w->mode());
                cs.setColor(w->color());
                parameterSettings->thresholdsSettingsRef().insertLowThreshold(
                    val, cs);
            }
        }
    }
    ...
    if (mStatesModified) {
        ...
        for (int i = 0; i < ui->statesTableWidget->rowCount(); i++) {
            auto *cl = qobject_cast<ColorLineEdit *>(
                ui->statesTableWidget->cellWidget(i, 2));
            if (cl) {
                qlonglong val =
                    ui->statesTableWidget->item(i, 1)->text().toLongLong();

                ColorSettings cs;
                cs.setMode(cl->mode());
                cs.setColor(cl->color());

                parameterSettings->statesSettingsRef().addState(true, val,
                                                                cl->text(), cs);
            }
        }
    }
    ...
    if (mBitfieldsModified) {
        for (int i = 0; i < 32; i++) {
            auto *check = qobject_cast<QCheckBox *>(
                ui->bitfieldsTableWidget->cellWidget(31 - i, 0));
            if (check) {
                parameterSettings->bitfieldsSettingsRef().setBitLogic(
                    i, check->isChecked());
            }
            parameterSettings->bitfieldsSettingsRef().setBitDescription(
                i, ui->bitfieldsTableWidget->item(31 - i, 1)->text());
        }
    }

    parameterSettings->setScatterShape(QCPScatterStyle::ScatterShape(
        ui->scatterStyleComboBox->currentIndex()));
    parameterSettings->setScatterSize(ui->scatterSizeSpinBox->value());
    parameterSettings->setPenWidth(ui->lineWidthSpinBox->value());

    parameterSettings->setGraphBrush(ParameterConfiguration::BrushStyle(
        ui->brushStyleComboBox->currentIndex()));
    parameterSettings->setGraphLineStyle(
        QCPGraph::LineStyle(ui->graphLineStyleComboBox->currentIndex()));
    parameterSettings->setItemsThresholdsVisible(
        ui->itemsThresholdsCheckBox->isChecked());

    parameterSettings->setCurveTracerVisible(
        ui->curveTracerCheckBox->isChecked());

    parameterSettings->setItemColorMode(ParameterConfiguration::ItemColor(
        ui->itemsColorComboBox->currentIndex()));
    parameterSettings->setItemStaticColor(ui->itemsColorLineEdit->text());
}

void ElementPropertiesWidget::on_colorModeComboBox_currentIndexChanged(
    int index)
{
    ui->colorLineEdit->setMode(ColorSettings::ColorMode(index + 1));
}

void ElementPropertiesWidget::on_defaultColorToolButton_clicked()
{
    const auto color = QColorDialog::getColor(ui->colorLineEdit->color(), this,
...
void ElementPropertiesWidget::on_validRangeCheckBox_stateChanged(int arg1)
{
...
void ElementPropertiesWidget::on_defaultColorToolButton_2_clicked()
{
    const auto color = QColorDialog::getColor(
        ui->outOfRangeColorLineEdit->color(), this, QString("Color picker"),
        QColorDialog::ShowAlphaChannel);
    if (color.isValid()) {
...
void ElementPropertiesWidget::
    on_outOfRangeColorModeComboBox_currentIndexChanged(int index)
{
    ui->outOfRangeColorLineEdit->setMode(ColorSettings::ColorMode(index + 1));
}

void ElementPropertiesWidget::on_listWidget_currentRowChanged(int currentRow)
{
    Q_UNUSED(currentRow)
    ...
    if (ui->listWidget->currentItem()) {
        if (myOptions.contains(ui->listWidget->currentItem()->text()))
            ui->propertiesStackedWidget->setCurrentIndex(
                myOptions.indexOf(ui->listWidget->currentItem()->text()));
    }
}

void ElementPropertiesWidget::on_connectedCheckBox_stateChanged(int arg1)
{
    emit connectProperties(arg1 == Qt::Checked);
}

void ElementPropertiesWidget::setEditionMode(const EditionMode &mode)
{
...
void ElementPropertiesWidget::on_outOfRangeCheckBox_stateChanged(int arg1)
{
    ui->outOfRangeColorWidget->setEnabled(arg1);
    ui->defaultColorToolButton_2->setEnabled(arg1);
}

void ElementPropertiesWidget::removeHighThreshold_clicked()
{
    for (int i = 0; i < ui->highThresholdsTableWidget->rowCount(); i++) {
        if (ui->highThresholdsTableWidget->cellWidget(i, 0) ==
            QObject::sender()) {
...
void ElementPropertiesWidget::removeLowThreshold_clicked()
{
    for (int i = 0; i < ui->lowThresholdsTableWidget->rowCount(); i++) {
        if (ui->lowThresholdsTableWidget->cellWidget(i, 0) ==
            QObject::sender()) {
...
void ElementPropertiesWidget::removeState_clicked()
{
...
void ElementPropertiesWidget::setPropertiesMode(
...
void ElementPropertiesWidget::on_itemsColorComboBox_currentIndexChanged(
    int index)
{
    ui->itemsColorWidget->setVisible(index == 3);
}

void ElementPropertiesWidget::on_itemsColorToolButton_clicked()
{
    const auto color = QColorDialog::getColor(ui->itemsColorLineEdit->color(),
                                              this, QString("Color picker"),
                                              QColorDialog::ShowAlphaChannel);
    if (color.isValid()) {
...
void ElementPropertiesWidget::on_statesTableWidget_cellDoubleClicked(int row,
                                                                     int column)
{
    Q_UNUSED(column)
    auto *lineEdit = qobject_cast<ColorLineEdit *>(
        ui->statesTableWidget->cellWidget(row, 2));
    ...
    if (lineEdit && itemText) {
        ...
        connect(&colorButton, &QPushButton::clicked, [&colorLineEdit]() {
            QColor color = QColorDialog::getColor(
                colorLineEdit.color(), nullptr, QString("Color picker"),
                QColorDialog::ShowAlphaChannel);
            if (color.isValid()) {
        ...
        connect(&modeCombobox, &QComboBox::currentIndexChanged,
                [&colorLineEdit](int index) {
        ...
        if (res == QDialog::Accepted) {
            mStatesModified = true;
            int rowCount = 0;
            for (rowCount = 0; rowCount < ui->statesTableWidget->rowCount();
                 rowCount++) {
                auto *cl = qobject_cast<ColorLineEdit *>(
                    ui->statesTableWidget->cellWidget(rowCount, 2));
                if (cl) {
                    qlonglong val = ui->statesTableWidget->item(rowCount, 1)
                                        ->text()
                                        .toLongLong();
                    if (valueSpinBox.value() > val) {
                        auto *item = new QTableWidgetItem(
                            QString::number(valueSpinBox.value()));
                        item->setTextAlignment(Qt::AlignCenter);
                        ...
                        auto *btn_edit = new QToolButton();
                        btn_edit->setIcon(
                            QIcon(QPixmap(":/icons8_cancel_50px.png")));
                        connect(btn_edit, &QToolButton::clicked, this,
                                &ElementPropertiesWidget::removeState_clicked);

                        ui->statesTableWidget->insertRow(rowCount);
                        ui->statesTableWidget->setCellWidget(rowCount, 0,
                                                             btn_edit);
                        ui->statesTableWidget->setItem(rowCount, 1, item);
                        ui->statesTableWidget->setCellWidget(rowCount, 2,
                                                             newLine);
                        ui->statesTableWidget->resizeColumnToContents(0);
                    ...
                    if (valueSpinBox.value() == val) {
                        ui->statesTableWidget->item(rowCount, 1)
                            ->setText(QString::number(valueSpinBox.value()));
            ...
            auto *item =
                new QTableWidgetItem(QString::number(valueSpinBox.value()));
            item->setTextAlignment(Qt::AlignCenter);
...
void ElementPropertiesWidget::on_editPropPushButton_clicked()
{
    auto label = ui->labelLineEdit->text();
    auto descr = ui->descriptionLineEdit->text();
    if (mProject) {
        ParameterConfiguration::Ptr parameterSettings =
            mProject->parameterSettings(std::move(label), std::move(descr));
        if (parameterSettings) {
            QDialog dial;
            dial.setWindowTitle("Edit Parameter Configuration");
            QVBoxLayout layout;
            ElementPropertiesWidget w(&dial);
            w.setEditionMode(ElementPropertiesWidget::emEdition);
            ...
            connect(&buttonBox, &QDialogButtonBox::rejected, &dial,
                    &QDialog::reject);
            connect(&buttonBox, &QDialogButtonBox::accepted,
                    [&dial, &w, parameterSettings]() {
                        w.updateParameterSettings(parameterSettings);
                        if (!parameterSettings->label().isEmpty() &&
                            !parameterSettings->description().isEmpty()) {
                            dial.accept();
                        }
                        else {
                            QMessageBox::warning(
                                &dial, "Properties",
                                "Error : incomplete parameter (label/descr)");
                            return;
                        }
                    });
...
void ElementPropertiesWidget::setProject(QSharedPointer<Project> project)
{
    mProject = project;
}

void ElementPropertiesWidget::createSharedConfiguration()
{
    if (mCurrentSettings) {
        if (!mCurrentSettings->label().isEmpty()) {
            ...
            if (configurations.contains(mCurrentSettings->label())) {
                ...
                auto descriptions =
                    configurations.value(mCurrentSettings->label());
                for (auto descr : descriptions.keys()) items.append(descr);

                bool ok;
                QString itemText = QInputDialog::getItem(
                    nullptr, tr("Create/Edit Configurations:"),
                    QString(
                        ...
                        .arg(mCurrentSettings->label()),
                    items, 0, false, &ok);

                if (ok && !itemText.isEmpty()) {
                    auto descr = itemText;
                    auto target = mProject->parameterSettings(
                        mCurrentSettings->label(), descr);
                    if (target) {
                        QTemporaryFile tmpFile;
                        QSettings settings(tmpFile.fileName(),
                                           QSettings::IniFormat);
                        mCurrentSettings->save(&settings,
                    ...
                    connect(&buttonBox, &QDialogButtonBox::accepted,
                            [&dial, &labelLineEdit, &descrLineEdit, this]() {
                                if (!labelLineEdit.text().isEmpty() &&
                                    !descrLineEdit.text().isEmpty()) {
                                    QExplicitlySharedDataPointer<
                                        ParameterConfiguration>
                                        parameterSettings =
                                            mProject->parameterSettings(
                                                labelLineEdit.text(),
                                                descrLineEdit.text());
                                    if (parameterSettings) {
                                        QMessageBox::warning(
                                            &dial, "Properties duplication",
                    ...
                    connect(&buttonBox, SIGNAL(rejected()), &dial,
                            SLOT(reject()));
                    ...
                    if (res == QDialog::Accepted) {
                        ...
                        mCurrentSettings->saveToFile(
                            mProject->paramSettingsPath());
...
void ElementPropertiesWidget::on_addStateToolButton_clicked()
{
    QDialog dial;
    ...
    connect(&colorButton, &QPushButton::clicked, [&colorLineEdit]() {
        QColor color = QColorDialog::getColor(colorLineEdit.color(), nullptr,
                                              QString("Color picker"),
                                              QColorDialog::ShowAlphaChannel);
        if (color.isValid()) {
            colorLineEdit.setColor(color);
    ...
    if (res == QDialog::Accepted) {
        ...
        for (rowCount = 0; rowCount < ui->statesTableWidget->rowCount();
             rowCount++) {
            ColorLineEdit *cl = qobject_cast<ColorLineEdit *>(
                ui->statesTableWidget->cellWidget(rowCount, 2));
            if (cl) {
                qlonglong val = ui->statesTableWidget->item(rowCount, 1)
                                    ->text()
...
void ElementPropertiesWidget::on_addHighThresholdToolButton_clicked()
{
    QDialog dial;
    ...
    if (res == QDialog::Accepted) {
        mThresholdsModified = true;
        int rowCount = 0;
        for (rowCount = 0; rowCount < ui->highThresholdsTableWidget->rowCount();
             rowCount++) {
            ColorLineEdit *w = qobject_cast<ColorLineEdit *>(
                ui->highThresholdsTableWidget->cellWidget(rowCount, 1));
            if (w) {
                double val = w->text().toDouble();
                if (valueSpinBox.value() > val) {
                    ColorLineEdit *newLine = new ColorLineEdit();
                    ...
                    btn_edit->setIcon(
                        QIcon(QPixmap(":/icons8_cancel_50px.png")));
                    connect(
                        btn_edit, &QToolButton::clicked, this,
                        &ElementPropertiesWidget::removeHighThreshold_clicked);

                    ui->highThresholdsTableWidget->insertRow(rowCount);
                    ui->highThresholdsTableWidget->setCellWidget(rowCount, 0,
                                                                 btn_edit);
                    ui->highThresholdsTableWidget->setCellWidget(rowCount, 1,
                                                                 newLine);
                    ui->highThresholdsTableWidget->resizeColumnToContents(0);
                    return;
                }
...
void ElementPropertiesWidget::on_addLowThresholdToolButton_clicked()
{
    QDialog dial;
    ...
    if (res == QDialog::Accepted) {
        mThresholdsModified = true;
        int rowCount = 0;
        for (rowCount = 0; rowCount < ui->lowThresholdsTableWidget->rowCount();
             rowCount++) {
            ColorLineEdit *w = qobject_cast<ColorLineEdit *>(
                ui->lowThresholdsTableWidget->cellWidget(rowCount, 1));
            if (w) {
                double val = w->text().toDouble();
                if (valueSpinBox.value() > val) {
                    ColorLineEdit *newLine = new ColorLineEdit();
                    ...
                    ui->lowThresholdsTableWidget->setCellWidget(rowCount, 0,
                                                                btn_edit);
                    ui->lowThresholdsTableWidget->setCellWidget(rowCount, 1,
                                                                newLine);
                    ui->lowThresholdsTableWidget->resizeColumnToContents(0);
                    return;
                }
...
void ElementPropertiesWidget::on_highThresholdsTableWidget_cellDoubleClicked(
    int row, int column)
{
    Q_UNUSED(column)
    auto *lineEdit = qobject_cast<ColorLineEdit *>(
        ui->highThresholdsTableWidget->cellWidget(row, 1));
    if (lineEdit) {
        QDialog dial;
        ...
        if (res == QDialog::Accepted) {
            ...
            int rowCount = 0;
            for (rowCount = 0;
                 rowCount < ui->highThresholdsTableWidget->rowCount();
                 rowCount++) {
                ColorLineEdit *w = qobject_cast<ColorLineEdit *>(
                    ui->highThresholdsTableWidget->cellWidget(rowCount, 1));
                if (w) {
                    ...
                    if (valueSpinBox.value() > val) {
                        ...
                        connect(btn_edit, &QToolButton::clicked, this,
                                &ElementPropertiesWidget::
                                    removeHighThreshold_clicked);

                        ui->highThresholdsTableWidget->insertRow(rowCount);
                        ui->highThresholdsTableWidget->setCellWidget(
                            rowCount, 0, btn_edit);
                        ui->highThresholdsTableWidget->setCellWidget(
                            rowCount, 1, newLine);
                        ui->highThresholdsTableWidget->resizeColumnToContents(
                            0);
...
void ElementPropertiesWidget::on_lowThresholdsTableWidget_cellDoubleClicked(
    int row, int column)
{
    Q_UNUSED(column)
    auto *lineEdit = qobject_cast<ColorLineEdit *>(
        ui->lowThresholdsTableWidget->cellWidget(row, 1));
    if (lineEdit) {
        QDialog dial;
        ...
        if (res == QDialog::Accepted) {
            ...
            int rowCount = 0;
            for (rowCount = 0;
                 rowCount < ui->lowThresholdsTableWidget->rowCount();
                 rowCount++) {
                ColorLineEdit *w = qobject_cast<ColorLineEdit *>(
                    ui->lowThresholdsTableWidget->cellWidget(rowCount, 1));
                if (w) {
...
ParameterConfiguration::Ptr ElementPropertiesWidget::currentSettings() const
{
