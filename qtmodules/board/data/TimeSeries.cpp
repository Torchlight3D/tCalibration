...
bool TimeSeries::valid() const { return mValid; }

void TimeSeries::write(double t, const DataValue value)
{
    if (mValid) {
        if (mSize == 1) {
...
void TimeSeries::write(double t, const QVector<DataValue> &values)
{
    if (mValid) {
...
{
    if (mValid) {
        if ((timestamps.size() == values.size()) && mSize == 1) {
            QMutexLocker locker(&mMutex);
...
void TimeSeries::read(bool unread, QVector<double> &timestamps,
                      QVector<DataValue> &values)
{
    if (mValid) {
        ...
        if (unread) {
            ...
            auto size = mWritePos > mReadPos ? mWritePos - mReadPos
                                             : mDepth + mWritePos - mReadPos;
