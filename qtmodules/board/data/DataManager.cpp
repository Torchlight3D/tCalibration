...
#include "LiveDataSourceInterface.h"
      ...
      mPlaybackEndValid(false)
{
    ...
    connect(&mParametersTimer, SIGNAL(timeout()), this,
            SIGNAL(parametersUpdated()));
    connect(this, SIGNAL(startParametersTimer()), &mParametersTimer,
            SLOT(start()));

    QSettings settings(QCoreApplication::applicationDirPath() +
                           QDir::separator() + QString("data_sources.profiles"),
                       QSettings::IniFormat);
...
QMap<QString, QString> DataManager::profiles() const { return mProfiles; }

QString DataManager::profileName() const { return mProfileName; }
...
{
    if (param && param->valid() && !param->name().isEmpty() &&
        param->depth() > 0) {
        ...
        if (!mLiveParametersIdByLabels.contains(param->name())) {
            if (mPlaybackParametersIdByLabels.contains(param->name())) {
                param->setParameterId(
                    mPlaybackParametersIdByLabels.value(param->name()));
            }
            else {
                mIndexParameters++;
            ...
            mLiveParametersIdByLabels.insert(param->name(),
                                             param->parameterId());
            ...
            //            logger()->warn()
            //                << "Live parameter already registered " <<
            //                param->name();
        }
    }
...
void DataManager::unregisterLiveParameter(
    const QSharedPointer<TimeSeries> &param)
{
    if (param) {
        QMutexLocker locker(&mMutex);
...
bool DataManager::registerPlaybackParameter(
    const QSharedPointer<TimeSeries> &param)
{
    if (param && param->valid() && !param->name().isEmpty() &&
        param->depth() > 0) {
        ...
        if (!mPlaybackParametersIdByLabels.contains(param->name())) {
            if (mLiveParametersIdByLabels.contains(param->name())) {
                param->setParameterId(
                    mLiveParametersIdByLabels.value(param->name()));
            }
            else {
                mIndexParameters++;
                param->setParameterId(mIndexParameters);
            ...
            mPlaybackParametersIdByLabels.insert(param->name(),
                                                 param->parameterId());
            mPlaybackParameterSourceNames.insert(param->name(),
                                                 param->sourceName());
...
void DataManager::unregisterPlaybackParameter(
    const QSharedPointer<TimeSeries> &param)
{
    if (param) {
        QMutexLocker locker(&mMutex);
...
void DataManager::updateLiveBoardData()
{
    for (int i = 0; i < mBoardParameterIds.size(); ++i) {
        if (mLiveParametersById.contains(mBoardParameterIds.at(i))) {
            ...
            mLiveParametersById[mBoardParameterIds.at(i)]->read(
            ...
            for (itTimestamps = timestamps.constBegin();
                 itTimestamps != timestamps.constEnd(); ++itTimestamps) {
                mLiveTimestamps[i][index] = *itTimestamps;
                index++;
            }
            index = 0;
            for (itValues = values.constBegin();
                 (itValues != values.constEnd()); ++itValues) {
...
QHash<quint32, QSharedPointer<TimeSeries>> DataManager::playbackParametersById()
    const
{
    return mPlaybackParametersById;
}

QHash<quint32, QSharedPointer<TimeSeries>> DataManager::liveParametersById()
    const
{
...
void DataManager::updatePlaybackBoardData()
{
    ...
    for (auto src : mPlaybackDataSources) {
        src->loadPlaybackData(boardParameters, mPlaybackTimestamps,
                              mPlaybackValues);
    }
}
...
bool DataManager::lastBoardData(quint32 paramId, QVector<DataValue> &values,
                                QVector<double> &timestamps)
{
    if (mLiveDataEnabled) {
        int pos = mBoardParameterIds.indexOf(paramId);
...
bool DataManager::lastBoardData(quint32 paramId, DataValue &value,
                                double &timestamp)
{
    if (mLiveDataEnabled) {
        int pos = mBoardParameterIds.indexOf(paramId);
        if (pos >= 0) {
            if (!mLiveValues[pos].empty() && mLiveTimestamps[pos].size()) {
        ...
        if (mPlaybackTimestamps.contains(paramId) &&
            ...
            auto itTimestamps =
                std::lower_bound(timestamps.begin(), timestamps.end(),
                                 mPlaybackTimestampSinceStartOfTheDay);
...
void DataManager::loadDataSources()
{
    ...
    for (int i = 0; i < profilesCount; i++) {
        ...
        if (name == mProfileName) {
            ...
            for (auto prop : playbackPropList) {
                playbackProperties.insert(prop,
                                          settings.value(prop).toString());
            }
            settings.endGroup();
    ...
    for (auto libPath : pathList) {
        ...
        for (const QString &filename :
             dataSrcDir.entryList(QDir::Files | QDir::Readable)) {
            QFileInfo fileinfo(filename);
            //            logger()->info() << "Loading data source " <<
            //            filename;

            QPluginLoader pluginLoader(dataSrcDir.absoluteFilePath(filename),
                                       this);

            QObject *plugin = pluginLoader.instance();
            if (plugin) {
                QSharedPointer<LiveDataSourceInterface> dataSourceLive =
                    QSharedPointer<LiveDataSourceInterface>(
                        qobject_cast<LiveDataSourceInterface *>(plugin));
                if (dataSourceLive) {
                    if (dataSourceLive->name() == liveSourceName) {
                        ...
                        for (auto it = liveProperties.begin();
                             it != liveProperties.end(); ++it) {
                            dataSourceLive->setProperty(
                                it.key().toStdString().c_str(), it.value());
                        }

                        mLiveDataSources.insert(dataSourceLive->name(),
                                                dataSourceLive);
                ...
                QSharedPointer<PlaybackDataSourceInterface> dataSourcePlayback =
                    QSharedPointer<PlaybackDataSourceInterface>(
                        qobject_cast<PlaybackDataSourceInterface *>(plugin));
                if (dataSourcePlayback) {
                    if (dataSourcePlayback->name() == playbackSourceName) {
                        ...
                        for (auto it = playbackProperties.begin();
                             it != playbackProperties.end(); ++it) {
                            dataSourcePlayback->setProperty(
                                it.key().toStdString().c_str(), it.value());
                        }

                        mPlaybackDataSources.insert(dataSourcePlayback->name(),
                                                    dataSourcePlayback);
...
QMap<QString, QSharedPointer<PlaybackDataSourceInterface>>
DataManager::playbackDataSources() const
{
    return mPlaybackDataSources;
}

QMap<QString, QSharedPointer<LiveDataSourceInterface>>
DataManager::liveDataSources() const
{
...
bool DataManager::liveDataEnabled() const { return mLiveDataEnabled; }

void DataManager::liveDataEnabled(bool live) { mLiveDataEnabled = live; }
...
void DataManager::setUseDataSourceAsReferenceClock(
    bool useDataSourceAsReferenceClock)
{
...
void DataManager::setReferenceTimeLiveDataSource(
    const QSharedPointer<LiveDataSourceInterface> &referenceTimeLiveDataSource)
{
    mReferenceTimeLiveDataSource = referenceTimeLiveDataSource;
}

QSharedPointer<LiveDataSourceInterface>
DataManager::referenceTimeLiveDataSource() const
{
