...
{
    for (int i = 0; i < mSubLayout->elementCount(); i++) mSubLayout->takeAt(i);
    mSubLayout->simplify();
...
bool SingleDisplay::headerVisible() const { return mHeaderVisible; }

void SingleDisplay::setHeaderVisible(bool visible)
{
    if (mHeaderVisible != visible) {
        mHeaderVisible = visible;
    }
}

bool SingleDisplay::valueVisible() const { return mValueVisible; }
