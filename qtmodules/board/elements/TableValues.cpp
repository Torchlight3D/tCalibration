...
#include "editor/ui_TableValuesEditor.h"
...
TableValuesEditor::TableValuesEditor(TableValues *display, QWidget *parent)
    : QDialog(parent), ui(new Ui::TableValuesEditor), mElement(display)
{
    ui->setupUi(this);
    ui->stackedWidget->setCurrentIndex(0);
    ui->listWidget->setCurrentRow(0);
    connect(ui->buttonBox->button(QDialogButtonBox::Apply),
            &QPushButton::clicked, this, &TableValuesEditor::updateElement);
    ...
    if (mElement) {
        ...
        ui->alternatingRowColorsCheckBox->setChecked(
            mElement->alternatingRowColors());
...
TableValuesEditor::~TableValuesEditor() { delete ui; }
...
void TableValuesEditor::updateElement()
{
    if (mElement) {
        ...
        mElement->setAlternatingRowColors(
            ui->alternatingRowColorsCheckBox->isChecked());
        mElement->setValueFormat(
            ValueStringFormat(ui->valueFormatComboBox->currentIndex()));
        mElement->setPrecision(ui->precisionSpinBox->value());
        mElement->rebuildElement(true);
    }
}

///------- TableValues starts from here
...
void TableValues::update(QCPLayoutElement::UpdatePhase phase)
{
    ...
    if (phase == upLayout) {
        auto rect = mEmptyElementBottom->outerRect().adjusted(
            mTableMargins.left(), mTableMargins.top(), -mTableMargins.right(),
            -mTableMargins.bottom());
        mTableWidget->resize(rect.size());
        mTableWidget->move(rect.topLeft());
    }
}
...
void TableValues::clearElement() { mTableWidget->setRowCount(0); }
...
void TableValues::loadSettings(QSettings *settings)
{
    ...
    mAlternatingRowColors =
        settings->value("AlternatingRowColors", true).toBool();
...
void TableValues::loadConfigurations(QSettings *settings)
{
    ...
    for (int i = 0; i < parametersCount(); i++) {
        ...
        if (dashParam) {
            ...
            mTableWidget->setItem(mTableWidget->rowCount() - 1, 1, unitItem);

            auto valueItem = new QTableWidgetItem("-");
            valueItem->setTextAlignment(Qt::AlignCenter);
            mTableWidget->setItem(mTableWidget->rowCount() - 1, 2, valueItem);
        }
    }
}
...
void TableValues::checkParameters()
{
    ...
    for (const auto &boardParam : mBoardParameters)
        boardParam->parameterConfiguration()->setPrecision(mPrecision);
}

QSharedPointer<BoardParameter> TableValues::addParameter(
...
void TableValues::addParameter(QSharedPointer<BoardParameter> boardParameter)
{
    ...
    if (mTableWidget->rowCount() == parametersMaxCount()) {
        mTableWidget->item(mTableWidget->rowCount() - 1, 0)
            ->setText(boardParameter->getDisplayedLabel());
        mTableWidget->item(mTableWidget->rowCount() - 1, 1)
            ->setText(boardParameter->getDisplayedUnit());
        ...
        auto unitItem =
            new QTableWidgetItem(boardParameter->getDisplayedUnit());
        unitItem->setTextAlignment(Qt::AlignCenter);
        mTableWidget->setItem(mTableWidget->rowCount() - 1, 1, unitItem);
...
bool TableValues::alternatingRowColors() const { return mAlternatingRowColors; }
...
bool TableValues::unitVisible() const { return mUnitVisible; }
...
ValueStringFormat TableValues::valueFormat() const { return mValueFormat; }

void TableValues::setValueFormat(ValueStringFormat valueFormat)
{
    mValueFormat = valueFormat;
}

int TableValues::precision() const { return mPrecision; }

void TableValues::setPrecision(int precision) { mPrecision = precision; }

QString TableValues::title() const { return mTitle; }

void TableValues::setTitle(const QString &title) { mTitle = title; }

bool TableValues::titleVisible() const { return mTitleVisible; }
