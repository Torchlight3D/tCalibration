...
#include "../utils/LayoutList.h"
#include "../utils/LayoutGrid.h"
#include "../widgets/AlarmConfigurationWidget.h"
      ...
      mHeaderText("Alarms")
{
    ...
    mListBorderPen = QPen(QApplication::palette().midlight().color(), 0,
...
void AlarmPanel::initialize(Board *dashboard)
{
    if (dashboard) {
        ...
        mMainLayout->setMargins({4, 4, 4, 4});
...
void AlarmPanel::clearElement() { delete mMainLayout; }
...
void AlarmPanel::edit()
{
    ...
    connect(
        &w, &AlarmConfigurationWidget::stateChanged, this,
        [&w, this](int state) {
            if (state == Qt::Checked) {
                w.setEditionMode(AlarmConfigurationWidget::emElementConnected);
                w.setAlarmConfig(mSharedAlarmConfiguration);
                ...
                w.setEditionMode(
                    AlarmConfigurationWidget::emElementDisconnected);
                w.setAlarmConfig(mExclusiveAlarmConfiguration);
                w.updateUi();
            }
        });

    connect(&autoRowHeightCheckBox, &QCheckBox::stateChanged, this,
            [&rowHeightSpinBox](int state) {
                rowHeightSpinBox.setEnabled(state != Qt::Checked);
            });

    dialLayout.addWidget(&w);

    QDialogButtonBox buttonBox(QDialogButtonBox::Ok | QDialogButtonBox::Apply |
    ...
    connect(buttonBox.button(QDialogButtonBox::Apply), &QPushButton::clicked,
            this,
            [this, &w, &autoRowHeightCheckBox, &rowHeightSpinBox,
             ...
             &headerTextLineEdit, &backgroundBox]() {
                ...
                setBackgroundMode(
                    BoardElement::BackgroundMode(backgroundBox.currentIndex()));
                mUpdatingConfig = false;
                rebuildElement(true);
            });
    ...
    for (int i = 0; i < mAlarmConfiguration->alarmsCount(); i++) {
        if (i < mDashParametersPrimary.count()) {
            if (mAlarmConfiguration->active().at(i)) {
                ...
                if (mDashParametersPrimary.at(i)
                        ->getTimeSeries()
                        .parameterId() > 0) {
                ...
                switch (mAlarmConfiguration->functions().at(i)) {
                    case Alarms::afGreaterThan:
                        if (aliveParam &&
                            mDashParametersPrimary.at(i)->getValueDouble() >
                                mAlarmConfiguration->functionArgs().at(i))
                            alarmSet = true;
                        break;
                    ...
                    case Alarms::afBitSet:
                        if (aliveParam)
                            alarmSet =
                                (1 ==
                                 ((mDashParametersPrimary.at(i)
                                           ->getValueBinaryWeight32() >>
                                       int(mAlarmConfiguration->functionArgs()
                                               .at(i)) &
                                   1)));
                        break;
                ...
                if (alarmSet) {
                    mTextElements.at(elementIndex)
                        ->setText(mAlarmConfiguration->messages().at(i));
                    mTextElements.at(elementIndex)
                        ->setTextColor(mAlarmConfiguration->colors().at(i));
                    mTextElements.at(elementIndex)
                    ...
                    if (mDashParametersSecondary.at(i)) {
                        if (mDashParametersSecondary.at(i)
                            ...
                            mTextElements.at(elementIndex)
                                ->setText(
                                    QString("   %1 : %2")
                                        .arg(mDashParametersSecondary.at(i)
                                                 ->getTimeSeries()
                                                 .name(),
                                             mDashParametersSecondary.at(i)
                                                 ->getValueString()));
                            mTextElements.at(elementIndex)
                                ->setTextColor(
                                    mAlarmConfiguration->colors().at(i));
                            mTextElements.at(elementIndex)->setVisible(true);
                            mTextElements.at(elementIndex)->setBoldText(true);
                            mListLayout->addElement(
                                mTextElements.at(elementIndex));
                            elementIndex++;
                            if (elementIndex == mTextElements.count())
                                return;
        ...
        if (boardParam) {
            ...
            if (id > 0) {
                if (mBoard->dataManager()) {
                    auto dataParam = mBoard->dataManager()->dataParameter(
                        boardParam->getTimeSeries().name());
                    if (dataParam) {
                        auto dataId = dataParam->parameterId();
                        if (dataId != id) {
                            mBoard->dataManager()->registerBoardParameterId(
                                dataId);
                            mBoard->dataManager()->unregisterBoardParameterId(
                                id);
...
void AlarmPanel::loadConfigurations(QSettings *settings)
{
    ...
    if (mAlarmConfigurationIsShared) {
        ...
        QExplicitlySharedDataPointer<Alarms> conf =
            mBoard->project()->alarmConfiguration(alarmName);
        if (conf) {
            addAlarm(conf);
        }
        else {
...
QSharedPointer<BoardParameter> AlarmPanel::addParameter(
...
void AlarmPanel::updateAlarmConfiguration()
{
    ...
    for (int i = 0; i < mAlarmConfiguration->alarmsCount(); i++) {
        QSharedPointer<BoardParameter> dashParamPrimary =
            BoardElement::addParameter(
                mAlarmConfiguration->watchedParameters().at(i));
        ...
        if (!mAlarmConfiguration->outputParameters().at(i).isEmpty()) {
            dashParamSecondary = BoardElement::addParameter(
                mAlarmConfiguration->outputParameters().at(i));
        }
        mDashParametersSecondary.append(dashParamSecondary);
    }
}

bool AlarmPanel::connected() const { return mAlarmConfigurationIsShared; }
...
bool AlarmPanel::autoRowHeight() const { return mAutoRowHeight; }
...
int AlarmPanel::rowHeight() const { return mRowHeight; }

void AlarmPanel::setRowHeight(int rowHeight) { mRowHeight = rowHeight; }

bool AlarmPanel::headerVisible() const { return mHeaderVisible; }
...
bool AlarmPanel::autoHeaderText() const { return mAutoHeaderText; }
...
QString AlarmPanel::headerText() const { return mHeaderText; }
