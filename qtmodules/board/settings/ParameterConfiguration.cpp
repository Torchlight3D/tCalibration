...
{
    // FIXME: recover removeSpecialChars()
    if (mSaveFilePath.isEmpty()) {
        ...
        mSaveFilePath = saveDirectory + QDir::separator() + name +
                        QString("_") + descr + QString(".param");
    }

    //    logger()->info()
    //        << QString("Saving ParameterConfiguration [%1][%2] to file [%3]")
    //               .arg(mLabel)
    //               .arg(mDescription)
    //               .arg(mSaveFilePath);
...
void ParameterConfiguration::save(QSettings *settings, ConfigurationMode mode)
{
    ...
    settings->setValue("DefaultColor",
                       mDefaultColorSettings.color().name(QColor::HexArgb));
    ...
    if (mode != cmState && mode != cmBitFields) {
        ...
        settings->setValue(
            "OutOfRangeColor",
            mOutOfRangeColorSettings.color().name(QColor::HexArgb));
        settings->setValue("OutOfRangeColorMode",
                           mOutOfRangeColorSettings.mode());
        ...
        settings->beginGroup("Thresholds");
        {
            settings->beginWriteArray("LowMapping");
            {
                ...
                QMapIterator<double, ColorSettings> itLow(
                    mThresholdsSettings.lowThresholds());
                while (itLow.hasNext()) {
                    itLow.next();

                    settings->setArrayIndex(lowCount);
                    settings->setValue(
                        "Color", itLow.value().color().name(QColor::HexArgb));
            ...
            settings->beginWriteArray("HighMapping");
            {
                ...
                QMapIterator<double, ColorSettings> itHigh(
                    mThresholdsSettings.highThresholds());
                while (itHigh.hasNext()) {
                    itHigh.next();

                    settings->setArrayIndex(highCount);
                    settings->setValue(
                        "Color", itHigh.value().color().name(QColor::HexArgb));
        ...
        settings->beginGroup("States");
        {
            settings->beginWriteArray("Mapping");
            {
                ...
                QMapIterator<qlonglong, ColorSettings> itStateColor(
                    mStatesSettings.statesColor());
                while (itStateColor.hasNext()) {
                    itStateColor.next();

                    settings->setArrayIndex(statesCount);
                    settings->setValue("Active",
                                       mStatesSettings.statesActive().value(
                                           itStateColor.key()));
                    settings->setValue(
                        "Color",
                        itStateColor.value().color().name(QColor::HexArgb));
                    settings->setValue("Mode", itStateColor.value().mode());
                    settings->setValue("Value", itStateColor.key());
                    settings->setValue(
                        "Text",
                        mStatesSettings.statesText().value(itStateColor.key()));
                    statesCount++;
                }
            }
            settings->endArray();
    ...
    if (mode == cmBitFields || mode == cmFull) {
        settings->beginGroup("Bitfields");
        {
            ...
            for (int i = 0; i < 32; i++) {
                ...
                settings->setValue("Logic",
                                   mBitfieldsSettings.bitLogics().at(i));
                settings->setValue("Description",
                                   mBitfieldsSettings.bitDescriptions().at(i));
            }
            settings->endArray();
        }
        settings->endGroup();
...
void ParameterConfiguration::load(QSettings *settings, ConfigurationMode mode)
{
    ...
    mDefaultColorSettings.setColor(
        QColor(settings->value("DefaultColor").toString()));
    mDefaultColorSettings.setMode(
        ColorSettings::ColorMode(settings->value("DefaultColorMode").toInt()));
    ...
    if (mode != cmState && mode != cmBitFields) {
        ...
        mOutOfRangeColorSettings.setColor(
            QColor(settings->value("OutOfRangeColor").toString()));
        mOutOfRangeColorSettings.setMode(ColorSettings::ColorMode(
            settings->value("OutOfRangeColorMode").toInt()));

        if (mode == cmFull) {
            settings->beginGroup("Plottable");

            setScatterShape(QCPScatterStyle::ScatterShape(
                settings->value("ScatterShape").toInt()));
            ...
            mGraphLineStyle =
                QCPGraph::LineStyle(settings->value("LineStyle").toInt());
            settings->endGroup();

            settings->beginGroup("Curve");
            mCurveLineStyle =
                QCPCurve::LineStyle(settings->value("LineStyle").toInt());
        ...
        settings->beginGroup("Thresholds");
        {
            ...
            int lowThresholdsCount = settings->beginReadArray("LowMapping");
            {
                for (int i = 0; i < lowThresholdsCount; i++) {
                    ...
                    ColorSettings cs;
                    cs.setMode(ColorSettings::ColorMode(
                        settings->value("Mode").toInt()));
                    cs.setColor(QColor(settings->value("Color").toString()));
                    mThresholdsSettings.insertLowThreshold(
                        settings->value("Value").toDouble(), cs);
                }
            }
            settings->endArray();
            ...
            int highThresholdsCount = settings->beginReadArray("HighMapping");
            {
                for (int i = 0; i < highThresholdsCount; i++) {
                    ...
                    cs.setMode(ColorSettings::ColorMode(
                        settings->value("Mode").toInt()));
                    cs.setColor(QColor(settings->value("Color").toString()));
                    mThresholdsSettings.insertHighThreshold(
                        settings->value("Value").toDouble(), cs);
                }
            }
            settings->endArray();
        ...
        settings->beginGroup("Bitfields");
        {
            int bits = settings->beginReadArray("Mapping");
            {
                for (int i = 0; i < bits; i++) {
                    settings->setArrayIndex(i);
                    mBitfieldsSettings.setBitLogic(
                        i, settings->value("Logic").toBool());
                    mBitfieldsSettings.setBitDescription(
                        i, settings->value("Description").toString());
                }
            }
            settings->endArray();
        }
...
int ParameterConfiguration::precision() const { return mPrecision; }
...
QString ParameterConfiguration::description() const { return mDescription; }
...
void ParameterConfiguration::setUserDefinedLabel(
    const QString &userDefinedLabel)
{
...
bool ParameterConfiguration::validRange() const { return mValidRange; }
...
double ParameterConfiguration::rangeMaximum() const { return mRangeMaximum; }
...
double ParameterConfiguration::rangeMinimum() const { return mRangeMinimum; }
...
QString ParameterConfiguration::label() const { return mLabel; }
...
void ParameterConfiguration::setOutOfRangeColorEnabled(
    bool outOfRangeColorEnabled)
{
    mOutOfRangeColorEnabled = outOfRangeColorEnabled;
}

States &ParameterConfiguration::statesSettingsRef() { return mStatesSettings; }
...
QString ParameterConfiguration::saveFilePath() const { return mSaveFilePath; }

double ParameterConfiguration::penWidth() const { return mPenWidth; }
...
void ParameterConfiguration::setItemColorMode(ItemColor itemColorMode)
{
...
void ParameterConfiguration::setItemsThresholdsVisible(
    bool itemsThresholdsVisible)
{
...
void ParameterConfiguration::setGraphLineStyle(QCPGraph::LineStyle style)
{
    if (style != mGraphLineStyle) {
        if (style == QCPGraph::lsStepLeft)
            mGraphLineStyle = QCPGraph::lsImpulse;
        else
            mGraphLineStyle = style;
        mModified = true;
...
void ParameterConfiguration::setScatterShape(
    QCPScatterStyle::ScatterShape shape)
{
    if (shape != mScatterShape) {
        mScatterShape = shape;
...
int ParameterConfiguration::scatterSize() const { return mScatterSize; }
...
void ParameterConfiguration::setCurveLineStyle(QCPCurve::LineStyle style)
{
    if (style != mCurveLineStyle) {
        mCurveLineStyle = style;
        mModified = true;
    }
}
