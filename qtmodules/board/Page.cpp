...
void Page::loadPageElements(Board *board)
{
    ...
    for (int i = 0; i < elementsCount; i++) {
        ...
        QRect elementRect =
            board->dashboardLayout()->loadElementRectSettings(&settings);
        settings.endGroup();

        auto *element = ElementFactory::Factory()->createElement(elementName);
        if (element) {
            ...
            if (board->dashboardLayout()->addElement(element, elementRect)) {
...
void Page::savePageElements(Board *board)
{
    ...
    for (int i = 0; i < board->dashboardLayout()->elementCount(); i++) {
        if (auto *el = qobject_cast<BoardElement *>(
                board->dashboardLayout()->elementAt(i))) {
            settings.setArrayIndex(i);
...
QString Page::name() const { return mName; }

void Page::setName(const QString &name)
{
    mName = name;
    mSettingsPath =
        mPageDirectory + QDir::separator() + mName + QString(".page");
}

QString Page::description() const { return mDescription; }
...
int Page::columnCount() const { return mColumnCount; }

void Page::setColumnCount(int columnCount) { mColumnCount = columnCount; }

int Page::rowCount() const { return mRowCount; }

void Page::setRowCount(int rowCount) { mRowCount = rowCount; }

int Page::singleElementColumnCount() const { return mSingleElementColumnCount; }
...
int Page::singleElementRowCount() const { return mSingleElementRowCount; }
...
QString Page::background() const { return mBackground; }
...
QString Page::pageDirectory() const { return mPageDirectory; }
