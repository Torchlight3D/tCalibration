...
BoardParameter::BoardParameter(const QSharedPointer<TimeSeries> &dataParameter,
                               Board *board)
    : mBoard(board),
      ...
      mParameterConfiguration(new ParameterConfiguration())
{
    ...
    mParameterConfiguration->defaultColorSettingsRef().setColor(
        mBoard->randomColor());
}
    ...
    if (mTimeSeries.parameterId() > 0) {
        switch (format) {
            ...
            case vsfHexaLsb16: {
                ...
                return QString("0x") +
                       QString("%1").arg(value, 4, 16, QChar('0')).toUpper();
            }
            case vsfHexaMsb16: {
                struct s_fToBits
                {
...
QString BoardParameter::getStateString() const
{
    ...
    if (mTimeSeries.parameterId() > 0) {
        if (mParameterConfiguration->statesSettingsRef().active()) {
            text = mParameterConfiguration->statesSettingsRef().text(
                mValue.toUint32());
        }
    }
    return text;
...
{
    return mParameterConfiguration->bitfieldsSettingsRef().bitDescriptions().at(
        bitNumber);
}

bool BoardParameter::getBitLogic(int bitNumber) const
{
    return mParameterConfiguration->bitfieldsSettingsRef().bitLogics().at(
        bitNumber);
}
...
{
    if (mTimeSeries.parameterId() > 0) {
        mBoard->dataManager()->lastBoardData(mTimeSeries.parameterId(), mValue,
                                             mTimestamp);
    }
}
...
void BoardParameter::processValueData()
{
    ...
    mForegroundColor =
        mParameterConfiguration->defaultColorSettingsRef().foregroundColor();
    mBackgroundBrush =
        mParameterConfiguration->defaultColorSettingsRef().backgroundBrush();

    if (mTimeSeries.parameterId() > 0) {
        bool validColor = false;
        double value = mValue.toDouble();
        if (mParameterConfiguration->validRange() &&
            mParameterConfiguration->outOfRangeColorEnabled()) {
            if (value < mParameterConfiguration->rangeMinimum() ||
                value > mParameterConfiguration->rangeMaximum()) {
                mColor = mParameterConfiguration->outOfRangeColorSettingsRef()
                             .color();
                mForegroundColor =
                    mParameterConfiguration->outOfRangeColorSettingsRef()
                        .foregroundColor();
                mBackgroundBrush =
                    mParameterConfiguration->outOfRangeColorSettingsRef()
                        .backgroundBrush();
        ...
        if (!validColor) {
            if (mParameterConfiguration->thresholdsSettingsRef().active()) {
                ColorSettings cs =
                    mParameterConfiguration->thresholdsSettingsRef()
                        .colorSettings(value, validColor);
                if (validColor) {
                    mColor = cs.color();
...
void BoardParameter::processStateData()
{
    ...
    if (mTimeSeries.parameterId() > 0) {
        ...
        if (mParameterConfiguration->statesSettingsRef().active()) {
            ColorSettings cs =
                mParameterConfiguration->statesSettingsRef().colorSettings(
                    mValue.toUint32(), validColor);
            if (validColor) {
                mColor = cs.color();
                mForegroundColor = cs.foregroundColor();
                mBackgroundBrush = cs.backgroundBrush();
...
void BoardParameter::saveParameterSettings(
    QSettings *settings, ParameterConfiguration::ConfigurationMode mode)
{
    settings->setValue("Connected", mParameterConfigurationIsShared);
    if (mParameterConfigurationIsShared) {
        settings->setValue("ConnectedParamLabel",
                           mSharedParameterConfiguration->label());
        settings->setValue("ConnectedProperties",
                           mSharedParameterConfiguration->description());
    }
    else {
...
void BoardParameter::loadParameterSettings(
    QSettings *settings, ParameterConfiguration::ConfigurationMode mode)
{
    ...
    if (mParameterConfigurationIsShared) {
        ...
        ParameterConfiguration::Ptr prop =
            mBoard->project()->parameterSettings(paramlabel, properties);
        if (prop) {
...
ParameterConfiguration::Ptr
BoardParameter::sharedParameterConfiguration() const
{
    return mSharedParameterConfiguration;
}

ParameterConfiguration::Ptr
BoardParameter::exclusiveParameterConfiguration() const
{
...
QColor BoardParameter::getColor() const { return mColor; }

QColor BoardParameter::getForegroundColor() const { return mForegroundColor; }

QBrush BoardParameter::getBackgroundBrush() const { return mBackgroundBrush; }
...
void BoardParameter::setExclusiveParameterConfiguration(
    const ParameterConfiguration::Ptr
...
void BoardParameter::setSharedParameterConfiguration(
    const ParameterConfiguration::Ptr
        &sharedParameterConfiguration,
    bool connected)
{
...
ParameterConfiguration::Ptr
BoardParameter::parameterConfiguration() const
{
